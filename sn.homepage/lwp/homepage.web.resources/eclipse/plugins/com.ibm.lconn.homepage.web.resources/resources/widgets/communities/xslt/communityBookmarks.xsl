...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="commBookMarkTitle"/>
	...
	<xsl:template match="/atom:feed">
		<xsl:if test="os:totalResults=0">
			...
			<div style="border-bottom: solid 1px #ddd;">
				<span style="font-size:1em;font-weight:bold;">
					<xsl:value-of select="$nobookmarks" />
				</span>
				<br />					
		...
		<xsl:for-each select="atom:entry">
			<div class="entry">
				...
				<div class="entryBody" style="margin: 0px 0px 0px 0px;">
					<h4>
						<span class="sprite">
							<img class="lconnSprite lconnSprite-iconDogear16" src="{$blankGif}">
								<xsl:attribute name="title"><xsl:value-of select="$commBookMarkTitle" /></xsl:attribute>
								<xsl:attribute name="alt"><xsl:value-of select="$commBookMarkTitle" /></xsl:attribute>
							</img>
							
							<span class="lotusAltText bidiAware"><xsl:value-of select="atom:title" /></span>
						</span>&#160;
						<a target="_blank">
							<xsl:attribute name="href">
