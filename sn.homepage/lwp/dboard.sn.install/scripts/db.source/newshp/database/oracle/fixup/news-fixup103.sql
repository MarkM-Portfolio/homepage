...
ALTER TABLE HOMEPAGE.NR_AS_SEEDLIST MODIFY UPDATE_DATE DEFAULT CURRENT_TIMESTAMP;
COMMIT;
...
CREATE INDEX HOMEPAGE.DISCOVERY_VIEW_COM 
	ON HOMEPAGE.NR_DISCOVERY_VIEW (IS_STORY_COMM, USE_IN_ROLLUP) TABLESPACE  NEWSINDEXTABSPACE;
COMMIT;	
...
ALTER TABLE HOMEPAGE.BOARD_ENTRIES
	ADD HTML_CONTENT 	CLOB 	LOB (HTML_CONTENT) STORE AS (TABLESPACE BOARDLOBTABSPACE STORAGE (INITIAL 1M) CHUNK 4000 NOCACHE NOLOGGING);
COMMIT;
...
--  [start indexes] NR_DISCOVERY_VIEW
CREATE INDEX HOMEPAGE.DISCOVERY_VIEW_RLL_IX ON HOMEPAGE.NR_DISCOVERY_VIEW (READER_ID, IS_VISIBLE, USE_IN_ROLLUP) TABLESPACE "NEWSINDEXTABSPACE";
COMMIT;
