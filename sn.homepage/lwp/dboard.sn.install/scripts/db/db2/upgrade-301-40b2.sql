...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: ++ REORCHK performed ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
commit;
...
--74281: migration: upgrade 301-40 scripts , SQL0204N error - HOMEPAGE.NR_COM_PER_STORIES_READER" is an undefined name.
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Main indexes dropped to improve performance migration ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
CREATE BUFFERPOOL BRD32KBP IMMEDIATE SIZE 500 AUTOMATIC PAGESIZE 32K;
...
CREATE LARGE TABLESPACE BOARD16TABSPACE 
	...
	USING (FILE 'BOARD16TABSPACE' 1000)
	EXTENTSIZE 16
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Duplicated records deleted from HP_UI table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Fixed source type names done ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
ALTER TABLE HOMEPAGE.PERSON
	...
	ADD MEMBER_TYPE SMALLINT NOT NULL DEFAULT 0
	ADD ORGANIZATION_ID VARCHAR(36)
	ADD COMMUNITY_INTERNAL_ONLY SMALLINT;
COMMIT;
...
ALTER TABLE HOMEPAGE.HP_UI
	...
	ADD WELCOME_NOTE SMALLINT DEFAULT 1;
COMMIT;
...
UPDATE HOMEPAGE.HP_UI SET LAST_ACTIONABLE_VISIT = CURRENT TIMESTAMP;
COMMIT;

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Unique index created on HOMEPAGE.HP_UI ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
----------------------------------------------------------------------
CREATE INDEX HOMEPAGE.TAB_INST_TAB_ID 
	ON HOMEPAGE.HP_TAB_INST (TAB_ID ASC);
COMMIT;
...
ALTER TABLE HOMEPAGE.WIDGET 
	ADD IS_GADGET SMALLINT DEFAULT 0
...
ALTER TABLE HOMEPAGE.NT_NOTIFICATION
	ADD NOTIFICATION_SOURCE_TYPE SMALLINT;

COMMIT;

reorg table homepage.nt_notification ALLOW NO ACCESS;
...
reorg table homepage.nr_news_saved ALLOW NO ACCESS; 
...
reorg table  HOMEPAGE.NR_NEWS_DISCOVERY ALLOW NO ACCESS;
...
-- partioned update
reorg table HOMEPAGE.NR_COMM_PERSON_STORIES ALLOW NO ACCESS;
runstats on table HOMEPAGE.NR_COMM_PERSON_STORIES AND INDEXES ALL;
COMMIT;
...
reorg table HOMEPAGE.NR_RESPONSES_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_PROFILES_STORIES ALLOW NO ACCESS;
runstats on table HOMEPAGE.NR_PROFILES_STORIES AND INDEXES ALL;
COMMIT;
...
reorg table HOMEPAGE.NR_ACTIVITIES_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_BLOGS_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_BOOKMARKS_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_FILES_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_FORUMS_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_WIKIS_STORIES ALLOW NO ACCESS;
COMMIT;
...
reorg table HOMEPAGE.NR_TAGS_STORIES ALLOW NO ACCESS;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: SOURCE_TYPE set on the STORIES tables ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
CREATE UNIQUE INDEX HOMEPAGE.NR_SN_ITEM_READ_UNQ 
	ON HOMEPAGE.NR_NEWS_STATUS_NETWORK (ITEM_ID ASC, READER_ID ASC);
...
-- NR_STORIES_CONTENT
------------------------------------------------
CREATE TABLE HOMEPAGE.NR_STORIES_CONTENT_40 (
...
DROP VIEW HOMEPAGE.STORIES_CONTENT_VIEW;
...
RENAME TABLE HOMEPAGE.NR_STORIES_CONTENT_40 TO NR_STORIES_CONTENT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_STORIES_CONTENT migrated to a bigger tablespace ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
CREATE UNIQUE INDEX HOMEPAGE.EMD_RESOURCE_PREF_UNQ 
	ON HOMEPAGE.EMD_RESOURCE_PREF (PERSON_ID ASC, RESOURCE_TYPE ASC);
COMMIT;
...
ALTER TABLE HOMEPAGE.EMD_TRANCHE
	ADD IS_LOCKED_DAILY SMALLINT NOT NULL WITH DEFAULT 0
	ADD IS_LOCKED_WEEKLY SMALLINT NOT NULL WITH DEFAULT 0
...
-- remove indexes:
----------------------------------------------------------------------
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.EMD_TRANCHE_INFO DATA CAPTURE NONE;
...
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.EMD_TRANCHE_INFO DATA CAPTURE CHANGES;
...
ALTER TABLE HOMEPAGE.EMD_EMAIL_PREFS
	ADD REPLYTO_ENABLED SMALLINT NOT NULL WITH DEFAULT 1;
...
reorg table HOMEPAGE.NR_COMM_PERSON_FOLLOW ALLOW NO ACCESS use NEWS4TMPTABSPACE;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: SOURCE_TYPE setted on the NR_STORIES table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
CREATE TABLE HOMEPAGE.NR_STORIES_40 (
	...
	ITEM_CORRELATION_AUTHOR_NAME VARCHAR (256),
	EVENT_SCOPE SMALLINT,
	RELATED_COMMUNITY_ID VARCHAR(36)
)
IN NEWS32TABSPACE;

CREATE VIEW HOMEPAGE.STORIES_VIEW AS (
SELECT  
	STORY_ID, EVENT_NAME, SOURCE, CONTAINER_ID, CONTAINER_NAME, CONTAINER_URL, ITEM_NAME,
	...
	R_TEXT_META_TEMPLATE, IS_COMMUNITY_STORY, ITEM_CORRELATION_NAME, SOURCE_TYPE 
...
SELECT * FROM HOMEPAGE.STORIES_VIEW WITH UR;
COMMIT;
...
ALLOW NO ACCESS COMMITCOUNT 2000	
INSERT INTO HOMEPAGE.NR_STORIES_40
	(	
	STORY_ID, EVENT_NAME, SOURCE, CONTAINER_ID, CONTAINER_NAME, CONTAINER_URL, ITEM_NAME,
	ITEM_URL, ITEM_ATOM_URL, ITEM_ID, ITEM_CORRELATION_ID, CREATION_DATE, BRIEF_DESC,
	ACTOR_UUID, EVENT_RECORD_UUID, TAGS, META_TEMPLATE, TEXT_META_TEMPLATE, R_META_TEMPLATE,
...
RENAME TABLE HOMEPAGE.NR_STORIES_40 TO NR_STORIES;

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_STORIES table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_STORIES ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- 73540: Migration: updating 301-40 Homepage database , SQL error "-204" ( undefined names at 8 Occurances against homepage.Readers)
...
-- Rename table
RENAME TABLE HOMEPAGE.NR_COMM_PERSON_STORIES TO NR_AGGREGATED_READERS;
COMMIT;

-- Adding two columns:
ALTER TABLE HOMEPAGE.NR_AGGREGATED_READERS
	ADD CATEGORY_READER_ID VARCHAR(36) DEFAULT ' ' NOT NULL
	ADD READER_ID VARCHAR(36)  DEFAULT ' ' NOT NULL
	ADD ROLLUP_ENTRY_ID VARCHAR(256)
	ADD USE_IN_ROLLUP SMALLINT
	ADD IS_NETWORK	SMALLINT
	ADD IS_FOLLOWER	SMALLINT
	ADD EVENT_TIME 	TIMESTAMP
	ADD IS_STORY_COMM SMALLINT
	ADD IS_BROADCAST SMALLINT
	ADD ORGANIZATION_ID VARCHAR(256)
	ADD ACTOR_UUID VARCHAR(256)
	ADD IS_VISIBLE SMALLINT DEFAULT 1 NOT NULL
	ADD ROLLUP_AUTHOR_ID VARCHAR (256);
COMMIT;

-- Increase the existing ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_AGGREGATED_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

reorg table HOMEPAGE.NR_AGGREGATED_READERS ALLOW NO ACCESS;
COMMIT;
runstats on table HOMEPAGE.NR_AGGREGATED_READERS AND INDEXES ALL;
COMMIT;


-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT COMM_PER_STORY_ID, COMM_PER_STORY_ID CATEGORY_READER_ID, COMM_PER_READER_ID READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_AGGREGATED_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(COMM_PER_STORY_ID, CATEGORY_READER_ID, READER_ID, USE_IN_ROLLUP) SELECT COMM_PER_STORY_ID, CATEGORY_READER_ID, READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(COMM_PER_STORY_ID, CATEGORY_READER_ID, READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_AGGREGATED_READERS (COMM_PER_STORY_ID, CATEGORY_READER_ID, READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;			

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_AGGREGATED_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_AGGREGATED_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_AGGREGATED_READERS 
	DROP COLUMN COMM_PER_STORY_ID DROP COLUMN COMM_PER_READER_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_AGGREGATED_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_AGGREGATED_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
ALTER TABLE HOMEPAGE.NR_RESPONSES_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT  FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_RESPONSES_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_RESPONSES_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;			

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_RESPONSES_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_RESPONSES_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_RESPONSES_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_RESPONSES_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_RESPONSES_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_PROFILES_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_PROFILES_READERS ALLOW NO ACCESS;
COMMIT;
runstats on table HOMEPAGE.NR_PROFILES_READERS AND INDEXES ALL;
COMMIT;


-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT  FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_PROFILES_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_PROFILES_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;			

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_PROFILES_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_PROFILES_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_PROFILES_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_PROFILES_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_PROFILES_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
FROM HOMEPAGE.NR_COMM_STORIES WITH UR;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_COMMUNITIES_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_COMMUNITIES_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_ACTIVITIES_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_ACTIVITIES_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_ACTIVITIES_READERS AND INDEXES ALL;
COMMIT;


-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT  FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_ACTIVITIES_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_ACTIVITIES_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;			

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_ACTIVITIES_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_ACTIVITIES_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_ACTIVITIES_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_ACTIVITIES_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_ACTIVITIES_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_BLOGS_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_BLOGS_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_BLOGS_READERS AND INDEXES ALL;
COMMIT;

-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT  FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_BLOGS_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_BLOGS_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;	

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_BLOGS_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_BLOGS_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_BLOGS_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_BLOGS_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_BLOGS_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_BOOKMARKS_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_FILES_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_FILES_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_FILES_READERS AND INDEXES ALL;
COMMIT;

-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT  FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_FILES_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_FILES_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;	

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_FILES_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

--ALTER TABLE HOMEPAGE.NR_FILES_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_FILES_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_FILES_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_FILES_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_FORUMS_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_FORUMS_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_FORUMS_READERS AND INDEXES ALL;
COMMIT;


-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_FORUMS_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_FORUMS_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;	

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data migrated to NR_FORUMS_READERS ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_FORUMS_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_FORUMS_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_FORUMS_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_FORUMS_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_WIKIS_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_WIKIS_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_WIKIS_READERS AND INDEXES ALL;
COMMIT;


-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT FOLLOWED_STORY_ID,  FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_WIKIS_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_WIKIS_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;	

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_WIKIS_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

--ALTER TABLE HOMEPAGE.NR_WIKIS_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_WIKIS_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_WIKIS_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data NR_WIKIS_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
...
-- Increase the exiting ITEM_ID to be 256
ALTER TABLE HOMEPAGE.NR_TAGS_READERS ALTER COLUMN ITEM_ID SET DATA TYPE VARCHAR(256);
COMMIT;

REORG TABLE HOMEPAGE.NR_TAGS_READERS ALLOW NO ACCESS;
COMMIT;
RUNSTATS ON TABLE HOMEPAGE.NR_TAGS_READERS AND INDEXES ALL;
COMMIT;

-- Moving data:
CREATE VIEW HOMEPAGE.READERS AS (SELECT FOLLOWED_STORY_ID, FOLLOWED_STORY_ID CATEGORY_READER_ID, 0 USE_IN_ROLLUP FROM HOMEPAGE.NR_TAGS_READERS);
COMMIT;

EXPORT TO data.tmp.ixf OF IXF METHOD N 		(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) SELECT FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP FROM HOMEPAGE.READERS WITH UR;
IMPORT FROM data.tmp.ixf OF IXF METHOD N 	(FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP) ALLOW NO ACCESS COMMITCOUNT 2000 INSERT_UPDATE INTO HOMEPAGE.NR_TAGS_READERS (FOLLOWED_STORY_ID, CATEGORY_READER_ID, USE_IN_ROLLUP);
COMMIT;

DROP VIEW HOMEPAGE.READERS;
COMMIT;	

SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_TAGS_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;

-- Dropping columns:

--ALTER TABLE HOMEPAGE.NR_TAGS_READERS DATA CAPTURE NONE;
ALTER TABLE HOMEPAGE.NR_TAGS_READERS 
	DROP COLUMN FOLLOWED_STORY_ID;
COMMIT;
-- fixing Reason code = "100" SQLSTATE=42997
--ALTER TABLE HOMEPAGE.NR_TAGS_READERS DATA CAPTURE CHANGES;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: NR_TAGS_READERS ready ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_STATUS_UPDATE_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_EXTERNAL_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_ACTIONABLE_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
reorg table HOMEPAGE.SR_GLOBAL_SAND_PROPS ALLOW NO ACCESS use TEMPSPACE1;
reorg indexes all for table HOMEPAGE.SR_GLOBAL_SAND_PROPS ALLOW NO ACCESS;
...
reorg table HOMEPAGE.SR_FILESCONTENT ALLOW NO ACCESS use TEMPSPACE1;
reorg indexes all for table HOMEPAGE.SR_FILESCONTENT ALLOW NO ACCESS;
COMMIT;
...
CREATE INDEX HOMEPAGE.SR_INDEX_DOCS_FRID_IDX ON  HOMEPAGE.SR_INDEX_DOCS (FILES_REF_ID ASC, SERVICE ASC, UPDATE_TIME DESC);
COMMIT;
...
reorg table HOMEPAGE.SR_INDEX_MANAGEMENT ALLOW NO ACCESS use TEMPSPACE1;
reorg indexes all for table HOMEPAGE.SR_INDEX_MANAGEMENT ALLOW NO ACCESS;
COMMIT;

RUNSTATS ON TABLE "HOMEPAGE"."SR_INDEX_MANAGEMENT" AND INDEXES ALL;
COMMIT;
...
EXPORT TO data.tmp.ixf OF IXF METHOD N 
	(	
		...
		EVENT_TIME, IS_STORY_COMM, IS_BROADCAST, ORGANIZATION_ID, ACTOR_UUID, IS_VISIBLE, ROLLUP_AUTHOR_ID
	)
SELECT * FROM HOMEPAGE.STATUS_UPDATE_READERS WITH UR;
COMMIT;
...
SELECT * FROM HOMEPAGE.AGGREGATED_READERS WITH UR;
COMMIT;
...
EXPORT TO data.tmp.ixf OF IXF METHOD N 
	(	
		...
		ITEM_ID, ITEM_UPDATE_DATE, ITEM_NAME, ITEM_URL
	)

SELECT * FROM HOMEPAGE.STORIES WITH UR;
COMMIT;
...
SELECT * FROM HOMEPAGE.READERS WITH UR;	
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_SAVED_READERS table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
EXPORT TO data.tmp.ixf OF IXF METHOD N 
	(	
		...
		CREATION_DATE,  STORY_ID, SOURCE_TYPE, USE_IN_ROLLUP, IS_NETWORK, IS_FOLLOWER, EVENT_TIME, IS_STORY_COMM, IS_BROADCAST, IS_VISIBLE
	)

SELECT * FROM HOMEPAGE.DISCOVERY WITH UR;
COMMIT;
...
SELECT CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP: Data imported in NR_DISCOVERY_VIEW table ' FROM HOMEPAGE.HOMEPAGE_SCHEMA;
COMMIT;
...
----------------------------------------------------------------------
---[START] ** SET IS_BROADCAST FOR TABLES: NR_DISCOVERY_VIEW, NR_PROFILES_VIEW,  NR_PROFILES_READERS, NR_SAVED_READERS 
----------------------------------------------------------------------
-- before to start we reorg the tables and we run the statisc. This should help performance.

-- NR_SAVED_READERS
reorg table HOMEPAGE.NR_SAVED_READERS ALLOW NO ACCESS;
commit;
reorg indexes all for table HOMEPAGE.NR_SAVED_READERS ALLOW NO ACCESS;
commit;
...
commit;
reorg indexes all for table HOMEPAGE.NR_DISCOVERY_VIEW ALLOW NO ACCESS;
commit;
...
reorg table HOMEPAGE.NR_PROFILES_READERS ALLOW NO ACCESS;
commit;
reorg indexes all for table HOMEPAGE.NR_PROFILES_READERS ALLOW NO ACCESS;
commit;
