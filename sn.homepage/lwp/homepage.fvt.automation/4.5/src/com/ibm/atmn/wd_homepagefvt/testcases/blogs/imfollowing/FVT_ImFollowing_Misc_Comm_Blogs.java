...
public class FVT_ImFollowing_Misc_Comm_Blogs extends FVT_BlogsMethods{
	...
	*/
	...
	public void testFollowPublicCommunityBlog() throws Exception {
			...
			assertTrue("Text not present!",driver.isTextPresent(" created the "+PublicBlogCommunity+" community blog."));
			
			filterBy("Blogs");
			
			Thread.sleep(2000);
			
			assertTrue("Text is present!",driver.isTextPresent(" created the "+PublicBlogCommunity+" community blog."));
	...
	public void testChangeAccess_PublicComm() throws Exception {
		...
		clickLink(FVT_CommunitiesObjects.ChangeAccessOption3);
		
		driver.getSingleElement("css=input[id='editCommunityName']").click();
			
		sleep(1000);
		
		driver.getFirstElement("css=[name='communityEditSubmit']").click();
		
		//clickLink(CommonObjects.SaveButton);
		
		sleep(1000);
		
		clickLink(FVT_CommunitiesObjects.LeftNavBlogs);
		...
		verifyImFollowingNewsStory(testUser2," created a blog entry named "+PublicBlogEntry2+" in the "+PublicBlogCommunity+" blog.","Blogs", true);
