...
public class FVT_ImFollowing_Misc_Blogs extends FVT_BlogsMethods{
	/*
	...
	@Test (dependsOnMethods = { "testCreateStandAloneBlogEntry" })
		public void testUnfollowFromNewsFeed() throws Exception {
			...
			String ActivitiesStory = "css=div.lotusPostContent:contains(' created a blog entry named "+StandaloneBlogEntry+" in the "+StandaloneBlogName+" blog.')";
			...
			smartSleep(FVT_HomepageObjects.StopFollowing3);
			builder.moveToElement((WebElement) driver.getSingleElement(ActivitiesStory).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_HomepageObjects.StopFollowing3).getBackingObject()).click().build().perform();
