...
public void testForumCreated_PublicComm() throws Exception {
		...
		driver.getFirstElement("css=[role='tab']:contains('Forums')").click();
		//Start a forum
...
public void testForumTopicReplyUpdated_PublicComm() throws Exception {
		...
		// Login to communities
		LoadComponent(CommonObjects.ComponentCommunities);
			Reporter.log("Load Component  "+CommonObjects.ComponentCommunities);
		//Login(CommonData.IC_LDAP_Username450, CommonData.IC_LDAP_Password450);
		Login(testUser1);
		
		assertTrue("Fail: Communities is not open", driver.isTextPresent("Communities"));	
		Reporter.log("PublicForumCommunity  == "+PublicForumCommunity +"  FVT_CommunitiesObjects.ImAnOwner "+FVT_CommunitiesObjects.ImAnOwner+"  FVT_CommunitiesObjects.LeftNavForums "+FVT_CommunitiesObjects.LeftNavForums);
		//Open Community
		openCommunity(PublicForumCommunity, FVT_CommunitiesObjects.ImAnOwner, FVT_CommunitiesObjects.LeftNavForums);
		
		//Open topic
		clickLink("link=" + PublicCommTopic);
		Reporter.log("Topic  "+PublicCommTopic);
		...
		Reporter.log("Looking for -->  updated the "+PublicCommTopic+" topic thread in the "+PublicCommForum+" forum.");
