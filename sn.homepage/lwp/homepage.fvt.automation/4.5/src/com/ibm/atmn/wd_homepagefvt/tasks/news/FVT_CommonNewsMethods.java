...
public class FVT_CommonNewsMethods extends CommonMethods{
	...
	public void verifyNewsStory(String NewsStory, String NavOption, String component, boolean Visible) throws Exception{
		...
		if(Visible==false){
			//Verify that the news story is there
			if(driver.isTextPresent(NewsStory)){
				Assert.fail("fail - NewsStory should not be visible");
			}
			}
			...
			if(!driver.isTextPresent(NewsStory)){
				Assert.fail("fail - NewsStory should be visible");
			}
	...
	public void verifyNewsStory(String NewsStory, String NavOption, String component, User user, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
		...
		else if (NavOption=="@Mentions"){
			//Click on the @Mentions link
	...
	public void verifyNewsStory_TwoFilters(User User, String NewsStory, String NavOption, String FirstComponent,String SecondComponent, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
		...
		while(itr.hasNext()){
			...
			if(Visible==false){
				//Verify that the news story is there
				if(driver.isTextPresent(User+NewsStory)){
					Assert.fail("fail - the NewsStory is here");
				}
				}
				...
				if(!driver.isTextPresent(NewsStory)){
					Assert.fail("fail- NewsStory is not here");
				}
	...
	public void verifyNewsStory_ThreeFilters(User User, String NewsStory, String NavOption, String FirstComponent,String SecondComponent,String ThirdComponent, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
	...
	public void verifyNewsStory(User User, String NewsStory, String NavOption, String component, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
	...
	public void verifyImFollowingNewsStory(User User,String NewsStory, String component, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
		...
		while(itr.hasNext()){
			...
			if(Visible==false){
				//Verify that the news story is there
				if(driver.isTextPresent(NewsStory)){
					Assert.fail("fail - News Story should not be visible");
				}
				}
				...
				if(!driver.isTextPresent(NewsStory)){
					Assert.fail("fail - News Story should be visible");
				}
	...
	*/
	...
	public void verifyImFollowingTagsNewsStory(User User,String NewsStory, String component, String Tagname, boolean Community, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
		...
		while(itr.hasNext()){
			...
			if(Visible==false){
				//Verify that the news story is there
				if(driver.isTextPresent(NewsStory)){
					Assert.fail("Fail - The news story should not be visible");
				}
				}
				...
				if(!driver.isTextPresent(NewsStory)){
					Assert.fail("Fail - The news story should be visible");
				}
	...
	public void verifyMyNotificationsNewsStory(User User, String NewsStory, String FilterByFor, String Component, boolean Visible) throws Exception{
		//Load the URL to the people view in Homepage
		LoadComponent(CommonObjects.ComponentNews);
	...
	public void verifyStatusUpdatesViews() throws Exception{
		...
		assertTrue("FAIL: Status Updates link missing!", driver.isTextPresent("Communities"));
	...
	public String statusUpdate_GlobalSharebox(String StatusUpdate) throws Exception{
		...
		//clickLink(Objects.SaveStatusUpdate);
		FormSaveButton(FVT_NewsObjects.PostStatusGlobal);
		
		return StatusUpdate;
			
	}
	
	public String statusUpdate_AnotherView(String StatusUpdate, String View) throws Exception{
		...
		if (View=="Discover"){
			...
			clickLink(FVT_HomepageObjects.Refresh_Saved);
		}
		else if (View=="Action Required"){
			clickLink(FVT_HomepageObjects.Refresh_ActionRequired);
		}
		else if (View=="My Notifications"){
			clickLink(FVT_HomepageObjects.Refresh_MyNotifications);
		}
		else if (View=="I'm Following"){
			clickLink(FVT_HomepageObjects.Refresh_ImFollowing);
		}
	...
	public String CommentOnStatus(String StatusUpdate) throws Exception {
		...
		String StatusComment = FVT_NewsData.StatusComment+CommonMethods.genDateBasedRandVal();
		...
		if (isTextPresent("An error occurred loading gadget resources.")){
	...
	public String CommentOnStatusNoFrame(String StatusUpdate) throws Exception {
	...
	public String CommentOnStatus(String StatusUpdate,String StatusComment) throws Exception {
		...
		builder.moveToElement((WebElement) driver.getFirstElement(StatusUpdate_).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_NewsObjects.Comment_CR1).getBackingObject()).click().build().perform();
	...
	public void deleteComment(String StatusComment) throws Exception {
		...
		//Reporter.log(StatusComment);
		
		String StatusComment2 = "css=div.lotusPostContent div.lotusPostDetails div.asLinkContainer:contains('"+StatusComment+"')";
	...
	public void deleteSecondComment(String StatusComment) throws Exception {
		...
		builder.moveToElement((WebElement) driver.getFirstElement(StatusComment2).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_NewsObjects.DeleteComment_Second).getBackingObject()).click().build().perform();
	...
	public void Open_EE(String StatusUpdate) throws Exception{
		//
		...
		for(i=0;i<10;i++)
		{
			...
			sleep(2000);
			switchToEE();
			if(driver.isElementPresent(FVT_NewsObjects.Repost)==true){
				//
				break;
			}
		//close ee	
			switchFromEE();
			driver.getFirstElement(FVT_NewsObjects.EE_Close).click();
			
		//open ee
			driver.getFirstElement("css=div.lotusPostContent div.lotusPostAction:contains("+StatusUpdate+")").click();
			
			if(driver.isTextNotPresent("An error occurred loading gadget resources.")){
	...
	public void SearchForUserByName(String User) throws Exception{
		/*
		 * Enter the Username in the search field and search for that user and 
		 * verify that the correct user is returned
		 */
	...
	public void FollowPerson( String User) throws Exception{
		...
		smartSleep(FVT_ProfilesObjects.FollowPerson);
		
		if(driver.isElementPresent(FVT_ProfilesObjects.FollowPerson)){
	...
	public boolean InviteToNetwork() throws Exception{
		...
		{
			if(driver.isElementPresent(FVT_ProfilesObjects.PendingInvite)){
				Pending = true;
			}else{
	...
	public void followTag(String Tagname) throws Exception{
		...
		smartSleep(FVT_HomepageObjects.TagInputField);
		
		//type in tag
		driver.getSingleElement(FVT_HomepageObjects.TagInputField).click();
		...
		// smartSleepText(ExpectedValue);
			
		if (driver.isTextPresent("The tag you attempted to add \""+Tagname+"\" already exists.")){	
	...
	public String replaceNewsStory(String NewsStory,String FirstComponentName,String SecondComponentName, String ThirdComponentName, String Username) throws Exception{
	...
	public void repostStatus(String StatusUpdate) throws Exception{
	...
	public void selectUser_peoplePicker(User user) throws Exception{
		...
		WebElement NewElement = wd.findElement(By.cssSelector("[id^='peoplePicker_popup']"));
	...
	public void selectUser_peoplePicker(String user) throws Exception{
	...
	public void multiFilter(String message, String FirstFilter, String SecondFilter, String ThirdFilter) throws Exception{
		
		/*	Method to deal with multiple filters when verifying news stories
		 * 
		 * 	Ensure message String variable is exactly as it appears in news story incl user name 
		 */
		
		//Default initial filter will be All
		assertTrue("Fail - NewsStory not visible", isTextPresent(message));
		
		//Filter by First component
		filterBy(FirstFilter);
		
		assertTrue("Fail - NewsStory not visible", isTextPresent(message));
		
		
		//Filter by second component
		...
		//Filter by third component
		if(ThirdFilter!=null){
			
			filterBy(ThirdFilter);
