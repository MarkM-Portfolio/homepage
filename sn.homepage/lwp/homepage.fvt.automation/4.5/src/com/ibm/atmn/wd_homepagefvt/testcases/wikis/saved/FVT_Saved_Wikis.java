...
public class FVT_Saved_Wikis extends FVT_WikisMethods {
	...
	String value = ""+genDateBasedRandVal();
	...
	public void testMarkingWikisStoriesAsSaved() throws Exception {
		...
		for(Element e:list){
			...
			if(we.getText().contains("created a wiki named "+FVT_WikisData.CI_Box_Public_Wiki)){
				e.hover();
				we.findElement(By.cssSelector("div.lotusPostMore ul.lotusInlinelist li.lotusFirst a")).click();
				break;		
			}
		}
		
		assertTrue("Fail: Link still there!!", we.getText().contains("Saved"));
		//assertTrue("Fail: Link still there!!", driver.isElementPresent("css=div.lotusPostMore ul.lotusInlinelist li.lotusFirst a:contains('Saved')"));
		...
		assertFalse("Fail: Link still there!!", driver.isElementPresent("css=a[title='Already Saved']"));
	...
	public void testMarkingWikisStoriesAsSavedPart2() throws Exception {
		...
		AddContentintheEditorforanExistingPage("  "+FVT_WikisData.New_Content_For_Public_Wiki+value);
		//AddContentintheEditorforanExistingPage("  "+FVT_WikisData.New_Content_For_Public_Wiki+value,"1");
		...
		VerifyAddedContentInHomepage(FVT_WikisData.New_Content_For_Public_Wiki+value);
		...
		driver.getFirstElement(WikiEditStory).hover();
		...
		for(Element e:list){
			...
			if(we.getText().contains("edited the wiki page "+FVT_WikisData.New_Peer_Page_For_Public_Wiki2+" in the "+FVT_WikisData.CI_Box_Public_Wiki+" wiki.")){
				we.findElement(By.cssSelector("div.lotusPostMore ul.lotusInlinelist li.lotusFirst a")).click();

				assertTrue("Fail: Link didnt change to save ", we.findElement(By.cssSelector("div.lotusPostMore ul.lotusInlinelist li.lotusFirst a")).getText().contains("Saved"));
				break;
			}
		}
	...
	@Test (dependsOnMethods = { "testMarkingWikisStoriesAsSavedPart3" })
	public void testMarkingCommentedWikisStoriesAsSaved() throws Exception {
		...
		String WikiStory = "css=div.lotusPostContent:contains('commented on the wiki page Welcome to "+FVT_WikisData.CI_Box_Public_Wiki2+" in the "+FVT_WikisData.CI_Box_Public_Wiki2+" wiki.')";
		...
		driver.getSingleElement("css=div.lotusPostContent:contains('commented on the wiki page Welcome to "+FVT_WikisData.CI_Box_Public_Wiki2+" in the "+FVT_WikisData.CI_Box_Public_Wiki2+" wiki.')").hover();
		...
		assertTrue("Fail: text is not present!!", driver.isTextPresent("commented on the wiki page Welcome to "+FVT_WikisData.CI_Box_Public_Wiki2+" in the "+FVT_WikisData.CI_Box_Public_Wiki2+" wiki."));

		assertTrue("Fail: Element is not present!!", driver.isElementPresent(FVT_HomepageObjects.RollUpComment+":contains("+FVT_WikisData.Comment_For_Public_Wiki+")"));
	...
	public void testMarkingCommentedWikisStoriesAsSavedPart2() throws Exception {
		...
		//Open wiki
		clickLink("link="+FVT_WikisData.CI_Box_Public_Wiki2);
		...
		driver.getSingleElement("css=div.lotusPostContent:contains('edited the wiki page "+FVT_WikisData.New_Peer_Page_For_Public_Wiki2+" in the "+FVT_WikisData.CI_Box_Public_Wiki2+" wiki.')").hover();
		...
		assertTrue("Fail: Element is not present!!", driver.getFirstElement(FVT_HomepageObjects.RollUpComment).isTextPresent(FVT_WikisData.Comment_For_Public_Wiki));
