...
public class FVT_Saved_Dogear extends FVT_DogearMethods {
	...
	public void testMarkingBookmarksStoriesAsSaved() throws Exception {
		...
		Reporter.log("Public bookmark name -> "+PUBLIC_BOOKMARK_NAME);
		...
		assertTrue("Fail: Link still there!!", driver.getFirstElement("css=div.lotusPostMore ul.lotusInlinelist li.lotusFirst a:nth(0)").isTextPresent("Saved"));
	...
	public void testMarkingBookmarksStoriesAsSavedPart2() throws Exception {
		...
		// Element might not appear on this page immediately, Page will be reloaded for additional minute to see if it appears  
		if(!driver.getFirstElement("css=div.activityStreamNewsItemContainer").isTextPresent(PUBLIC_BOOKMARK_NAME)){
			sleep(60000);
			driver.getFirstElement("css=a#_myNotifications").click();
			filterBy("Bookmarks");
		}
		...
		//assertTrue("Fail: Link still there!!", driver.isElementPresent("css=a[title='Already Saved']"));
		assertTrue("Fail: Link still there!!", driver.getFirstElement("css=div.lotusPostContent div.lotusPostMore ul.lotusInlinelist li.lotusFirst a").isTextPresent("Saved"));
