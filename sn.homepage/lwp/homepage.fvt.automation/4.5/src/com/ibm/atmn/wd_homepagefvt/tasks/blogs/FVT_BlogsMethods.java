...
public class FVT_BlogsMethods extends FVT_CommunitiesMethods {
	...
	public String CreateANewBlogEntry(String BlogEntryName) throws Exception {
		...
		Thread.sleep(2000);
		getActiveElement(FVT_BlogsObjects.BlogsNewEntryPost).click();
	...
	public void AddACommentToEntry() throws Exception {
		...
		driver.getFirstElement(FVT_BlogsObjects.BlogsAddACommentLink).hover();
		driver.getFirstElement(FVT_BlogsObjects.BlogsAddACommentLink).click();
		
		// Fill in the comment form
		//driver.typeNative(BlogsData.BlogsCommentText);
	...
	public void notifyAboutBlogEntry(User Username) throws Exception {
		...
		driver.getSingleElement(FVT_BlogsObjects.NOTIFY_TYPEAHEAD).type(Username.getLastName());
		
		driver.getSingleElement(FVT_BlogsObjects.NOTIFY_TYPEAHEAD_SEARCH).click();
		
		driver.getSingleElement(FVT_BlogsObjects.NOTIFY_TYPEAHEAD_IDENTIFIER+":contains('"+Username.getEmail()+"')").click();
	...
	public void assignUserRole(String Role, String Fullname, String Lastname) throws Exception {
		...
		//Type and select user from typeahead
		driver.getSingleElement(FVT_BlogsObjects.MEMBER_TYPEAHEAD).type(Lastname);
		
		driver.getSingleElement(FVT_BlogsObjects.MEMBER_TYPEAHEAD_SEARCH).click();
		
		driver.getFirstElement(FVT_BlogsObjects.MEMBER_TYPEAHEAD_IDENTIFIER+":contains('"+Fullname+"')").click();
		
		//Send Notification
		driver.getFirstElement(CommonObjects.SaveButton).click();
		
		assertTrue("Fail: User not added correctly!", driver.isTextPresent("User ["+Fullname+"] has been added as "+Role+" on this blog."));	
	...
	public void likeBlogComment_CR1(){
		
		clickLink(FVT_BlogsObjects.LIKE_BLOG_COMMENT_CR1);
		
		smartSleep(FVT_BlogsObjects.LIKE_BUTTON_POPUP_CR1);
		
		clickLink(FVT_BlogsObjects.LIKE_BUTTON_POPUP_CR1);
	}
