...
public class FVT_ActivitiesMethods extends FVT_CommunitiesMethods {
	...
	public void selectMemberFromDropdown(String userName) {
		...
		//driver.getSingleElement(FVT_ActivitiesObjects.selectedActivityUserIdentifier+":contains('"+userName+"')").click();
	...
	*/
	...
	public String addEntryToActivity_Basic(String ActivityName, FormInputHandler typist, boolean accessPrivate) {
		...
		//
		clickLink(FVT_ActivitiesObjects.New_Entry2);
		
		if(!isTextPresent("Title:"))
		{
			clickLink(FVT_ActivitiesObjects.New_Entry2);
		}
		...
		smartSleep(FVT_ActivitiesObjects.New_Entry_InputText_Title);
		
		driver.getSingleElement(FVT_ActivitiesObjects.New_Entry_InputText_Title).click();
	...
	*/
	...
	public String addToDoToActivity(String ToDoTitle, FormInputHandler typist, boolean accessPrivate, boolean assignToDo, User Username) {
		...
		//sel.keyPressNative(String.valueOf(KeyEvent.VK_ENTER));
		//sleep(1000);
		if(!isTextPresent("To Do:"))
		{
			clickLink(FVT_ActivitiesObjects.AddToDo);
		}
		
		smartSleep(FVT_ActivitiesObjects.ToDoMoreOptions);
		...
		if(assignToDo==true){
			...
			driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_FilterBox).clear();
			driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_FilterBox).type(Username.getDisplayName());
			clickLink(FVT_ActivitiesObjects.ToDo_Assign_CheckBox);
			}
	...
	public String addToDoToActivityTags(String ToDoTitle, String TagTitle, FormInputHandler typist, boolean accessPrivate, boolean assignToDo, User Username) {
		...
		if(TagTitle!=null){
			...
			driver.getSingleElement(FVT_ActivitiesObjects.ToDo_InputText_Tag).clear();
			typist.type(FVT_ActivitiesObjects.ToDo_InputText_Tag, TagTitle);
		}
		...
		if(assignToDo==true){
			...
			driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_Person).hover();
			clickLink(FVT_ActivitiesObjects.ToDo_Assign_Person);
			sleep(500);
			//sel.focus(FVT_ActivitiesObjects.ToDo_Assign_FilterBox);
			driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_FilterBox).clear();
	...
	public String addCommentToActivityEntry(FormInputHandler typist, boolean accessPrivate) {
		...
		sleep(2000);
		//Save
		clickLink(FVT_ActivitiesObjects.SaveComment);
		
		return description;
		
	}
	
	public String editCommentToActivityEntry(FormInputHandler typist, boolean accessPrivate) {
	...
	public void followActivity(String ActivityName, User User) {
	...
	//Method to count action required events present in badge and add figure to expected result
	public String countActionsRequired(User user, int expectedResult){
		...
		//Find badge and retrieve actions required number
		baseActions = driver.getFirstElement("xpath=//*[@id='lconn_homepage_as_badge_ActionRequiredBadge_0']").getText();
		
		//if no badge present assign it value of 0
		if(baseActions.equals("")){
			
			baseActions = "0";
		}
		
		//Convert Actions string to int and add required actions
		int base = Integer.parseInt(baseActions);
		
		base = base +expectedResult;
		
		//Convert total back to string - used as comparison later
