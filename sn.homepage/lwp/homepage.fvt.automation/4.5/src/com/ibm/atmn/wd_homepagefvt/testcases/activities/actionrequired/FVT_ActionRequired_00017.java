...
public class FVT_ActionRequired_00017 extends FVT_ActivitiesMethods {
	...
	public void testEdit_AssignedToDo() throws Exception {
		...
		// Click on To-DO
		driver.getFirstElement("css=[id^='activityPageNodeContainer'][id$='_miniTitle']").click();
		
		clickLink("css=[id^='activityPageNodeContainer'][id$='_editLink']");
		
		clickLink("css=a:contains('Replace')");
		
		driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_FilterBox).clear();
		driver.getSingleElement(FVT_ActivitiesObjects.ToDo_Assign_FilterBox).type(testUser2.getDisplayName());
		...
		//verify newsstory is deleted from user 1 stream
		verifyNewsStory(testUser1, ActionNewsStory, FVT_NewsData.ACTION_REQUIRED_VIEW, null, false);

		
		//verify newsstory exists on user 2 stream
		Login(testUser2);
