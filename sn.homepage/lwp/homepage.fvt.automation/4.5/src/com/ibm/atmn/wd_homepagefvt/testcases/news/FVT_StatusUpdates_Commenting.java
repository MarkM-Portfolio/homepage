...
public class FVT_StatusUpdates_Commenting extends FVT_CommunitiesMethods{
	...
	public void testStatusUpdateAll() throws Exception {
		...
		Reporter.log("CommentOnStatus  =   "+StatusUpdate);
		//Save status
		...
		//builder.moveToElement((WebElement) driver.getFirstElement(StatusUpdatePost).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_HomepageObjects.SaveThis_StatusUpdates).getBackingObject()).click().build().perform();
		//clickLink(FVT_HomepageObjects.SaveThis_StatusUpdates);
		//clickLink("css=div.lotusPostMore > ul.lotusInlinelist > li a:nth(1)");
	...
	public void testStatusUpdateCommunitiesIFollow_Part1() throws Exception {
		...
		PublicCommunity = CreateNewCommunity(FVT_CommunitiesData.PublicCommunityName+DateTimeStamp, FVT_CommunitiesData.CommunityHandle+DateTimeStamp, FVT_CommunitiesObjects.CommunityAccessOption1, "Members", FVT_CommunitiesObjects.CommunityMembersTypeAhead, testUser2.getDisplayName());
	...
	public void testStatusUpdateCommunitiesIFollow_Part3() throws Exception {
		...
		Reporter.log("Part3 StatusUpdate Before update : "+StatusUpdate);
		
		StatusUpdate = FVT_NewsData.UpdateStatus+CommonMethods.genDateBasedRandVal();
		
		Reporter.log("Part3 StatusUpdate After update : "+StatusUpdate);
		//verify sharebox displays
	...
	public void testStatusUpdateCommunitiesIFollow_Part4() throws Exception {
		...
		Reporter.log("status update   = "+StatusUpdate);
		...
		//builder.moveToElement((WebElement) driver.getFirstElement(StatusUpdatePost).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_HomepageObjects.SaveThis_StatusUpdates).getBackingObject()).click().build().perform();
		builder.moveToElement((WebElement) driver.getFirstElement(StatusUpdatePost).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_HomepageObjects.SaveThis_StatusUpdates).getBackingObject()).click().build().perform();
		//builder.moveToElement((WebElement) driver.getFirstElement(StatusUpdatePost).getBackingObject()).moveToElement((WebElement) driver.getFirstElement("link=Save this").getBackingObject()).click().build().perform();
