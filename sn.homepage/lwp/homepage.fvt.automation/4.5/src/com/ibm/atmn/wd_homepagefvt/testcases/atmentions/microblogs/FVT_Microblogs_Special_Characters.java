...
public class FVT_Microblogs_Special_Characters extends FVT_AtMentionsMethods{
	

	@Test
	public void UserId_Special_Characters() throws Exception {
		
		
		System.out.println("INFO: Start of test_At_Mentions_47_UserId_Special_Characters");
		
		//Create Array of Users
		String [][] users = new String [][]{{"Combo .@2", "Pr0bl3m()* Combo .@2","Combo .@2"},
		...
		driver.getSingleElement(FVT_AtMentionsObjects.EmbeddedSharebox).type(FVT_AtMentionsData.MentionUserPostPrivate);
		
		//Enter user to mention
		mentionMultipleUsers(User2Name, FVT_AtMentionsData.MentionUserPost, FVT_AtMentionsObjects.EmbeddedSharebox);
		...
		//Verify NewsStory
		String atMentionsNewsStory = replaceNewsStory(NewsStoryData.MENTIONED_YOU, null, null, User1Name);
		
		//Verify under ImFollowing All
		clickLink(FVT_HomepageObjects.ImFollowing);
		assertTrue("Fail - NewsStory not present under profile activity stream.", driver.isTextPresent(atMentionsNewsStory));
		
		//Verify under ImFollowing Status Updates
		filterBy(FVT_NewsData.STATUS_UPDATES);
		assertTrue("Fail - NewsStory not present under profile activity stream.", driver.isTextPresent(User1Name+" mentioned "+User2Name+" in a message."));
		
		//Logout
		LogoutAndClose();
		
		// Issue -> news story not appearing under my notifications - commented out
		//verifyMyNotificationsNewsStory(testUser2, atMentionsNewsStory, FVT_NewsData.FOR_ME, FVT_NewsData.STATUS_UPDATES, true);
		...
		System.out.println("INFO: End of test_@Mention22 MESSAGE DIRECTED IN A PUBLIC COMMUNITY STATUS UPDATE");
