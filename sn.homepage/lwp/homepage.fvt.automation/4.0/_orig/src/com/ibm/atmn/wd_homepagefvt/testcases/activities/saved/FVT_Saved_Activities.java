...
public class FVT_Saved_Activities extends FVT_ActivitiesMethods {
	...
	public void testMarkingActivitySaved_Part2() throws Exception {
			...
			String ActivitiesStory = "css=div.lotusPostContent:contains(' created a public activity named "+publicActivityName+".')";
			
			//builder.moveToElement((WebElement) BlogsStory).moveToElement((WebElement) SaveThis).click().perform();
			builder.moveToElement((WebElement) driver.getSingleElement(ActivitiesStory).getBackingObject()).moveToElement((WebElement) driver.getFirstElement(FVT_HomepageObjects.SaveThis).getBackingObject()).click().build().perform();
			...
			assertTrue("Fail: text is not present!!", driver.isTextPresent(" created a public activity named "+publicActivityName+"."));
	...
	public void testSavedActivities_CommentingAfterSaving_Part1() throws Exception {
			...
			publicActivityName2 = startAnActivity(FVT_ActivitiesData.Start_Public_Activity_InputText_Name_Data+genDateBasedRandVal(),typist,CommonData.IC_LDAP_UserName451_Typeahead);
