...
package com.ibm.atmn.homepagefvt.tasks.wikis;
...
import com.ibm.atmn.homepagefvt.appobjects.common.Initialize;
...
public class FVT_WikisMethods extends com.ibm.atmn.homepagefvt.tasks.communities.FVT_CommunitiesMethods{
		...
		for (int second = 0;; second++) {
			if (second >= 60) Assert.fail("Login Link has not appeared after 60 seconds");
				try {
					...
					if (sel.isVisible(WikisObjects.Login_Link))
	...
	public void createPrivateWikiWithUsers(String wikiTitle) throws Exception{
		...
		SelectMembershipRoleOnCreateWikiForm(WikisObjects.MembershipRolesUsersDropdown, Data.Reader_Role);	
		ActivateTypeAheadForMembersField(BrowserInfo, Data.Reader_Username_Typeahead451);
		selectMemberFromUserDropdown(Data.Reader_Username_Typeahead451);
	...
	public String typeInTextField_FVT(String textField, String textContent) throws Exception {
		//Type text provided in textfield identified
		sel.type(textField, textContent);
	...
	public void verifyNewHomePageUI(String WikiName, String UserFullName, String TagName) throws Exception {    
		...
		Assert.assertTrue("FAIL: Wiki name isn't visible", sel.isTextPresent(WikiName));
		Assert.assertTrue("FAIL: Breadcrumbs aren't visible", sel.isElementPresent(WikisObjects.All_Breadcrumb_Text));
		...
		//Verify tagging UI
		Assert.assertTrue("FAIL: Tag Name isn't visible", sel.isTextPresent(WikisObjects.NoTagsIndicator));
		Assert.assertTrue("FAIL: Add tags link isn't visible", sel.isElementPresent(WikisObjects.Add_tags_Link));	
		//Verify recommendations
		Assert.assertTrue("FAIL: Recommendations isn't correct", sel.isTextPresent(WikisObjects.NoRecommendationsIndicator));
		
		//Verify that all page buttons are visible
		Assert.assertTrue("FAIL: Edit button is missing", sel.isElementPresent(WikisObjects.Edit_Button));
		Assert.assertTrue("FAIL: Page Actions button is missing", sel.isElementPresent(WikisObjects.Page_Actions_Button));
		Assert.assertTrue("FAIL: Follow button is missing", sel.isElementPresent(WikisObjects.Follow_Button));
		Assert.assertTrue("FAIL: Wiki Actions button is missing", sel.isElementPresent(WikisObjects.Wiki_Actions_Button));	
	
		//Verify all inline tabs are visible
		Assert.assertTrue("FAIL: Comments tab is missing", sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Versions tab is missing", sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Attachments tab is missing", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: About tab is missing", sel.isElementPresent(WikisObjects.About_Tab));	
	...
	public void OpenWikiFromMyWikisTab(String Wiki_Title) throws Exception {				
		...
		/*while (!sel.isElementPresent("link=" + Wiki_Title)){
			...
			while (!sel.isElementPresent("link=" + Wiki_Title)){
				...
				if ((sel.isElementPresent(WikisObjects.Top_Next_Page_Link_InActive))&&(!sel.isElementPresent("link=" + Wiki_Title)))
					break;
			}	
			
			//Assert.assertTrue("FAIL: Wiki doesn't appear to have been created", sel.isElementPresent("link=" + Wiki_Title));
		}*/
	...
	public void VerifyIamFollowingFilterIsEmpty(String Wiki_Title) throws Exception {				
		...
		try
		{
			Assert.assertTrue("FAIL: Wiki is appearing in the I'm Following filter when it should not!", !sel.isElementPresent("link=" + Wiki_Title));
		}
		catch (Exception e)
		{
	...
	public void verifyRecommendationUI(int pageRecommends, boolean pageRecommended) throws Exception {
		...
		if(sel.isElementPresent(WikisObjects.Recommendations_Info_Alternate))
			...
			InfoText = sel.getText(WikisObjects.Recommendations_Info);
	...
	public void ValidateLinksInCreateWikiPageMode() throws Exception {
		...
		Assert.assertTrue("FAIL: Save & Close link is missing", sel.isElementPresent(WikisObjects.Save_and_Close_Link));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
		Assert.assertTrue("FAIL: Save link is missing", sel.isElementPresent(WikisObjects.Save_Link));
	}
	...
	public boolean verifyWikiExists(String WikiName, String WikiType) throws Exception {
		...
		if (WikiType.equals("Private")){
			...
			if(sel.isElementPresent("link=" + WikiName))
				wikiExists = true;
	...
	public void LoginUIValidation() {	
		...
		Assert.assertTrue("FAIL: username field isn't visible", sel.isElementPresent(WikisObjects.WIKI_USERNAME_FIELD));
		Assert.assertTrue("FAIL: password field isn't visible", sel.isElementPresent(WikisObjects.WIKI_Password_FIELD));
		Assert.assertTrue("FAIL: login button isn't visible", sel.isElementPresent(WikisObjects.WIKI_Login_Button));	
	...
	public void ServicePageUIValidation() throws Exception {
		...
		Assert.assertTrue("FAIL: Public Wikis tab isn't visible", sel.isElementPresent(WikisObjects.Public_Wikis_Tab));
		Assert.assertTrue("FAIL: My Wikis tab isn't visible", sel.isElementPresent(WikisObjects.My_Wikis_Tab));
		Assert.assertTrue("FAIL: Public Wikis tab isn't in focus", sel.isElementPresent(WikisObjects.Public_Wikis_Tab_Focus));
	}
	...
	public void verifyRecommendationUI_OLDCode(int pageRecommends, boolean pageRecommended) throws Exception {
		//Verify the Recommendation UI before a user recommends the page
		if ((pageRecommends==0)&&(!pageRecommended))
			Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.getText(WikisObjects.Recommendations_Info).equals("No recommendations"));
			...
			String InfoText = sel.getText(WikisObjects.Recommendations_Info);
				if (InfoText.contains(" person")){
					...
					Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.isTextPresent(numericValue + " person"));
					hoverMouseOver(WikisObjects.Recommendations_Info);
					Assert.assertTrue("FAIL: Recommend this page link in recommendation info is incorrect", sel.isElementPresent(WikisObjects.Recommend_This_Page_Link));
					Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(numericValue + " other person recommended this:"));
					if (!pageRecommended)
						Assert.assertTrue("FAIL: Recommending members list is incorrect", sel.isElementPresent("xpath=//*[contains(@id,'lconn_wikis_widget_tooltip_Recommend_')]/div[1]/div/div/div/ul/li[" + pageRecommends + "]/a"));
					else
						Assert.assertTrue("FAIL: Recommending members list is incorrect", sel.isElementPresent("xpath=//*[contains(@id,'lconn_wikis_widget_tooltip_Recommend_')]/div[1]/div/div/div/ul/li[" + pageRecommends++ + "]/a"));	
				}
					...
					Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.isTextPresent(numericValue + " people"));
					hoverMouseOver(WikisObjects.Recommendations_Info);
					Assert.assertTrue("FAIL: Recommend this page link in recommendation info is incorrect", sel.isElementPresent(WikisObjects.Recommend_This_Page_Link));
					Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(numericValue + " other people recommended this:"));
					...
					if (Integer.parseInt(numericValue) == 1){
						Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.isTextPresent("You and " + numericValue + " other"));
						hoverMouseOver(WikisObjects.Recommendations_Info);
						Assert.assertTrue("FAIL: Recommend this page link in recommendation info is incorrect", sel.isElementPresent(WikisObjects.Recommend_This_Page_Link));
						Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(numericValue + " other person recommended this:"));
					}
					else{
						Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.isTextPresent("You and " + numericValue + " others"));
						hoverMouseOver(WikisObjects.Recommendations_Info);
						Assert.assertTrue("FAIL: Recommend this page link in recommendation info is incorrect", sel.isElementPresent(WikisObjects.Recommend_This_Page_Link));
						Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(numericValue + " other people recommended this:"));
					}
					...
					Assert.assertTrue("FAIL: Recommendation text is incorrect", sel.getText(WikisObjects.Recommendations_Info).equals("You"));
					hoverMouseOver(WikisObjects.Recommendations_Info);
					Assert.assertTrue("FAIL: Popup text for recommendation info is incorrect", sel.isTextPresent("You have recommended this page."));
				}
		}
	}
	...
	public void ValidatePageActionsItems(String UserAccess) throws Exception {
		if (UserAccess.equals("Owner")){
			...
			Assert.assertTrue("FAIL: Page Actions contains more items than expected", !sel.isElementPresent(WikisObjects.Menu_Item_7));
		}
			...
			Assert.assertTrue("FAIL: Page Actions item is missing - Move Page", MovePage.contains("Move Page"));
			Assert.assertTrue("FAIL: Page Actions item is missing - Download Page", DownloadPage.contains("Download Page"));
			Assert.assertTrue("FAIL: Page Actions contains more items than expected", !sel.isElementPresent(WikisObjects.Menu_Item_6));
		}
		else {
	...
	public void CheckTagsLinks() throws Exception {
		...
		Assert.assertTrue("FAIL: Cloud link is missing", sel.isElementPresent(WikisObjects.Link_for_Tag_Cloud));
	}
	...
	public void VerifyMyWikisICanUI() throws Exception {
		Assert.assertTrue("FAIL: I'm a Reader filter is missing", sel.isElementPresent(WikisObjects.MyWikis_Reader_Filter));
		Assert.assertTrue("FAIL: I'm an Editor filter is missing", sel.isElementPresent(WikisObjects.MyWikis_Editor_Filter));
		Assert.assertTrue("FAIL: I'm an Owner filter is missing", sel.isElementPresent(WikisObjects.MyWikis_Owner_Filter));
	}
	
	public void checkFilterResultsUI() throws Exception {
		Assert.assertTrue("FAIL: Filter with the tag name isn't visible on tag filter page", sel.isElementPresent(WikisObjects.Filter_with_TagName)); 
		Assert.assertTrue("FAIL: Sort by Name Option on TagFilter Page is not Present", sel.isElementPresent(WikisObjects.Sortby_Name_Option));
		Assert.assertTrue("FAIL: Sort by Created Option on TagFilter Page is not Present", sel.isElementPresent(WikisObjects.Sortby_Created_Option));
		Assert.assertTrue("FAIL: Sort by Updated Option on TagFilter Page is not Present", sel.isElementPresent(WikisObjects.Sortby_Updated_Option));
		Assert.assertTrue("FAIL: Link toshow 25 Wikis per Page on TagFilter Page is not Present", sel.isElementPresent(WikisObjects.Link_toshow_25Wikis_perpage_TagFilterPage));	
		Assert.assertTrue("FAIL: Link to show 50 Wikis per Page on TagFilter Page is not Present", sel.isElementPresent(WikisObjects.Link_toshow_50Wikis_perpage_TagFilterPage));	
		Assert.assertTrue("FAIL: PageId showing 10 Wikis per Page on the TagFilter Page is not Present", sel.isElementPresent(WikisObjects.PageID_Showing_10Wikis_perpage_on_TagFilterPage));	
	}
	...
	public boolean ValidateNewWikiInPublicWikisTab(String Wiki_Title, String WikisState) throws Exception {		
		...
		if (WikisState.equals("Public")){
			...
			Assert.assertTrue("FAIL: wiki doesn't appear in 'Public Wikis'", sel.isElementPresent("link=" + Wiki_Title));		
		}
		...
		//Validate that a new private wiki doesn't appear in the 'Public Wikis' tab
			...
			while ((!sel.isElementPresent("link=" + Wiki_Title)) && (stopSearch == false)){				
				...
				if (!sel.isElementPresent(WikisObjects.Public_Wikis_TopCell)){
					Assert.assertTrue("FAIL: 'No items found' text should appear if no public wikis have been created", sel.isTextPresent("No items found"));	
			...
			Assert.assertTrue("FAIL: wiki shouldn't appear in 'Public Wikis'", !sel.isElementPresent("link=" + Wiki_Title));	
		}
	...
	public void ValidatePublicWikisUI(boolean ItemsCheck, boolean Authenticated) throws Exception {
		...
		Assert.assertTrue("FAIL: Find a wiki header shouldn't be visible", !sel.isTextPresent(WikisObjects.Find_A_Wiki_Header));
		Assert.assertTrue("FAIL: Popular Tags header isn't visible", sel.isTextPresent(WikisObjects.Public_Tags_Header));
		Assert.assertTrue("FAIL: Find a wiki section shouldn't be visible", !sel.isElementPresent(WikisObjects.Find_In_Wiki_List));
		Assert.assertTrue("FAIL: Popular Tags search box isn't visible", sel.isElementPresent(WikisObjects.Tag_Search_Textfield));	
		Assert.assertTrue("FAIL: Popular Tags search icon isn't visible", sel.isElementPresent(WikisObjects.Public_Tag_Search_Field_SearchIcon));	
		Assert.assertTrue("FAIL: Popular Tags container isn't visible", sel.isElementPresent(WikisObjects.Tags_Container));	
	
		//Verify some UI elements of 'Public Wikis' page
		Assert.assertTrue("FAIL: 'Start a Wiki' button is missing", sel.isElementPresent(WikisObjects.Start_New_Wiki_Button));	
	...
	public void ValidateNewToWikisSectionUI() throws Exception {
		Assert.assertTrue("FAIL: 'New to Wikis?' section isn't visible", sel.isElementPresent(WikisObjects.Welcome_Section));
		Assert.assertTrue("FAIL: Learn more button isn't visible", sel.isElementPresent(WikisObjects.Learn_More_Button));
		Assert.assertTrue("FAIL: Watch demo graphic link isn't visible", sel.isElementPresent(WikisObjects.Watch_Demo_Button));
	}
	
	public void ValidateCreateNewWikiUI() throws Exception {
		//Validate 'Create a new Wiki' lightbox UI
		Assert.assertTrue("FAIL: Start a Wiki title isn't correct", sel.isTextPresent(WikisObjects.Start_A_Wiki_Title));
		
		Assert.assertTrue("FAIL: Name heading is missing", sel.isTextPresent("Name:"));
		Assert.assertTrue("FAIL: Wiki name field is missing", sel.isElementPresent(WikisObjects.Wiki_Name_Field));
		
		Assert.assertTrue("FAIL: Tags heading is missing", sel.isTextPresent("Tags:"));
		Assert.assertTrue("FAIL: Wiki name field is missing", sel.isElementPresent(WikisObjects.Start_A_Wiki_Tags_Field));
		
		//Verify controls to Manage Access
		this.VerifyManageAccessUI(true);
		
		Assert.assertTrue("FAIL: Members heading is missing", sel.isTextPresent("Members:"));
		Assert.assertTrue("FAIL: Members dropdown is missing", sel.isElementPresent(WikisObjects.Add_Members_Dropdown));
		Assert.assertTrue("FAIL: Members textfield is missing", sel.isElementPresent(WikisObjects.Members_Field));
		
		clickLink(WikisObjects.AddGroups_Link);
		
		Assert.assertTrue("FAIL: Groups heading is missing", sel.isTextPresent("Groups:"));
		Assert.assertTrue("FAIL: Groups dropdown is missing", sel.isElementPresent(WikisObjects.Start_A_Wiki_Groups_Dropdown));
		Assert.assertTrue("FAIL: Groups textfield is missing", sel.isElementPresent(WikisObjects.Groups_Field));
		
		Assert.assertTrue("FAIL: Description heading is missing", sel.isTextPresent("Description:"));
		Assert.assertTrue("FAIL: Description field is missing", sel.isElementPresent(WikisObjects.Description_Field));

		Assert.assertTrue("FAIL: Save button is missing", sel.isElementPresent(WikisObjects.Save_Button));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void ValidateWikiPageUI(String UserRole) throws Exception {
		...
		if (sel.isTextPresent("This page has unsaved changes that were autosaved")){
			...
			CheckTitle = sel.getText(WikisObjects.WikiHomePageTitleField);
		...
		if ((UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role))){
			...
			Assert.assertTrue("FAIL: Navigation pane is missing", sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
			Assert.assertTrue("FAIL: New Page link is missing", sel.isElementPresent(WikisObjects.Create_New_Page_Link));
			Assert.assertTrue("FAIL: Edit Button is missing", sel.isElementPresent(WikisObjects.Edit_Button));
			...
			if (UserRole.equals(Data.Owner_Role)){
				...
				Assert.assertTrue("FAIL: Page Actions button is missing", sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Editors));

			//Verify tags link on homepage
			if(sel.isTextPresent(WikisObjects.NoTagsIndicator)){
				Assert.assertTrue("FAIL: Add a Tag link isn't appearing", sel.isElementPresent(WikisObjects.Add_tags_Link));
				...
				Assert.assertTrue("FAIL: Add or Remove Tags link isn't appearing", sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link));
			...
			Assert.assertTrue("FAIL: Navigation pane is missing", sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
			Assert.assertTrue("FAIL: New Page link shouldn't appear for users with less than editor access", !sel.isElementPresent(WikisObjects.Create_New_Page_Link));
			Assert.assertTrue("FAIL: Edit Button shouldn't appear for users with less than editor access", !sel.isElementPresent(WikisObjects.Edit_Button));
			Assert.assertTrue("FAIL: Page Actions button should appear for users with less than editor access", sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Readers));
		
			//Verify tags link isn't visible on homepage
			Assert.assertTrue("FAIL: Add tags link shouldn't appear for users with less than editor access", !sel.isElementPresent(WikisObjects.Add_tags_Link));
			Assert.assertTrue("FAIL: Add or Remove Tags link shouldn't appear for users with less than editor access", !sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link));
		}
	}
	...
	public void ValidateLeftSideMenuBar() throws Exception{
		...
		Assert.assertTrue("FAIL: Tags section is missing", sel.isTextPresent("Tags"));
		Assert.assertTrue("FAIL: Tags expand/collapse icon is missing", sel.isElementPresent(WikisObjects.Tags_Expand_Collapse_Arrow));
		Assert.assertTrue("FAIL: Incorrect ID for Popular Tags expand/collapse section", sel.getText(WikisObjects.Tags_Expand_Collapse_Arrow).contains("Expand and collapse Tags section"));

		//Verify that Members section appears
		Assert.assertTrue("FAIL: Members expand/collapse icon is missing", sel.isElementPresent(WikisObjects.Members_Expand_Collapse_Arrow));
		Assert.assertTrue("FAIL: Incorrect ID for Members expand/collapse section", sel.getText(WikisObjects.Members_Expand_Collapse_Arrow).contains("Expand and collapse Members section"));
	}

	public boolean ValidateOfExpandCollapseOfTagsUI() throws Exception {
		boolean contentVisible = false;
		
		Assert.assertTrue("FAIL: Tags arrow isn't visible", sel.isElementPresent(WikisObjects.MyWikis_Tags_Collapsible_Icon));
		sel.click(WikisObjects.MyWikis_Tags_Collapsible_Icon);
		Thread.sleep(1000);

		if(!sel.isElementPresent(WikisObjects.My_Tags_Search_Section))
			contentVisible = true;
	...
	public void ValidatePageFooterElements() throws Exception {
		Assert.assertTrue("FAIL: Comments link is missing", sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Versions is missing", sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Attachments link is missing", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: About link is missing", sel.isElementPresent(WikisObjects.About_Tab));
		//Assert.assertTrue("FAIL: Username doesn't appear in footer", sel.isElementPresent(Objects.Username_on_PageView));
		Assert.assertTrue("FAIL: Updated by field is missing", sel.isElementPresent(WikisObjects.Updated_By_Field));
		...
		Assert.assertTrue("FAIL: 'Subscribe to this page' link is missing", sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));	
	}
	...
	public void ValidateHomePageJavelinCard() throws Exception {
		...
		//Thread.sleep(3000);
		//Assert.assertTrue("FAIL: business card text doesn't appear", sel.isElementPresent("class=javlinHover"));
	}
	...
	public void ValidateActionButtonsInPageView(String UserRole) throws Exception{
		if ((UserRole.equals("Owner"))||(UserRole.equals("Editor"))){
			Assert.assertTrue("FAIL: Edit button doesn't appear for Owner", sel.isElementPresent(WikisObjects.Edit_Button));
			Assert.assertTrue("FAIL: Page Actions button doesn't appear for Owner", sel.isElementPresent(WikisObjects.Page_Actions_Button));
		}
		else {
			Assert.assertFalse("FAIL: Edit button shouldn't appear for user with less than Editor access", sel.isElementPresent(WikisObjects.Edit_Button));
			Assert.assertTrue("FAIL: Page Actions button should appear for user with less than Editor access", sel.isElementPresent(WikisObjects.Page_Actions_Button));
		}
	}
	
	public void ValidatePageBreadcrumbElements(String Wiki_Title) throws Exception{
		Assert.assertTrue("FAIL: Breadcrumbs rootpage is missing", sel.isElementPresent(WikisObjects.Breadcrumbs_Root_Page));
		Assert.assertTrue("FAIL: Some breadcrumbs text is incorrect", sel.isElementPresent(WikisObjects.All_Breadcrumb_Text));
	...
	public String ValidatePageInfoBreadcrumbElements(String Wiki_Title) throws Exception{
		Assert.assertTrue("FAIL: breadcrumbs rootpage is missing", sel.isElementPresent(WikisObjects.Breadcrumbs_Root_Page));
		Assert.assertTrue("FAIL: breadcrumbs are missing", sel.isElementPresent(WikisObjects.All_Breadcrumb_Text));
	...
	public void ValidateTaggingUIAfterTagsAreAdded(String UserRole) throws Exception {
		if ((UserRole.equals("Owner"))||(UserRole.equals("Editor"))){
			Assert.assertTrue("FAIL: Add or Remove Tags link doesn't appear for Owner or Editor", sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link));
			Assert.assertTrue("FAIL: No tags indicator shouldn't appear when tags are added", !sel.isTextPresent(WikisObjects.NoTagsIndicator));
		}
		else {
			Assert.assertTrue("FAIL: Add or Remove Tags link shouldn't appear for user with less than editor access", !sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link));
			Assert.assertTrue("FAIL: No tags indicator shouldn't appear when tags are added", !sel.isTextPresent(WikisObjects.NoTagsIndicator));
		}
	}
	...
	public void ValidateTaggingUIBeforeTagsAreAdded(String UserRole) throws Exception {
		if ((UserRole.equals("Owner"))||(UserRole.equals("Editor"))){
			Assert.assertTrue("FAIL: Add tags link don't appear for Owner or Editor", sel.isElementPresent(WikisObjects.Add_tags_Link));
			Assert.assertTrue("FAIL: No tags indicator doesn't appear for Owner or Editor", sel.isTextPresent(WikisObjects.NoTagsIndicator));
		}
		else {
			Assert.assertTrue("FAIL: Add tags link is appearing for anonymous user", !sel.isElementPresent(WikisObjects.Add_tags_Link));
			Assert.assertTrue("FAIL: No tags indicator doesn't appear for user with less than editor access", sel.isTextPresent(WikisObjects.NoTagsIndicator));
		}
	}	
	...
	public void ValidateCommentsEditor() throws Exception {
	    ...
	    Assert.assertTrue("FAIL: on Add a comment editor", sel.isElementPresent(WikisObjects.Add_Comment_Editor));
	    Assert.assertTrue("FAIL: on Save button in comment editor", sel.isElementPresent(WikisObjects.Save_Button));
	    Assert.assertTrue("FAIL: on Cancel button in comment editor", sel.isElementPresent(WikisObjects.Cancel_Comments_Link));
	}
	...
	public void ValidateLinksInComments(String Current_Contributor, String Contributor_Role, int index) throws Exception {
		...
		for (int UserNo = index; UserNo > 0; UserNo--){
			...
			if ((UserName.contains(Current_Contributor))&((Contributor_Role.equals(Data.Owner_Role))||(Contributor_Role.equals(Data.Editor_Role))||(Contributor_Role.equals(Data.Reader_Role)))){	
				if (UserName.length() > 10){
					if (!UserName.substring(0, 11).equals(Data.Wiki_LDAP_Reader_Username1_Groups)){
						Assert.assertTrue("FAIL: Comments Edit link isn't available", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/div/ul/li/a"));
						Assert.assertTrue("FAIL: Comments Delete link isn't available", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/a/img"));
			...
			//If current user isn't the owner of the comment but has Owner access to the wiki, the Delete icon should be available for all comments.*/
				...
				Assert.assertFalse("FAIL: Comments Edit link shouldn't appear if you aren't the contributor of this comment", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/div/ul/li/a"));
				Assert.assertTrue("FAIL: Comments Delete link isn't available (being viewed by a manager)", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/a/img"));
			}
			...
			//If current user isn't the owner of the comment and has Editor/Reader access to the wiki, the Edit link & Delete icon shouldn't be available for comments not owned by the contributor.*/
				...
				Assert.assertFalse("FAIL: Comments Edit link shouldn't appear if you aren't the contributor of this comment", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/div/ul/li/a"));
				Assert.assertFalse("FAIL: Comments Delete link shouldn't appear if you aren't the contributor of this comment", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/a/img"));
			}
			
			//If current user is anonymous all comments should appear without Edit links & Delete icons*/
			else {
				Assert.assertFalse("FAIL: Comments Edit link shouldn't appear if you access the wiki anonymously", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/div/ul/li/a"));
				Assert.assertFalse("FAIL: Comments Delete link shouldn't appear if you access the wiki anonymously", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/a/img"));
			}
		}
	}
	...
	public void ValidateLinksInComments(int index) throws Exception {
		for (int UserNo = index; UserNo > 0; UserNo--){
			...
			Assert.assertTrue("FAIL: Edit link exists for comment (being viewed by unauthenticated user)", !sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/div/ul/li/a"));
			Assert.assertTrue("FAIL: Delete link exists for comment (being viewed by unauthenticated user)", !sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+UserNo+"]/div[2]/div[2]/a/img"));
		}
		Assert.assertFalse("FAIL: 'Add a comment' link exists for comment (being viewed by unauthenticated user)", sel.isVisible(WikisObjects.Add_Comment_Link));	
	}
	...
	public void VerifyWikiVisibilityInLatestUpdates(String Link_for_Wiki_Title, String Wiki_Title_Text) throws Exception {	
		...
		Assert.assertTrue("FAIL: Wiki shouldn't be visible when a user is anonymous", !sel.isElementPresent(Link_for_Wiki_Title));
		Assert.assertTrue("FAIL: Wiki shouldn't be visible when a user is anonymous", !sel.isTextPresent(Wiki_Title_Text));
		Assert.assertTrue("FAIL: Start a Wiki button shouldn't appear when a user is anonymous", !sel.isElementPresent(WikisObjects.Start_New_Wiki_Button));
	...
	public void ValidateNewlyDeletedComment(int OldCommentCount, int NewCommentCount) throws Exception {
		...
		Assert.assertTrue("FAIL: Comment field has been deleted so shouldn't be visible", !sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+index+"]/div[2]/div[1]/div"));
	...
	public void ValidateNewlyCancelledDeletion(int CommentCount, int LatestCommentCount) throws Exception {
		...
		Assert.assertTrue("FAIL: Comment field is missing even though the deletion was cancelled", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+index+"]"));
	...
	public void ValidateTitleAndBreadcrumbs(String Breadcrumb_Header_Link,String Wiki_Title) {
		Assert.assertTrue("FAIL: on breadcrumb header", sel.isElementPresent(Breadcrumb_Header_Link));
	...
	public void OpenCommentsEditAndValidate(int CommentsIndex, String CommentForEditing) throws Exception {
		...
		Assert.assertTrue("FAIL: Edit link in comments isn't available", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+CommentsIndex+"]/div[2]/div[2]/div/ul/li/a"));
		sel.clickAt("xpath=id('wikiPageComments')/div[2]/div["+CommentsIndex+"]/div[2]/div[2]/div/ul/li/a", "Edit");
		Thread.sleep(2000);
		
		//Verify that the Editor opens and that the Save Button and the Cancel Link appear
		Assert.assertTrue("FAIL: Add comments editor is missing", sel.isElementPresent(WikisObjects.Add_Comment_Editor));
		Assert.assertTrue("FAIL: Save button in comments editor is missing", sel.isElementPresent(WikisObjects.Save_Button));
		
		Assert.assertTrue("FAIL: Editable comment in editor is missing", EditableText.equals(CommentForEditing));
		Assert.assertTrue("FAIL: Cancel button in comments editor is missing", sel.isElementPresent(WikisObjects.Cancel_Comments_Link));    
	}
	...
	public void OpenDeleteAndValidateUI(int CommentsIndex) throws Exception {
		...
		Assert.assertTrue("FAIL: Delete link in comments is missing", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div["+CommentsIndex+"]/div[2]/div[2]/a/img"));
	...
	public void ValidateWikiEditMode() throws Exception {
		...
		Assert.assertTrue("FAIL: Rich Text tab isn't visible", sel.isElementPresent(WikisObjects.Rich_Text_Tab));
		Assert.assertTrue("FAIL: HTML Source tab isn't visible", sel.isElementPresent(WikisObjects.HTML_Source_Tab));
		Assert.assertTrue("FAIL: Wiki Text tab isn't visible", sel.isElementPresent(WikisObjects.Wiki_Text_tab));
		Assert.assertTrue("FAIL: Preview tab isn't visible", sel.isElementPresent(WikisObjects.Preview_Tab));		
	}
	...
	public void ValidateNewPageInEditMode() throws Exception {
		...
		Assert.assertTrue("FAIL: New Page textfield is missing", sel.isElementPresent("class=lotusText wikiPageTitle"));
	...
	public void ValidateTaggingUIInEditMode()throws Exception {
		...
		if ((sel.isElementPresent(WikisObjects.Add_tags_Link))&
				(sel.isElementPresent(WikisObjects.NoTags))){
					System.out.println("PASS: Tagging links appear fine. No tags are added.");
		}
		else if ((sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link))&
					(!sel.isElementPresent(WikisObjects.NoTags))){
	...
	public void ValidateLinksInEditMode() throws Exception {
		...
		Assert.assertTrue("FAIL: Edit Page title is missing", sel.isElementPresent(WikisObjects.Edit_Page_Title));
	}
	...
	public void VerifyBreadcrumbsInWikiEditMode() throws Exception {
		...
		Assert.assertTrue("FAIL: breadcrumbs in edit mode are missing", sel.isElementPresent(WikisObjects.Breadcrumbs_In_Edit_Mode));
		Assert.assertTrue("FAIL: root page breadcrumb link is missing", sel.isElementPresent(WikisObjects.Breadcrumbs_In_Edit_Mode_Root));                      
	}
	...
	public void VerifyBreadcrumbsInNewPageEditor(String WikiTitle) throws Exception {
		...
		Assert.assertTrue("FAIL: breadcrumbs parent in New Page edit mode is missing", sel.isElementPresent(WikisObjects.Breadcrumbs_In_Edit_Mode));
	...
	public void ValidatePageInfoHeadings(int NumberOfChildPages) throws Exception {
		...
		Assert.assertTrue("FAIL: About this file heading is missing", sel.isTextPresent("About this file"));
		
		Assert.assertTrue("FAIL: Child pages section is missing", sel.isElementPresent(WikisObjects.ChildPages_Heading));
		...
		Assert.assertTrue("FAIL: Hierarchy heading is missing", sel.isTextPresent("Hierarchy"));
	}
	...
	public int VerifyChildPagesOnPageInfoPage(String UserRole) throws Exception {
		...
		if ((UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role)))
			Assert.assertTrue("FAIL: Create a new child page link doesn't appear in Page Info", sel.isElementPresent(WikisObjects.Create_newChildPage_Link));
		
		else
			Assert.assertTrue("FAIL: Create a new child page link shouldn't appear for user with less than Editor access", !sel.isElementPresent(WikisObjects.Create_newChildPage_Link));
	...
	public void VerifyHierarchyOnPageInfoPage() throws Exception {
		...
		Assert.assertTrue("FAIL: Move link is missing in Hierarchy section", sel.isElementPresent(WikisObjects.MovePage_Link));
		Assert.assertTrue("FAIL: Hierarchy rootpage breadcrumb is missing", sel.isElementPresent(WikisObjects.Hierarchy_breadcrumb_RootPage));
		Assert.assertTrue("FAIL: Hierarchy homepage breadcrumb is missing", sel.isElementPresent(WikisObjects.Hierarchy_breadcrumb_HomePage));
	...
	public void VerifyRemainingAboutThisFileFields() throws Exception {
		...
		Assert.assertTrue("FAIL: Updated field title isn't correct", sel.isTextPresent(WikisObjects.Updated_Field));
		Assert.assertTrue("FAIL: Added field title isn't correct", sel.isTextPresent(WikisObjects.Added_Field));
		Assert.assertTrue("FAIL: Size field title isn't correct", sel.isTextPresent(WikisObjects.Size_Field));
		Assert.assertTrue("FAIL: Page Views field title isn't correct", sel.isTextPresent(WikisObjects.PageViews_Field));
		Assert.assertTrue("FAIL: Page Viewed by field title shouldn't appear", !sel.isTextPresent(WikisObjects.PageViewedby_Field));
	}
	...
	public void VerifyAboutThisFileUpdatedField(String username) throws Exception {
		...
		Assert.assertTrue("FAIL: About This File Updated field is missing", sel.isElementPresent(WikisObjects.About_This_File_Updated_Field));
		...
		Thread.sleep(2000);
		Assert.assertTrue("FAIL: javelin text is missing", sel.isTextPresent("Click here to view business card"));	
	}
	...
	public void VerifyAboutThisFileAddedField(String username) throws Exception {
		...
		Assert.assertTrue("FAIL: About this file 'Added' field is missing", sel.isElementPresent(WikisObjects.About_This_File_Added_Field));
	...
	public int VerifyAboutThisFileSizeForPage() throws Exception {
		...
		Assert.assertTrue("FAIL: Size field is missing", sel.isElementPresent(WikisObjects.About_This_File_Size_Field));
	...
	public int VerifyAboutThisFileSizeOverall(int PageSize) throws Exception {
		...
		Assert.assertTrue("FAIL: Size field element doesn't appear", sel.isElementPresent(WikisObjects.About_This_File_Size_Field));
	...
	public int VerifyAboutThisFilePageViewsField() throws Exception {
		Assert.assertTrue("FAIL: Page Views field doesn't appear in Page Info", sel.isElementPresent(WikisObjects.About_This_File_PageViews_Field));
	...
	public void VerifyAboutThisFilePageViewedByField(String username) throws Exception {
		...
		Assert.assertTrue("FAIL: Page Viewed by field is missing", sel.isElementPresent(WikisObjects.About_This_File_Page_Viewed_By_Field));
		Assert.assertTrue("FAIL: First element of Page Viewed by field should be current user", PageViewedByName.contains(username));
		
		do{
			Assert.assertTrue("FAIL: Page Viewed by username is missing", sel.isElementPresent("xpath=//*[contains(@id,'quickr_widget_DownloadInfoStream')]/div[1]/span["+index+"]/a"));
			Assert.assertTrue("FAIL: bullet point missing beside Page Viewed by username", sel.isElementPresent("xpath=//*[contains(@id,'quickr_widget_DownloadInfoStream')]/div[1]/span["+index+"]/img"));
	...
	public void ValidateTaggingEditor() throws Exception {    
		...
		Assert.assertTrue("FAIL: Tag editor textfield is missing", sel.isElementPresent(WikisObjects.TagEditorTextField));
		Assert.assertTrue("FAIL: Tag editor Save button is missing", sel.isElementPresent(WikisObjects.Save_Button)); 
		Assert.assertTrue("FAIL: Tag editor Cancel button is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void ValidateXinEditor(int tagCount) throws Exception {
		...
		while(index <= tagElements){
			index++;	
			Assert.assertTrue("FAIL: 'x' is missing from a tag", sel.isElementPresent("xpath=//*[contains(@id,'quickr_widget_Tagger')]/a["+index+"]/img"));
			index++;
			XCount++;
		}
	...
	public void VerifyCommentIsAdded(String Added_Comment) throws Exception {
		Assert.assertTrue("FAIL: Comment hasn't been added correctly", sel.isTextPresent(Added_Comment)); 
	}
	...
	public void ActivateTagDeletionPopup(String Delete_Tag_Title) throws Exception {
		...
		Assert.assertTrue("FAIL: Delete icon next to tag is missing", sel.isElementPresent("css=img[title='Remove tag "+Delete_Tag_Title+"']"));
		clickLink("css=img[title='Remove tag "+Delete_Tag_Title+"']");
	   
		//Validate popup dialog
		Assert.assertTrue("FAIL: Remove Tag confirmation popup text is incorrect", sel.isTextPresent("Are you sure you want to remove the tag "+Delete_Tag_Title));
		...
		Assert.assertTrue("FAIL: OK button is missing", sel.isElementPresent(WikisObjects.OK_Button_In_Popup));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void RemoveRemainingTags(int NumberOfTags) throws Exception {
		...
		while (NumberOfTags > 0){
			...
			//Click on the Delete icon next to the tag added
			Assert.assertTrue("FAIL: Delete icon next to tag is missing", sel.isElementPresent("xpath=//*[contains(@id,'quickr_widget_Tagger')]/a["+xIndex+"]/img"));
	...
	public void ValidateTagDeletionPopupForRemainingTags(int NumberOfTags, String Delete_Tag_Title) throws Exception {
		...
		//Validate popup dialog
		Assert.assertTrue("FAIL: Remove Tag confirmation popup text is incorrect", sel.isTextPresent("Are you sure you want to remove the tag "+TagLowercaseText));
		...
		Assert.assertTrue("FAIL: Remove button is missing", sel.isElementPresent(WikisObjects.OK_Button_In_Popup));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	
	public void VerifyTagIsDeleted(String Deleted_Tag_Title) throws Exception {
		Assert.assertTrue("FAIL: Tag is still visible, it should have been removed", !sel.isTextPresent(Deleted_Tag_Title));
	}
	
	public void VerifyTagIsNotDeleted(String Deleted_Tag_Title) throws Exception {
		Assert.assertTrue("FAIL: tag is no longer visible, it hasn't been removed", sel.isTextPresent(Deleted_Tag_Title));
	}
	...
	public void VerifyAllTagsRemoved() throws Exception {
		...
		Assert.assertTrue("FAIL: Add tags link is missing", sel.isElementPresent(WikisObjects.Add_tags_Link));
	...
	public void ValidateDraftWarningMessageClearedFromHomepage() throws Exception {
			Assert.assertTrue("FAIL: Draft warning message shouldn't be visible", !sel.isTextPresent("This page has unsaved changes that were autosaved today at "));
			Assert.assertTrue("FAIL: Edit link shouldn't be visible", !sel.isElementPresent(WikisObjects.Drafts_Homepage_Edit_Link));
			Assert.assertTrue("FAIL: Discard link shouldn't be visible", !sel.isElementPresent(WikisObjects.Drafts_Homepage_Discard_Link));
	}
	...
	public void ValidateDraftWarningMessageCleared(String WikiTitle) throws Exception {
		if (!sel.isElementPresent("xpath=id('lotusContent')/div[1]/div[2]/span/span[1]")){                                          
			Assert.assertTrue("FAIL: Draft warning message shouldn't be visible", !sel.isTextPresent("You have unsaved changes to the following pages:"));
			Assert.assertTrue("FAIL: Draft text shouldn't be visible", !sel.isElementPresent(WikisObjects.Draft_Text));
			Assert.assertTrue("FAIL: Edit link shouldn't be visible", !sel.isElementPresent(WikisObjects.Drafts_Edit_Link));
			Assert.assertTrue("FAIL: Discard link shouldn't be visible", !sel.isElementPresent(WikisObjects.Drafts_Discard_Link));
		}
		else {
	...
	public void ValidateDiscardWarningMessage() throws Exception {
		Assert.assertTrue("FAIL: Discard lightbox heading is incorrect", sel.isTextPresent("Confirm"));
		Assert.assertTrue("FAIL: Discard lightbox text content is incorrect", sel.isTextPresent("Are you sure you wish to discard these unsaved changes?"));
	...
	public void ValidateAttachmentDeletionWarningMessage(String fileNameForDeletion) throws Exception {
		Assert.assertTrue("FAIL: Attachment deletion lightbox heading is incorrect", sel.isTextPresent("Confirm"));
		Assert.assertTrue("FAIL: Attachment deletion lightbox text content is incorrect", sel.isTextPresent("Are you sure you wish to delete this file? " + fileNameForDeletion));
		Assert.assertTrue("FAIL: OK button is missing", sel.isElementPresent(WikisObjects.OK_Button));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	
	public void ValidateDraftWarningMessage(String WikiTitle) throws Exception {
		Assert.assertTrue("FAIL: Draft warning message isn't visible", sel.isTextPresent("You have unsaved changes to the following pages:"));
	...
	public void ValidateDraftWarningMessageInHomepage() throws Exception {
		Assert.assertTrue("FAIL: Draft warning message isn't visible", sel.isTextPresent("This page has unsaved changes that were autosaved today at "));
	...
	public void ValidateAutosaveMessage() throws Exception {
		Assert.assertTrue("FAIL: Autosave message isn't visible", sel.isElementPresent(WikisObjects.Autosave_Indicator));
	...
	public void SaveAndCloseWithDraftOpen() throws Exception {
		Assert.assertTrue("FAIL: CKEditor Save & Close link is missing", sel.isElementPresent(WikisObjects.FckEditor_Save_And_Close));
		sel.click(WikisObjects.FckEditor_Save_And_Close);
		Thread.sleep(2000);
	}
	
	public void CancelWithDraftOpen() throws Exception {
		Assert.assertTrue("FAIL: on FCKEditor Cancel link", sel.isElementPresent(WikisObjects.Cancel_Link));
		sel.click(WikisObjects.Cancel_Link);
		Thread.sleep(2000);
	}
	...
	public void NavigateToFirstPageOfComments() throws Exception {
		if (sel.isElementPresent(WikisObjects.CommentPagination_Activated1))
			clickLink(WikisObjects.CommentPagination_Activated1);
	}

	public void CheckPaginationFunctions() throws Exception {		
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.CommentPagination_Deactivated));
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.CommentPagination_Activated2));
		Assert.assertTrue("FAIL: comments page pagination", sel.isVisible(WikisObjects.Top_Previous_Page_Link_InActive));
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.Top_Next_Page_Link_Active));		
			    
		System.out.println("PASS: All pagination controls appear fine on first comments page");
			
		clickLink(WikisObjects.CommentPagination_Activated2);
		
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.CommentPagination_Activated1));
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.CommentPagination_Deactivated));
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.Top_Previous_Page_Link_Active));
		Assert.assertTrue("FAIL: comments page pagination", sel.isElementPresent(WikisObjects.Top_Next_Page_Link_InActive));
	...
	public void VerifyUtilityIcons() throws Exception {
		...
		Assert.assertTrue("FAIL: on Download icon", sel.isElementPresent(WikisObjects.DownLoad_Icon));
		Assert.assertTrue("FAIL: on Print icon", sel.isElementPresent(WikisObjects.Print_Icon));
		Assert.assertTrue("FAIL: on RSS icon", sel.isElementPresent(WikisObjects.RSS_Icon));	
			
		System.out.println("PASS: Icons appear fine in page header");
	}
	
	public void RemoveContentFromPageForSizeCheck() throws Exception {
		Assert.assertTrue("FAIL: CKEditor isn't visible", sel.isElementPresent(WikisObjects.WikiText_Editor));
	...
	public int AddContentToPageForSizeCheck() throws Exception {
		Assert.assertTrue("FAIL: problem with CKEditor", sel.isElementPresent(WikisObjects.WikiText_Editor));
	...
	public String DeleteAttachment() throws Exception {
		Assert.assertTrue("FAIL: Attachments Delete link is missing", sel.isElementPresent(WikisObjects.Delete_Link));
	...
	public void ValidateAttachmentDeleting(int InitialAttachmentsCount, int CurrentAttachmentsCount, String Filename) throws Exception {		
		Assert.assertTrue("FAIL: File hasn't been deleted", !sel.isTextPresent(Filename));	
	...
	public void OpenEditAttachment() throws Exception {
		Assert.assertTrue("FAIL: Attachments Edit link is missing", sel.isElementPresent(WikisObjects.Edit_Link));
		sel.click(WikisObjects.Edit_Link);
		Thread.sleep(2000);
	}
	
	public void ValidateEditAttachmentPopup() throws Exception {
		Assert.assertTrue("FAIL: Edit Attachments title is missing", sel.isTextPresent("Edit Attachment"));
		Assert.assertTrue("FAIL: Edit Attachments File name label is missing", sel.isTextPresent("*File name:"));
		Assert.assertTrue("FAIL: Edit Attachments Description label is missing", sel.isTextPresent("Description:"));
		Assert.assertTrue("FAIL: Edit Attachments Replace Contents label is missing", sel.isTextPresent("Replace contents:"));

		Assert.assertTrue("FAIL: Edit Attachments File name textfield is missing", sel.isElementPresent(WikisObjects.Filename_Textfield));
		Assert.assertTrue("FAIL: Edit Attachments Description textfield is missing", sel.isElementPresent(WikisObjects.Description_Textfield));
		Assert.assertTrue("FAIL: Edit Attachments Replace Contents textfield is missing", sel.isElementPresent(WikisObjects.Replace_Contents_Textfield));

		Assert.assertTrue("FAIL: Edit Attachments OK button is missing", sel.isElementPresent(WikisObjects.OK_Button));	
		Assert.assertTrue("FAIL: Edit Attachments Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void VerifyIfAttachmentIsAddedToPageInfoPage(int InitialAttachCount, int CurrentAttachCount) throws Exception {
		Assert.assertTrue("FAIL: on Attachments visible in Page Info", sel.isElementPresent(WikisObjects.Wiki_Page_Attachments));
	
		//Verify the elements under the Attachments heading
		Assert.assertTrue("FAIL: on Add an Attachment link in Page Info", sel.isElementPresent(WikisObjects.AddanAttachment_Link));
		Assert.assertTrue("FAIL: on whether attachment heading has been incremented", CurrentAttachCount == InitialAttachCount + 1);
			...
			for (int index = 1; index <= CurrentAttachCount; index++){
				Assert.assertTrue("FAIL: on attachment content1", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[1]"));
				Assert.assertTrue("FAIL: on attachment content2", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[4]/a"));
				Assert.assertTrue("FAIL: on attachment content3", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[5]/a"));
			}
	...
	public void VerifyChildPagesAreVisible(int ChildPagesCount, String UserRole) throws Exception {
		...
		String ValidXPath = "";
			...
			if (ChildPagesCount==0){
				if ((UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role)))
					Assert.assertTrue("FAIL: on Create a new child page link", sel.isElementPresent(WikisObjects.Create_newChildPage_Link));
				else
	...
	public void VerifyAttachmentsUI(int AttachmentsCount, String UserRole) throws Exception {
		...
		if ((UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role))){
			...
			Assert.assertTrue("FAIL: Add an Attachment link is missing", sel.isElementPresent(WikisObjects.AddanAttachment_Link));
		}
		else {
			//Verify the 'Add an attachment' link under the Attachments heading
			Assert.assertTrue("FAIL: Add an Attachment link shouldn't appear for users with less than Editor access", !sel.isElementPresent(WikisObjects.AddanAttachment_Link));
		}
		//Verify the 'Subscribe to these attachments' link under the Attachments heading
		Assert.assertTrue("FAIL: Subscribe to these attachments link is missing", sel.isElementPresent(WikisObjects.Subscribe_To_These_Attachments_Link));
		...
		if (AttachmentsCount==0){
			...
			while(ElementExists){
				if (AttachmentsCount >= 11){
					Assert.assertTrue("FAIL: Second page Next link is missing", sel.isElementPresent(WikisObjects.Top_Next_Page_Link_Active));
				...
				if (sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[1]/a")){
					Assert.assertTrue("FAIL: Link to attachment is missing", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[1]"));
					Assert.assertTrue("FAIL: Delete link is missing", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[4]/a"));
					Assert.assertTrue("FAIL: Edit link is missing", sel.isElementPresent("xpath=id('wikiPageAttachments')/div[2]/table/tbody/tr["+index+"]/td[5]/a"));
					index++;
					AttachmentsAdded++;
				}
				else
	...
	public void CheckWikiNameLengthRestrictionCondition() throws Exception {
		Assert.assertTrue("FAIL: warning text is incorrect", sel.isTextPresent("The page name is too long."));
		Assert.assertTrue("FAIL: remaining warning text is incorrect", sel.isElementPresent("link=Shorten page name?"));
	}
	
	public void CheckWikiNameAsDuplicate() throws Exception {
		Assert.assertTrue("FAIL: duplication verification of wiki isn't correct", sel.isTextPresent("A page or file with the name you have specified already exists. Please choose a different name."));
		Assert.assertTrue("FAIL: Wiki name duplication error message isn't correct", sel.isTextPresent("There were problems with some of your entries."));
	}
	
	public void CheckWikiNameAsBlank() throws Exception {
		Assert.assertTrue("FAIL: existence of text warning about blank data input is incorrect", sel.isTextPresent("Please enter a name."));
	}
	...
	public void ValidateWikiActionsItems() throws Exception {
		...
		Assert.assertTrue("FAIL: Wiki Actions contains more items than expected", !sel.isElementPresent(WikisObjects.Menu_Item_3));	
	}
	...
	public void ValidateFollowMenuItems() throws Exception {
		...
		Assert.assertTrue("FAIL: Follow menu contains more items than expected", !sel.isElementPresent(WikisObjects.Menu_Item_3));	
	}
	
	public void VerifyHomepageVisibilityInNavigator(String WikiTitle) throws Exception {
		Assert.assertTrue("FAIL: Wiki page isn't visible in Navigator", sel.isElementPresent(WikisObjects.Wiki_Title_In_Nav));
	...
	public void VerifyAttachmentIncrementation(int InitialAttachNumber, int CurrentAttachNumber) throws Exception {
		...
		Assert.assertTrue("FAIL: Attachments link should corresond to attachments count", ElementCount(sel.getText(WikisObjects.Attachments_Tab)) == CurrentAttachNumber);
	}
	...
	public void verifyAttachmentsPaging() throws Exception {
		Assert.assertTrue("FAIL: Second page link hasn't been created for more than 10 attachments", sel.isElementPresent(WikisObjects.Link_For_2nd_Page));
		Assert.assertTrue("FAIL: Second page Next link hasn't been created for more than 10 attachments", sel.isElementPresent(WikisObjects.Top_Next_Page_Link_Active));
	}
	...
	public int ValidateVersionsNumericValue() throws Exception{
		Assert.assertTrue("FAIL: Versions heading is missing", sel.isElementPresent(WikisObjects.Versions_Tab));
	...
	public void ValidateVersionsJavelinCard(int NoOfVersions) throws Exception {
		...
		for (int index=1; index <= NoOfVersions; index++){
			...
			if ((NoOfVersions > 10)&&(ActivePage.equals("2"))){
				Assert.assertTrue("FAIL: on existence of pagination for versions",( sel.isElementPresent(WikisObjects.Versions_Top_Pagination_Section))&
			...
			Assert.assertTrue("FAIL: on existence of username for each version", sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr["+index+"]/td[3]/a"));	
			sel.mouseOver("xpath=id('pageVersions')/div[2]/table/tbody/tr["+index+"]/td[3]/a");
			Thread.sleep(3000);
			Assert.assertTrue("FAIL: javelin card text is missing", sel.isTextPresent("Click here to view business card"));
		}
	}
	...
	public void VerifyViewAndRestoreInVersionsPage(int NoOfVersions, String UserRole) throws Exception{
		...
		if (NoOfVersions == 1){
			Assert.assertTrue ("FAIL: View link doesn't appear in most recent version info", sel.isElementPresent(WikisObjects.Versions_Inactive_Version_View_Link));
		}
			...
			for (int index = 2; index <= NoOfVersions; index++){
				...
				Assert.assertTrue ("FAIL: View link doesn't appear in version number "+index, sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr["+index+"]/td[4]/a"));
				...
				if ((UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role))){
					...
					Assert.assertTrue ("FAIL: Restore link doesn't appear in version number "+index, sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr["+index+"]/td[5]/a"));
					...
					Assert.assertTrue ("FAIL: Restore link shouldn't appear for user with less than editor access ", !sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr["+index+"]/td[5]/a"));
			}
		}
	}
	...
	public void ValidateNoAccessPage() throws Exception {
		Assert.assertTrue("FAIL: No access text is incorrect", sel.isTextPresent(WikisObjects.No_Access_Text1));
		Assert.assertTrue("FAIL: No access text description is incorrect", sel.isTextPresent(WikisObjects.No_Access_Text2));
		Assert.assertTrue("FAIL: Back to Wikis link is missing", sel.isElementPresent(WikisObjects.BacktoWikis));
		sel.click(WikisObjects.BacktoWikis);
		Thread.sleep(2000);
	}
	
	public void ValidatePageCannotBeDisplayed() throws Exception {
		Assert.assertTrue("FAIL: Cannot display error message is incorrect", sel.isTextPresent(WikisObjects.Cannot_Display_Text));
		Assert.assertTrue("FAIL: Back to Wikis link is missing", sel.isElementPresent(WikisObjects.BacktoWikis));
		sel.click(WikisObjects.BacktoWikis);
		Thread.sleep(2000);
	}
	...
	public void ValidateDraftTitleInEditMode(String DraftTitle) throws Exception {
		...
		Assert.assertTrue("FAIL: Editable page title field isn't visible", sel.isElementPresent(WikisObjects.Page_Name_Textfield_In_Editor));
	...
	public String GetDraftMessageTitle() throws Exception {
		Assert.assertTrue("FAIL: Draft message is missing", sel.isElementPresent(WikisObjects.Draft_Message_Title));
	...
	public String GetTitle() throws Exception {
		Assert.assertTrue("FAIL: Homepage title is missing", sel.isElementPresent(WikisObjects.WikiHomePageTitleFieldWithDrafts));
	...
	public void NavigateBackToHomepageUsingBreadcrumbs(String WikiHomePageLink) throws Exception {
		Assert.assertTrue("FAIL: home page breadcrumb link is missing", sel.isElementPresent(WikiHomePageLink));
	...
	public void SelectAllPagesMenuItem(String AllPagesOption) throws Exception {
		if (AllPagesOption.equals("Go to Page...")){
			...
			Assert.assertTrue("FAIL: Pages search field is missing", sel.isElementPresent(WikisObjects.Search_Page_Field));
			Assert.assertTrue("FAIL: Pages search icon is missing", sel.isElementPresent(WikisObjects.Search_Page_Button));
		}
		...
		if (AllPagesOption.equals("I Edited")){
			...
			Assert.assertTrue("FAIL: Pages Edited by search field is missing", sel.isElementPresent(WikisObjects.Search_EditedBy_Field));
			Assert.assertTrue("FAIL: Pages Edited by search icon is missing", sel.isElementPresent(WikisObjects.Search_EditedBy_Button));
		} 
	}
	...
	public String getMemberRoleFilterText() throws Exception {
		...
		Assert.assertTrue("Filtered by text is incorrect", sel.isTextPresent(WikisObjects.MyWikis_Filter_Text));
		...
		if ((filterText.contains("Groups only"))||(filterText.contains("Users only")))
			filterText = sel.getText(WikisObjects.Role_Or_Kind_Filter_Icon);
	...
	public void ValidateEditWikiUI() throws Exception {
		Assert.assertTrue("FAIL: Edit Wiki header is missing", sel.isTextPresent("Edit Wiki"));
		
		Assert.assertTrue("FAIL: Wiki title label is missing", sel.isTextPresent("*Name:"));
		Assert.assertTrue("FAIL: Wiki title textfield is missing", sel.isElementPresent(WikisObjects.Edit_Wiki_Name_Textfield));
		
		Assert.assertTrue("FAIL: Tags label is missing", sel.isTextPresent("Tags:"));
		Assert.assertTrue("FAIL: Tags textfield is missing", sel.isElementPresent(WikisObjects.Edit_Wiki_Tags_Textfield));
		
		Assert.assertTrue("FAIL: Wiki creator label is missing", sel.isTextPresent("Created by:"));
		
		Assert.assertTrue("FAIL: Description label is missing", sel.isTextPresent("Description:"));
		Assert.assertTrue("FAIL: Wiki description textfield is missing", sel.isElementPresent(WikisObjects.Edit_Wiki_Description_Textfield));
	...
	public void verifySubView(String wikiTitle, String pageTitle, String subView, boolean isPublicOrAuthenticated) throws Exception {
		...
		if (subView.equals("Comments")){
			...
			if(!isPublicOrAuthenticated)
				...
				Assert.assertTrue("FAIL: Comments subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Subscribe_To_Comments_Text));
		}
			...
			if(!isPublicOrAuthenticated)
				...
				Assert.assertTrue("FAIL: Versions subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Subscribe_To_Versions_Text));
		}
			...
			if(!isPublicOrAuthenticated)
				...
				Assert.assertTrue("FAIL: Attachments subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Subscribe_To_Attachments_Text));
		}
			...
			if(!isPublicOrAuthenticated)
				...
				Assert.assertTrue("FAIL: Updated label doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Updated_Field));
				Assert.assertTrue("FAIL: Added label doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Added_Field));
				Assert.assertTrue("FAIL: Size label doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Size_Field));
				Assert.assertTrue("FAIL: Page views label doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.PageViews_Field));
				Assert.assertTrue("FAIL: Child Pages doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Child_Pages_Text));
				Assert.assertTrue("FAIL: Hierarchy label doesn't appear so About subview isn't opened automatically through address bar", sel.isTextPresent(WikisObjects.Hierarchy_Text));
			}
		}
	}
	...
	public void verifyParticularComment(String wikiTitle, String pageTitle, String [] commentUUID, String commentText, boolean isPublicOrAuthenticated) throws Exception {
		...
		if(!isPublicOrAuthenticated)
			...
			Assert.assertTrue("FAIL: Specified comment isn't opened automatically through address bar", sel.isTextPresent(commentText + 1));
		...
		if(!isPublicOrAuthenticated)
			...
			//Verify particular comment is visible
			Assert.assertTrue("FAIL: Specified comment isn't opened automatically through address bar", sel.isTextPresent(commentText + 15));
	}
	...
	public void verifyParticularVersion(String wikiTitle, String pageTitle, String versionUUID, boolean isPublicOrAuthenticated) throws Exception {
		...
		if(!isPublicOrAuthenticated)
			...
			Assert.assertTrue("FAIL: Specified version isn't opened automatically through address bar", sel.isTextPresent("Welcome [1] : Versions"));
			Assert.assertTrue("FAIL: Specified wiki & page aren't visible in breadcrumbs", sel.isTextPresent(wikiTitle + " >"));
		}
	}
	...
	public void verifyParticularComparison(String wikiTitle, String pageTitle, boolean isPublicOrAuthenticated) throws Exception {
		...
		if(!isPublicOrAuthenticated)
			...
			Assert.assertTrue("FAIL: Specified comparison isn't opened automatically through address bar", sel.isTextPresent("Welcome : Version Comparison"));
			Assert.assertTrue("FAIL: Specified wiki & page aren't visible in breadcrumbs", sel.isTextPresent(wikiTitle + " >"));
		}
	}
	...
	public void ValidateManageAccessPopup() throws Exception {
		...
		Assert.assertTrue("FAIL: Save button is missing", sel.isElementPresent(WikisObjects.Save_Button));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
		Assert.assertTrue("FAIL: Close icon is missing", sel.isElementPresent(WikisObjects.Close_Icon));	
	}
	...
	public void VerifyManageAccessUI(boolean CreateNewWiki) throws Exception {
		...
		Assert.assertTrue("FAIL: First radio button is invalid", sel.isElementPresent(WikisObjects.Radio_Button1));
		Assert.assertTrue("FAIL: Second radio button is invalid", sel.isElementPresent(WikisObjects.Radio_Button2));
		Assert.assertTrue("FAIL: Third radio button is invalid", sel.isElementPresent(WikisObjects.Radio_Button3));
		Assert.assertTrue("FAIL: Fourth radio button is invalid", sel.isElementPresent(WikisObjects.Radio_Button4));
		
		Assert.assertTrue("FAIL: Read Access heading is missing", sel.isTextPresent("Read Access:"));
		Assert.assertTrue("FAIL: Edit Access heading is missing", sel.isTextPresent("Edit Access:"));
	...
	public void ChangeUserAccessRightsAsAdministrator(String Role, String MemberType, String Name) throws Exception {
		...
		if (MemberType.equals("Group")){
			...
			else if (Name.equals(Data.Wiki_LDAP_Readers_Group))
				Typeahead = Data.Wiki_LDAP_Readers_Group_TypeAhead;
	...
	public void VerifyUserUIInTableFormat(String Username, String Role) throws Exception {
		...
		if (memberNotFound == false){
			Assert.assertTrue("FAIL: Members Edit link isn't appearing", sel.isElementPresent("xpath=id('list')/div[2]/table/tbody/tr["+index1+"]/td["+index2+"]/table/tbody/tr/td[2]/div[2]/ul/li[1]/a"));
	...
	public void ValidateEditMemberLightbox(String Username) throws Exception {
		...
		Assert.assertTrue("FAIL: Reader radio button is missing", sel.isElementPresent(WikisObjects.Reader_Role_Option_onEditMemberLightBox));
		Assert.assertTrue("FAIL: Editor radio button is missing", sel.isElementPresent(WikisObjects.Editor_Role_Option_onEditMemberLightBox));
		Assert.assertTrue("FAIL: Owner radio button is missing", sel.isElementPresent(WikisObjects.Owner_Role_Option_onEditMemberLightBox));
		Assert.assertTrue("FAIL: Role label is missing", sel.isTextPresent("Role:"));
		Assert.assertTrue("FAIL: on OK button", sel.isElementPresent(WikisObjects.Members_And_Permissions_OK));
		Assert.assertTrue("FAIL: on Cancel link", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void VerifyAccessIsCorrect(String Role) throws Exception {
		if (Role.equals("Owner")){
			Assert.assertTrue("FAIL: Owner radio button isn't selected", sel.isChecked(WikisObjects.Owner_Role_Option_onEditMemberLightBox));
		}
		else if (Role.equals("Editor")){
			Assert.assertTrue("FAIL: Editor radio button isn't selected", sel.isChecked(WikisObjects.Editor_Role_Option_onEditMemberLightBox));
		}
		else if (Role.equals("Reader")){
			Assert.assertTrue("FAIL: Reader radio button isn't selected", sel.isChecked(WikisObjects.Reader_Role_Option_onEditMemberLightBox));
		}
	}
	
	public void ValidateMembersAndRolesPopupContent() throws Exception {
		Assert.assertTrue("FAIL: on Members label", sel.isTextPresent("Members"));
		Assert.assertTrue("FAIL: on All Authenticated Users field", sel.isElementPresent(WikisObjects.All_Authenticated_Users));
		Assert.assertTrue("FAIL: on Anonymous Users field", sel.isElementPresent(WikisObjects.Anonymous_Users_Field));
		Assert.assertTrue("FAIL: on Owner field", sel.isElementPresent(WikisObjects.Owner_Field));
		Assert.assertTrue("FAIL: on User/Group dropdown", sel.isElementPresent(WikisObjects.User_Group_Dropdown));
		...
		Assert.assertTrue("FAIL: on User Role dropdown", sel.isElementPresent(WikisObjects.User_Role_Dropdown));
		...
		Assert.assertTrue("FAIL: on username textfield", sel.isElementPresent(WikisObjects.Username_Textfield));
		Assert.assertTrue("FAIL: on OK button", sel.isElementPresent(WikisObjects.Members_And_Permissions_OK));
		Assert.assertTrue("FAIL: on Cancel link", sel.isElementPresent(WikisObjects.Cancel_Link));	
	}
	...
	public void ValidateAddMembersPopupContent() throws Exception {
		...
		Assert.assertTrue("FAIL: Members label is missing", sel.isTextPresent("Add members:"));

		//Verify that Members can be added
		Assert.assertTrue("FAIL: Members dropdown is missing", sel.isElementPresent(WikisObjects.MembershipRolesUsersDropdown));
		...
		Assert.assertTrue("FAIL: Members textfield is missing", sel.isElementPresent(WikisObjects.Members_Textfield_Typeahead));
		
		//Verify that Groups can be added
		Assert.assertTrue("FAIL: Add groups link is missing", sel.isElementPresent(WikisObjects.AddGroups_Link));
		sel.click(WikisObjects.AddGroups_Link);
		Thread.sleep(1000);
		
		Assert.assertTrue("FAIL: Groups dropdown is missing", sel.isElementPresent(WikisObjects.MembershipRolesGroupsDropdown));
		...
		Assert.assertTrue("FAIL: Groups textfield is missing", sel.isElementPresent(WikisObjects.Groups_Textfield_Typeahead));
	
		Assert.assertTrue("FAIL: OK button is missing", sel.isElementPresent(WikisObjects.Members_And_Permissions_OK));
		Assert.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void ValidateAllPagesResultantPage(String AllPagesOption, String FullUsername) throws Exception {
		...
		if (AllPagesOption.equals("Index")){
			Assert.assertTrue("FAIL: Index page doesn't appear", sel.isElementPresent(WikisObjects.Pages_Index_Title));
			...
			Assert.assertTrue("FAIL: Pages Edited by Me page doesn't appear", sel.isElementPresent(WikisObjects.Pages_Edited_By_Me_Title));
			...
			Assert.assertTrue("FAIL: 'Pages Edited by ...' page doesn't appear", sel.isElementPresent(WikisObjects.Pages_Edited_By_Me_Title));
			String PEBM = sel.getText(WikisObjects.Pages_Edited_By_Me_Title);
			Assert.assertTrue("FAIL: Resultant page doesn't have the correct title for Pages Edited By Me", PEBM.equals("Pages Edited by " + FullUsername));
		}
	}
	...
	public void ValidatePageDisplayViews() throws Exception {
		Assert.assertTrue("FAIL: 'Display' label is missing", sel.isTextPresent("Display:"));
		
		if (sel.isElementPresent(WikisObjects.PressedTableIcon))
			Assert.assertTrue("FAIL: inactive list view icon isn't visible", sel.isElementPresent(WikisObjects.UnPressedListIcon));
		
		else if (sel.isElementPresent(WikisObjects.PressedListIcon))
			Assert.assertTrue("FAIL: inactive table view icon isn't visible", sel.isElementPresent(WikisObjects.UnPressedTableIcon));
	}
	
	public void ValidatePagePaginationUI() throws Exception {
		Assert.assertTrue("FAIL: top pagination bar is missing", sel.isVisible(WikisObjects.Page_Index_Top_Pagination));
		Assert.assertTrue("FAIL: 'Previous' in top pagination bar is missing", sel.isVisible(WikisObjects.Top_Previous_Page_Link_InActive));
		Assert.assertTrue("FAIL: bottom pagination bar is missing", sel.isVisible(WikisObjects.Page_Index_Bottom_Pagination));
		Assert.assertTrue("FAIL: 'Previous' in bottom pagination bar is missing", sel.isVisible(WikisObjects.Bottom_Previous_Page_Link_InActive));
		...
		if (NoOfWikis < 10){
			Assert.assertTrue("FAIL: 'Next' in top pagination bar is missing", sel.isVisible(WikisObjects.Top_Next_Page_Link_InActive));
			Assert.assertTrue("FAIL: 'Next' in bottom pagination bar is missing", sel.isVisible(WikisObjects.Bottom_Next_Page_Link_InActive));
			Assert.assertTrue("FAIL: Jump to page section shouldn't appear for a single page", !sel.isElementPresent(WikisObjects.Jump_To_Page));
		}
		else {
			Assert.assertTrue("FAIL: 'Next' in top pagination bar is missing", sel.isVisible(WikisObjects.Top_Next_Page_Link_Active));
			Assert.assertTrue("FAIL: 'Next' in bottom pagination bar is missing", sel.isVisible(WikisObjects.Bottom_Next_Page_Link_Active));
			Assert.assertTrue("FAIL: Jump to page section is missing", sel.isElementPresent(WikisObjects.Jump_To_Page));
		}

		Assert.assertTrue("FAIL: 'Show' text missing from bottom pagination bar is missing", sel.isTextPresent("Show"));
		if (sel.isElementPresent(WikisObjects.Link_toshow_10Members_perpage)){}else Assert.assertTrue("FAIL: numeric link '10' in bottom pagination bar is missing", sel.isElementPresent("xpath=id('10')"));		
		if (sel.isElementPresent(WikisObjects.Link_toshow_25Members_perpage)){}else Assert.assertTrue("FAIL: numeric link '25' in bottom pagination bar is missing", sel.isElementPresent("xpath=id('25')"));
		if (sel.isElementPresent(WikisObjects.Link_toshow_50Members_perpage)){}else Assert.assertTrue("FAIL: numeric link '50' in bottom pagination bar is missing", sel.isElementPresent("xpath=id('50')"));
		Assert.assertTrue("FAIL: Closing text missing from bottom pagination bar is missing", sel.isTextPresent("items"));
	}
	...
	public void ValidateSortByLinksInMainTabs() throws Exception {
		Assert.assertTrue("FAIL: Sort by Name Option is not present", sel.isElementPresent(WikisObjects.Sortby_Name_Option));
		Assert.assertTrue("FAIL: Sort by Created Option is not present", sel.isElementPresent(WikisObjects.Sortby_Created_Option));
		Assert.assertTrue("FAIL: Sort by Updated Option is not present", sel.isElementPresent(WikisObjects.Sortby_Updated_Option));
	}
	
	public void ValidateSortByLinksInPagesTableLinks() throws Exception {
		Assert.assertTrue("FAIL: Sort by Name option is missing in table view", sel.isElementPresent(WikisObjects.Sortby_Name_Option));
		Assert.assertTrue("FAIL: Sort by Updated option is missing in table view", sel.isElementPresent(WikisObjects.Sortby_Updated_Option));
		Assert.assertTrue("FAIL: Sort by Visits option is missing in table view", sel.isElementPresent(WikisObjects.Sortby_Visits_Option));
		Assert.assertTrue("FAIL: Sort by Recommendations option is missing in table view", sel.isElementPresent(WikisObjects.Sortby_Recommendations_Option));
		Assert.assertTrue("FAIL: Sort by Comments option is missing in table view", sel.isElementPresent(WikisObjects.Sortby_Comments_Option));
	}
	
	public void ValidateSortByLinksInPagesListLinks() throws Exception {
		Assert.assertTrue("FAIL: Sort by Name option is missing in list view", sel.isVisible(WikisObjects.Sortby_Name_Option));
		Assert.assertTrue("FAIL: Sort by Updated option is missing in list view", sel.isVisible(WikisObjects.Sortby_Updated_Option));
		Assert.assertTrue("FAIL: Sort by Created option is missing in list view", sel.isVisible(WikisObjects.Sortby_Created_Option));
		Assert.assertTrue("FAIL: Sort by Page Views option is missing in list view", sel.isVisible(WikisObjects.Sortby_Page_Views_Option));
		Assert.assertTrue("FAIL: Sort by Size option is missing in list view", sel.isVisible(WikisObjects.Sortby_Size_Option));
	}
	...
	public void ValidatePageWithoutContents(String FormatView) throws Exception {
		...
		Assert.assertTrue("FAIL: 'No items found' text doesn't appear", sel.isTextPresent("No items found"));
		Assert.assertTrue("FAIL: empty top pagination bar doesn't appear", sel.isElementPresent("xpath=id('list')/div[1]"));
		Assert.assertTrue("FAIL: empty bottom pagination bar doesn't appear", sel.isElementPresent("xpath=id('list')/div[3]"));
	}
	...
	public void VerifyPageInTableFormat() throws Exception {
		...
		Assert.assertTrue("FAIL: Page Icon doesn't exist in Table format", sel.isVisible(WikisObjects.Page_Icons));
		Assert.assertTrue("FAIL: Page link doesn't exist in Table format", sel.isVisible(WikisObjects.Page_Links));
		Assert.assertTrue("FAIL: Most Visited Icon doesn't exist in Table format", sel.isVisible(WikisObjects.PageIndex_MostVisitedIcon));
		Assert.assertTrue("FAIL: Most Recommended Icon doesn't exist in Table format", sel.isVisible(WikisObjects.PageIndex_MostRecommendedIcon));
		Assert.assertTrue("FAIL: Most Commented Icon doesn't exist in Table format", verifyCommentsIcon());
		Assert.assertTrue("FAIL: More link doesn't exist in Table format", sel.isVisible(WikisObjects.More_Link));
	
		clickLink(WikisObjects.More_Link);
		if (!sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link))
			Assert.assertTrue("FAIL: Tags link doesn't exist in Table format", sel.isElementPresent(WikisObjects.Add_tags_Link));
		Assert.assertTrue("FAIL: Download Page link doesn't exist in Table format", sel.isElementPresent(WikisObjects.Download_Page_Link));
	...
	public void VerifyPageInListFormat() throws Exception {
		...
		Assert.assertTrue("FAIL: Page icon doesn't exist in List format", sel.isVisible(WikisObjects.Page_Icons));
		Assert.assertTrue("FAIL: Page link doesn't exist in List format", sel.isVisible(WikisObjects.Page_Links));
		Assert.assertTrue("FAIL: More link doesn't exist in List format", sel.isVisible(WikisObjects.More_Link));
	}
	...
	public void ValidateWikiForAllowedCharacters() throws Exception {
		Assert.assertTrue("FAIL: on appearance of warning message for allowed characters",!sel.isTextPresent("Page name may not contain the following characters:"));
	}
	
	public void ValidatePageForDisallowedCharacters() throws Exception {
		Assert.assertTrue("FAIL: Warning message for invalid characters is incorrect", sel.isTextPresent("Page name may not contain the following characters:"));
		Assert.assertTrue("FAIL: Warning message link to insert allowed characters is incorrect", sel.isElementPresent("link=Replace invalid characters with '_'?"));
	}
	
	public void ValidateWikiForDisallowedCharacters() throws Exception {
		Assert.assertTrue("FAIL: Warning message for invalid characters is incorrect", sel.isTextPresent("Wiki title may not contain the following characters:"));
		Assert.assertTrue("FAIL: Warning message link to insert allowed characters is incorrect", sel.isElementPresent("link=Replace invalid characters with '_'?"));
	}
	...
	public void VerifyUIOfPagesListing() throws Exception {    
		...
		if (ParsedDigit > 10){
			Assert.assertTrue("FAIL: Top Next page link on Pages Index is missing", sel.isVisible(WikisObjects.Top_Next_Page_Link_Active ));
			Assert.assertTrue("FAIL: Bottom Next page link on Pages Index is missing", sel.isVisible(WikisObjects.Bottom_Next_Page_Link_Active ));
		}
		else{
			Assert.assertTrue("FAIL: Top Next page link on Pages Index is missing", sel.isVisible(WikisObjects.Top_Next_Page_Link_InActive ));
			Assert.assertTrue("FAIL: Bottom Next page link on Pages Index is missing", sel.isVisible(WikisObjects.Bottom_Next_Page_Link_InActive ));
		}
		Assert.assertTrue("FAIL: Top Previous page link on Pages Index is missing", sel.isVisible(WikisObjects.Top_Previous_Page_Link_InActive));
		Assert.assertTrue("FAIL: Bottom Previous page link on Pages Index is missing", sel.isVisible(WikisObjects.Bottom_Previous_Page_Link_InActive));
	}
	...
	public void verifyQuotaWarningMsg()throws Exception{
		...
		Assert.assertTrue("FAIL: Quota warning is incorrect", sel.isTextPresent("You are nearly at you wiki quota"));
		Thread.sleep(2000);
	}
	
	public void verifyQuotaExcess()throws Exception{
		//Verify that quota excess is as expected
		Assert.assertTrue("FAIL: Behaviour isn't correct for quota excess", sel.isTextPresent("You are nearly at you wiki quota"));
		Thread.sleep(2000);
	}
	...
	public void FunctionIfPageAlreadyExists() throws Exception {		
		if (sel.isTextPresent("A page or file with the name you have specified already exists. Please choose a different name.")){
			...
			//Assert.assertTrue("FAIL: on confirmation popup", sel.isConfirmationPresent());
			//Assert.assertTrue("FAIL: on confirmation text in popup", sel.getConfirmation().matches("^Are you sure you want to abandon your changes[\\s\\S]  Press OK to continue or cancel to return to editing\\.$"));
		}
	}
	...
	public void VerifyUIofFooterElementsonMyWikisMainpage() throws Exception {    
		...
		Assert.assertTrue("FAIL: IBM Connections heading isn't present", sel.isTextPresent(WikisObjects.IBM_Lotus_Connections_Heading));
		Assert.assertTrue("FAIL: Home link isn't present", sel.isElementPresent(WikisObjects.Home_Link));
		Assert.assertTrue("FAIL: Demo link isn't present", sel.isElementPresent(WikisObjects.Demo_Link));
		
		Assert.assertTrue("FAIL: Help heading isn't present", sel.isTextPresent(WikisObjects.Help_Heading));
		Assert.assertTrue("FAIL: Help link isn't present", sel.isElementPresent(WikisObjects.Help_Link));
		Assert.assertTrue("FAIL: IBM Lotus Support Forums link isn't present", sel.isElementPresent(WikisObjects.IBMLotusSupportForums_Link));
		
		Assert.assertTrue("FAIL: Tools heading isn't present", sel.isTextPresent(WikisObjects.Tools_Heading));
		
		Assert.assertTrue("FAIL: About heading isn't present", sel.isTextPresent(WikisObjects.About_Heading));
		Assert.assertTrue("FAIL: About link isn't present", sel.isElementPresent(WikisObjects.About_Tab));
		Assert.assertTrue("FAIL: IBM Connections on ibm.com Link isn't present", sel.isElementPresent(WikisObjects.IBMLotusConnectionson_ibmcom_Link));
		Assert.assertTrue("FAIL: Submit Feedback link isn't present", sel.isElementPresent(WikisObjects.Submit_Feedback_Link));
	}
	...
	public void VerifyUIofHeaderElementsonMyWikisMainpage() throws Exception {    
		...
		Assert.assertTrue("FAIL: Help link isn't present", sel.isElementPresent(WikisObjects.Help_Link));
		Assert.assertTrue("FAIL: Logout link isn't present", sel.isElementPresent(WikisObjects.Logout_Link));
		Assert.assertTrue("FAIL: Page Header Login Name isn't present", sel.isElementPresent(WikisObjects.Page_Header_Login_Name));
		
		Assert.assertTrue("FAIL: Page Header IBM Connections Image isn't present", sel.isElementPresent(WikisObjects.Page_Header_Lotus_Connections_Image));
		Assert.assertTrue("FAIL: Page Header Files Link isn't present", sel.isElementPresent(WikisObjects.Page_Header_Files_Link));
		Assert.assertTrue("FAIL: Page Header Wikis Link isn't present", sel.isElementPresent(WikisObjects.Page_Header_Wikis_Link));
		
		if (sel.isElementPresent(WikisObjects.PressedListIcon))
			Assert.assertTrue("FAIL: Table Display option isn't present", sel.isElementPresent(WikisObjects.UnPressedTableIcon));
		else
			Assert.assertTrue("FAIL: List Display option isn't present", sel.isElementPresent(WikisObjects.UnPressedTableIcon));
	...
	public void VerifyStartaNewWikiButtonFunctionalityonLatestUpdatestab() throws Exception {    
		...
		Assert.assertTrue("FAIL: Mandatory Field Indicator is not Present", sel.isElementPresent(WikisObjects.Mandatory_Field_Indicator));
		Assert.assertTrue("FAIL: Wiki Name Field is not Present", sel.isElementPresent(WikisObjects.Wiki_Name_Field));
		Assert.assertTrue("FAIL: Wiki Name Field is not Editable", sel.isEditable(WikisObjects.Wiki_Name_Field));
		Assert.assertTrue("FAIL: Members Field is not Present", sel.isElementPresent(WikisObjects.Members_Field));
		Assert.assertTrue("FAIL: Members Field is not Editable", sel.isEditable(WikisObjects.Members_Field));
		Assert.assertTrue("FAIL: Description Field is not Present", sel.isElementPresent(WikisObjects.Description_Field));
		Assert.assertTrue("FAIL: Description Field is not Editable", sel.isEditable(WikisObjects.Description_Field));
		Assert.assertTrue("FAIL: Tags Field is not Present", sel.isElementPresent(WikisObjects.Tags_Field));
		Assert.assertTrue("FAIL: Tags Field is not Editable", sel.isEditable(WikisObjects.Tags_Field));
		Assert.assertTrue("FAIL: Create Wiki Button is not Present", sel.isElementPresent(WikisObjects.Save_Button));
		Assert.assertTrue("FAIL: Cancel Link is not Present", sel.isElementPresent(WikisObjects.Cancel_Link));
	...
	public void clickItemInSearchDropdown(String dropdownItem) throws Exception {
		...
		Thread.sleep(100);
		for (; index < 200; index++){
			if (sel.isElementPresent("dijit_MenuItem_"+ index +"_text"))
				searchItem = sel.getText("dijit_MenuItem_"+ index +"_text");
	...
	public void validateErrorPage(boolean IsAuthenticated) throws Exception {
		...
		if (!IsAuthenticated)
			sel.isElementPresent(WikisObjects.LinktotheLoginPage);
	}
	...
	public void VerifyStartAWikiButtonFunctionality() throws Exception {    
		...
		Assert.assertTrue("FAIL: Members Field is not Present", sel.isElementPresent(WikisObjects.Members_Field));
		Assert.assertTrue("FAIL: Members Field is not Editable", sel.isEditable(WikisObjects.Members_Field));
		Assert.assertTrue("FAIL: Groups Field is not Present", sel.isElementPresent(WikisObjects.Groups_Field));
		Assert.assertTrue("FAIL: Groups Field is not Editable", sel.isEditable(WikisObjects.Groups_Field));
		Assert.assertTrue("FAIL: Description Field is not Present", sel.isElementPresent(WikisObjects.Description_Field));
		Assert.assertTrue("FAIL: Description Field is not Editable", sel.isEditable(WikisObjects.Description_Field));
		Assert.assertTrue("FAIL: Tags Field is not Present", sel.isElementPresent(WikisObjects.Tags_Field));
		Assert.assertTrue("FAIL: Tags Field is not Editable", sel.isEditable(WikisObjects.Tags_Field));
	...
	public void VerifyUIofFooterElementsonWikiServiceMainpage() throws Exception {    
		//Ensure that the Footer Headers and Footer links are present and correct		
		//Assert.assertTrue("FAIL: IBM Connections Heading is not Present", sel.isTextPresent(Objects.IBM_Lotus_Connections_Heading));
		//Assert.assertTrue("FAIL: Help Heading is not Present", sel.isTextPresent(Objects.Help_Heading));
		//Assert.assertTrue("FAIL: Tools Heading is not Present", sel.isTextPresent(Objects.Tools_Heading));
		//Assert.assertTrue("FAIL: About Heading is not Present", sel.isTextPresent(Objects.About_Heading));
		Assert.assertTrue("FAIL: Home Link is not Present", sel.isElementPresent(WikisObjects.Home_Link));
		Assert.assertTrue("FAIL: Demo Link is not Present", sel.isElementPresent(WikisObjects.Demo_Link));
		Assert.assertTrue("FAIL: Help Link is not Present", sel.isElementPresent(WikisObjects.Help_Link));
		Assert.assertTrue("FAIL: IBM Lotus Support Forums Link is not Present", sel.isElementPresent(WikisObjects.IBMLotusSupportForums_Link));
		Assert.assertTrue("FAIL: About Link is not Present", sel.isElementPresent(WikisObjects.About_Tab));
		Assert.assertTrue("FAIL: IBM Connections on ibm.com Link is not Present", sel.isElementPresent(WikisObjects.IBMLotusConnectionson_ibmcom_Link));
		Assert.assertTrue("FAIL: Submit Feedback Link is not Present", sel.isElementPresent(WikisObjects.Submit_Feedback_Link));
	}
	
	public void VerifyUIofHeaderElementsonWikiServiceMainpage() throws Exception {    
		//Ensure that the Header Elements Links are present and correct		
		Assert.assertTrue("FAIL: Help Link is not Present",sel.isElementPresent(WikisObjects.Help_Link));
		Assert.assertTrue("FAIL: Logout Link is not Present",sel.isElementPresent(WikisObjects.Logout_Link));
		Assert.assertTrue("FAIL: Page Header IBM Connections Image is not Present",sel.isElementPresent(WikisObjects.Page_Header_Lotus_Connections_Image));
		//Assert.assertTrue("FAIL: Page Header Files Link is not Present",sel.isElementPresent(Objects.Page_Header_Files_Link));
		//Assert.assertTrue("FAIL: Page Header Wikis Link is not Present",sel.isElementPresent(Objects.Page_Header_Wikis_Link));
		Assert.assertTrue("FAIL: Page Header Login Name is not Present",sel.isElementPresent(WikisObjects.Page_Header_Login_Name));
	...
	public void VerifyUIofTabsandWelcomeSectiononWikiServiceMainpage() throws Exception {
		...
		Assert.assertTrue("FAIL: Public Wikis tab is not Present",sel.isElementPresent(WikisObjects.Public_Wikis_Tab));
		Assert.assertTrue("FAIL: MyWikis tab is not Present",sel.isElementPresent(WikisObjects.My_Wikis_Tab));
		Assert.assertTrue("FAIL: Welcome Section is not Present",sel.isElementPresent(WikisObjects.Welcome_Section));
		Assert.assertTrue("FAIL: Learn More Button is not Present",sel.isElementPresent(WikisObjects.Learn_More_Button));
		Assert.assertTrue("FAIL: Watch Demo Button is not Present",sel.isElementPresent(WikisObjects.Watch_Demo_Button));	
		Assert.assertTrue("FAIL: Start a New Wiki Buton is not Present",sel.isElementPresent(WikisObjects.Start_New_Wiki_Button));	

	}
	
	public void VerifyPublicTagsUI() throws Exception {    
		//Ensure that all the elements under Public Tags heading are visible
		Assert.assertTrue("FAIL: Public Tag Search section isn't visible", sel.isElementPresent(WikisObjects.Public_Tags_Search_Section));
		Assert.assertTrue("FAIL: Public Tag Search field Search icon isn't visible", sel.isElementPresent(WikisObjects.Public_Tag_Search_Field_SearchIcon));
		Assert.assertTrue("FAIL: Public Tag Search field Tooltip isn't visible", sel.isElementPresent(WikisObjects.Tooltip_for_Public_Tags_Section));
	...
	public void VerifyMyTagsUI() throws Exception {    
		...
		Assert.assertTrue("FAIL: My Tag heading is incorrect", "Tags".equals(sel.getText(WikisObjects.Tags_Section_Title)));
		Assert.assertTrue("FAIL: My Tag expand/collapse icon isn't functioning correctly", ValidateOfExpandCollapseOfTagsUI());
		
		Assert.assertTrue("FAIL: My Tag Search section isn't visible", sel.isElementPresent(WikisObjects.My_Tags_Search_Section));
		Assert.assertTrue("FAIL: My Tag Search field Search icon isn't visible", sel.isElementPresent(WikisObjects.My_Tag_Search_Field_SearchIcon));
		Assert.assertTrue("FAIL: My Tag Search field Tooltip isn't visible", sel.isElementPresent(WikisObjects.Tooltip_for_My_Tags_Section));
		VerifyRemainingTagsUI();
	}
	...
	public void VerifyRemainingTagsUI() throws Exception {    
		...
		Assert.assertTrue("FAIL: Tag Search field isn't visible", sel.isElementPresent(WikisObjects.Tag_Search_Textfield));
		...
		if (!sel.isTextPresent(WikisObjects.Tags_NoTags_Text))
			Assert.assertTrue("FAIL: Link for Tag cloud isn't visible", sel.isElementPresent(WikisObjects.Link_for_Tag_Cloud));
	}
	...
	public void VerifyBacktoWikisLinkFunctionalityonUnAuthorisedAccesspage() throws Exception {    
		...
		Assert.assertTrue("FAIL: Public Wikis tab isn't visible", sel.isElementPresent(WikisObjects.Public_Wikis_Tab));
		Assert.assertTrue("FAIL: My Wikis tab isn't visible", sel.isElementPresent(WikisObjects.My_Wikis_Tab));
		Assert.assertTrue("FAIL: Public Wikis list isn't visible", sel.isElementPresent(WikisObjects.Public_Wikis_List));			
	}
	
	public void VerifythatthePageisOpenedinEditMode() throws Exception {    
	    //Method to verify that the Page is opened in Edit Mode
		Assert.assertTrue("FAIL: CK Editor isn't visible", sel.isElementPresent(WikisObjects.CKEditor));
		Assert.assertTrue("FAIL: Save and Close button isn't visible", sel.isElementPresent(WikisObjects.Save_and_Close_Link));
		Assert.assertTrue("FAIL: Cancel button isn't visible", sel.isElementPresent(WikisObjects.Cancel_Link));
		Assert.assertTrue("FAIL: Save button isn't visible", sel.isElementPresent(WikisObjects.Save_Link));
	}
	
	public void VerifytheUIofCreateaNewWikiPage() throws Exception {    
		//Verify the UI of the create new Wiki page
		Assert.assertTrue("FAIL: Page Name Field isn't present", sel.isElementPresent(WikisObjects.New_Page_Title_Textfield));
		Assert.assertTrue("FAIL: CK Editor isn't present", sel.isElementPresent(WikisObjects.CKEditor));	
		Assert.assertTrue("FAIL: Link to add the Tag isn't present", sel.isElementPresent(WikisObjects.Add_tags_Link));
		Assert.assertTrue("FAIL: Save and Close button isn't present", sel.isElementPresent(WikisObjects.Save_and_Close_Link));
		Assert.assertTrue("FAIL: Cancel button isn't present", sel.isElementPresent(WikisObjects.Cancel_Link));
		Assert.assertTrue("FAIL: Save button isn't present", sel.isElementPresent(WikisObjects.Save_Link));			
	}
	...
	public void VerifyNewPageCreation(String PageName, String PageContent) throws Exception {    
		...
		Assert.assertTrue("FAIL: Page is created but page title information isn't visible", sel.isTextPresent(PageName));
		Assert.assertTrue("FAIL: Page is created but tagging information isn't visible", sel.isElementPresent(WikisObjects.Add_tags_Link));
		Assert.assertTrue("FAIL: Page is created but breadcrumb information isn't visible", sel.isElementPresent(WikisObjects.All_Breadcrumb_Text));
		
		Assert.assertTrue("FAIL: Page title isn't correct", PageName.equals(sel.getText(WikisObjects.WikiHomePageTitleField)));
						
	    //Verify whether page content is visible
	    Assert.assertTrue("FAIL: Create New Page functionality is incorrect. Page content isn't displayed", sel.isTextPresent(PageContent));
	}	
	...
	public void VerifyUIofUnAuthorisedAccessofMyWikisPage() throws Exception {    
		...
		Assert.assertTrue("FAIL: The Error Message Heading is not Present", sel.isElementPresent(WikisObjects.ErrorMessageHeading));
		Assert.assertTrue("FAIL: The Link to the Login Page is not Present", sel.isElementPresent(WikisObjects.LinktotheLoginPage));
		Assert.assertTrue("FAIL: The Link to the Wiki Main Page is not Present", sel.isElementPresent(WikisObjects.BacktoWikis));						
	}
	...
	public void VerifytheUIofUnAuthorisedAccesstoCreatingaNewWikiPage() throws Exception {    
		...
		Assert.assertTrue("FAIL: The Error Message Heading isn't present", sel.isElementPresent(WikisObjects.ErrorMessageHeading));
		Assert.assertTrue("FAIL: The Error Message Text saying You can't create a new page isn't present", sel.isTextPresent("You can't create a new page"));
		Assert.assertTrue("FAIL: The Link to the Login Page isn't present", sel.isElementPresent(WikisObjects.BacktoPage));
		Assert.assertTrue("FAIL: The Link to the Wiki Main Page isn't present", sel.isElementPresent(WikisObjects.BacktoWikis));			
	...
	public void VerifytheUIofUnAuthorisedAccesstoEditaWikiPage() throws Exception {    
		...
		Assert.assertTrue("FAIL: The Error Message Heading isn't present", sel.isElementPresent(WikisObjects.ErrorMessageHeading));
		Assert.assertTrue("FAIL: The Error Message Text saying You Can't Edit this Page isn't present", sel.isTextPresent("You can't edit this page"));
		Assert.assertTrue("FAIL: The Link to the Login Page isn't present", sel.isElementPresent(WikisObjects.BacktoPage));
		Assert.assertTrue("FAIL: The Link to the Wiki Main Page isn't present", sel.isElementPresent(WikisObjects.BacktoWikis));				
	}
	...
	public void SetUptheHomePageforWikiwithOnlyOwnerasMemberfortheWiki() throws InterruptedException {
		...
		try{
			sel.open(WikisObjects.TestURL+"wikis/home?lang=en#/wiki/Wiki_with_NoMembers_Except_Owner/page/Wiki_with_NoMembers_Except_Owner"); 
	...
	public void VerifyTagsSectionUIOnPageView() throws Exception {    
		if (!sel.isTextPresent(WikisObjects.Tags_NoTags_Text)){
			...
			if(sel.isElementPresent(WikisObjects.Link_for_Find_a_Tag))
				clickLink(WikisObjects.Link_for_Find_a_Tag);
			Assert.assertTrue("FAIL: Tag Search field is missing", sel.isElementPresent(WikisObjects.Tag_Search_Textfield));
			Assert.assertTrue("FAIL: Tag Search field Submit button is missing", sel.isElementPresent(WikisObjects.Tag_Search_Field_SubmitButton));
	...
	public void VerifytheUIofUnAuthorisedAccesstoaWikiHomePagebyEnteringtheURLfortheWikiHomePage() throws Exception {    
		...
		Assert.assertTrue("FAIL: The Error Message Heading is not present",sel.isElementPresent(WikisObjects.ErrorMessageHeading));
		Assert.assertTrue("FAIL: The Error Message Heading is not present",sel.isTextPresent("You do not have access to this page"));
		Assert.assertTrue("FAIL: The Link to the Wiki Main Page is not present",sel.isElementPresent(WikisObjects.BacktoWikis));			
	...
	public void SetUptheCreateRootPageforWikiwithRolesAssignedtoIndividualUsers() throws InterruptedException {
		...
		//to use variable for the URL which is set in Objects. 
		 try{
			 sel.open(WikisObjects.TestURL+"wikis/home?lang=en#/wiki/Wiki1_with_MembersSelected_for_DifferentRoles/pages/create?rel=root"); 
			 sel.waitForPageToLoad("5000");
		}
		catch (Exception e){
		} 
	...
	public void SetUptheCreateChildPageforWikiwithRolesAssignedtoIndividualUsers() throws InterruptedException {
		...
		//to use variable for the URL which is set in Objects.
		 try{
		sel.open(WikisObjects.TestURL+"wikis/home?lang=en#/wiki/Wiki1_with_MembersSelected_for_DifferentRoles/pages/create"); 
		sel.waitForPageToLoad("5000");
		}
		catch (Exception e){
	...
	public void SetUptheCreatePeerPageforWikiwithRolesAssignedtoIndividualUsers() throws InterruptedException {
		...
		//to use variable for the URL which is set in Objects.
		 try{
		sel.open(WikisObjects.TestURL+"wikis/home?lang=en#/wiki/Wiki1_with_MembersSelected_for_DifferentRoles/pages/create?rel=sibling"); 
		sel.waitForPageToLoad("5000");
		}
		catch (Exception e){
		//System.err.println("ERROR: was the page loaded correctly????");
	...
	public void VerifyUIofUnAuthorisedAccessofCreateChildPagebyAnonymousUser() throws Exception {    
		...
		Assert.assertTrue("FAIL: Error message heading isn't visible", sel.isElementPresent(WikisObjects.ErrorMessageHeading));
		Assert.assertTrue("FAIL: Error message heading text isn't correct", sel.isTextPresent("Certain portions of Wikis can only be accessed when you are logged in."));
		Assert.assertTrue("FAIL: Login link isn't visible", sel.isElementPresent(WikisObjects.LinktotheLoginPage));
		Assert.assertTrue("FAIL: Back to Wikis link isn't visible", sel.isElementPresent(WikisObjects.BacktoWikis));			
	...
	public void VerifyUIofPageViewforaReaderoftheWiki() throws Exception {
		...
		Assert.assertFalse("FAIL: Add Tags or Add or Remove Tags link shouldn't be visible since a Reader has no privilege to Add Tags for the Wiki", sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link)||sel.isElementPresent(WikisObjects.Add_Tag_Link));
		Assert.assertFalse("FAIL: Create New Page Link shouldn't be visible since a Reader has no privilege to Add New Pages to the Wiki", sel.isElementPresent(WikisObjects.Create_New_Page_Link));
		Assert.assertTrue("FAIL: Index Link isn't displayed, a Reader has the privilege View Pages Index Sections for the Wiki", sel.isElementPresent(WikisObjects.Index_Link));
		Assert.assertTrue("FAIL: Members link isn't displayed, a Reader has the privilege to View Members Page",sel.isElementPresent(WikisObjects.Members_Link));
		Assert.assertTrue("FAIL: Members section isn't displayed, a Reader has the privilege to access the Members page", sel.isElementPresent(WikisObjects.Members_Expand_Collapse_Arrow));
		
		Assert.assertTrue("FAIL: Comments Tab isn't displayed", sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Versions Tab isn't displayed",sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Attachments Tab isn't displayed",sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: About Tab isn't displayed", sel.isElementPresent(WikisObjects.About_Tab));
		
		Assert.assertTrue("FAIL: Recommend link isn't displayed, a Reader has the privilege to Recommend a Wiki Page.", sel.isElementPresent(WikisObjects.Recommend_Link));
		Assert.assertTrue("FAIL: Recommend image isn't displayed, a Reader has the privilege to Recommend a Wiki Page.", sel.isElementPresent(WikisObjects.Recommend_Image));
		Assert.assertTrue("FAIL: Navigation table isn't displayed, a Reader has the privilege to View the Navigation Table of the Wiki.",sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
		Assert.assertTrue("FAIL: Navigation tree isn't displayed, a Reader has the privilege to View the Navigation Tree of the Wiki.",sel.isElementPresent(WikisObjects.Wiki_Nav_Tree));
		Assert.assertTrue("FAIL: 'Subscribe to this page' link isn't displayed, a Reader has the privilege to View the Subscribe to this Page Link.",sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));
		Assert.assertTrue("FAIL: 'Subscribe to these comments' link isn't displayed, a Reader has the privilege to Subscribe to these comments",sel.isElementPresent(WikisObjects.Subscribe_To_These_Comments_Link));
		Assert.assertTrue("FAIL: Page Actions Button isn't displayed, a Reader has the privilege to Work with Options in Page Actions.",sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Readers));
		Assert.assertFalse("FAIL: Wiki Actions Button shouldn't be visible since a Reader has no privilege to Access these Options", sel.isElementPresent(WikisObjects.Wiki_Actions_Button));
		Assert.assertFalse("FAIL: Edit button shouldn't appear", sel.isElementPresent(WikisObjects.Edit_Button));
		...
		this.clickLink(WikisObjects.Versions_Tab);
		Assert.assertTrue("FAIL: 'Subscribe to these page versions' link isn't displayed, a Reader has the privilege to Subscribe to these page versions",sel.isElementPresent(WikisObjects.Subscribe_To_These_Page_Versions_Link));
		...
		Assert.assertTrue("FAIL: 'Subscribe to these attachments' link isn't displayed, a Reader has the privilege to Subscribe to these attachments", sel.isElementPresent(WikisObjects.Subscribe_To_These_Attachments_Link));
		Assert.assertTrue("FAIL: Attachments tab title isn't displayed but should be for a member with Reader access", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertFalse("FAIL: Add an Attachment Link should not be Present.Since a Reader has no privilege to Add Attachments for the Page.", sel.isElementPresent(WikisObjects.AddanAttachment_Link));
	}
	...
	public void VerifyUIofPagesExpandAndCollapseMenuonPageView() throws Exception {   
		...
		Assert.assertTrue("FAIL: Go to page... link isn't displayed, an user has the privilege to jump to any page in the wiki",sel.isElementPresent(WikisObjects.Go_To_Page_Link));
		Assert.assertTrue("FAIL: I Edited Link isn't displayed, a user has the privilige to view the pages that he has edited before.",sel.isElementPresent(WikisObjects.IEdited_Link));
		Assert.assertTrue("FAIL: Edited By... link isn't displayed", sel.isElementPresent(WikisObjects.EditedBy_Link));
	}
	...
	public void VerifyUIofDateExpandAndCollapseMenuonPageView() throws Exception {   
		...
		Assert.assertTrue("FAIL: Today link isn't displayed, an user has the privilege to view pages that are updated Today in the wiki",sel.isElementPresent(WikisObjects.Date_Links_Today));
		Assert.assertTrue("FAIL: Last week link isn't displayed, an user has the privilige to view pages that are updated in the last week in the Wiki.",sel.isElementPresent(WikisObjects.Date_Links_Last_Week));
		Assert.assertTrue("FAIL: Last 30 days link isn't displayed, an user has the privilige to view pages that are updated in last 30 days in the Wiki.",sel.isElementPresent(WikisObjects.Date_Links_Last_30_Days));
		Assert.assertTrue("FAIL: Last year link isn't displayed, an user has the privilige to view pages that are updated in the last year in the Wiki.",sel.isElementPresent(WikisObjects.Date_Links_Last_Year));
	}
	public void VerifyUIofPageViewforanEditoroftheWiki() throws Exception {    
		...
		Assert.assertTrue("FAIL: Add Tags or Add or Remove Tags link is not displayed, an Editor has the privilege to Add Tags for the Wiki.",(sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link))||(sel.isElementPresent(WikisObjects.Add_tags_Link)));
		Assert.assertTrue("FAIL: Create New Page Link is not displayed, an Editor has the privilege to Add New Pages to the Wiki.",sel.isElementPresent(WikisObjects.Create_New_Page_Link));
		Assert.assertTrue("FAIL: Index Link is not displayed, an Editor has the privilege to view Pages Index section",sel.isElementPresent(WikisObjects.Index_Link));
		Assert.assertTrue("FAIL: Members link is not displayed, an Editor has the privilege to View members for the Wiki.",sel.isElementPresent(WikisObjects.Members_Link));
		Assert.assertTrue("FAIL: Members section isn't displayed, a Editor has the privilege to access the Members page", sel.isElementPresent(WikisObjects.Members_Expand_Collapse_Arrow));
		
		Assert.assertTrue("FAIL: Comments Tab is not displayed, an Editor has the privilege to Add Comments for the Wiki.",sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Versions Tab is not displayed, an Editor has the privilege to view page versions",sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Attachments Tab is not displayed, an Editor has the privilege to View the Attachments for the Page.",sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: About Page Tab is not displayed, an Editor has the privilege to View About Tab Info.",sel.isElementPresent(WikisObjects.About_Tab));
		
		Assert.assertTrue("FAIL: Recommend Image isn't displayed, an Editor has the privilege to Recommend a Wiki Page.",sel.isElementPresent(WikisObjects.Recommend_Image));
		Assert.assertTrue("FAIL: Recommend Link isn't displayed, an Editor has the privilege to Recommend a Wiki Page.",sel.isElementPresent(WikisObjects.Recommend_Link));
		Assert.assertTrue("FAIL: Navigation Table is not displayed, an Editor has the privilege to View the Navigation Table of the Wiki.",sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
		Assert.assertTrue("FAIL: Navigation Tree is not displayed, an Editor has the privilege to View the Navigation Tree of the Wiki.",sel.isElementPresent(WikisObjects.Wiki_Nav_Tree));
		Assert.assertTrue("FAIL: 'Subscribe to this page' link is Not displayed, an Editor has the privilege to Subscribe for the Page.",sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));
		
		Assert.assertTrue("FAIL: Edit Button is not displayed, an Editor has the privilege to Edit the Wiki.",sel.isElementPresent(WikisObjects.Edit_Button));
		Assert.assertTrue("FAIL: Page Actions Button is not displayed, an Editor has the privilege to Work with the Options in Page Actions.",sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Editors));
		Assert.assertFalse("FAIL: Wiki Actions Button shouldn't be visible since an Editor has no Privilege to Access these options",sel.isElementPresent(WikisObjects.Wiki_Actions_Button));

		Assert.assertTrue("FAIL: 'Subscribe to these 'comments' link isn't displayed, but an Editor has the privilege to Subscribe to page comments",sel.isElementPresent(WikisObjects.Subscribe_To_These_Comments_Link));	
		...
		Assert.assertTrue("FAIL: Subscribe to these page versions link isn't displayed", sel.isElementPresent(WikisObjects.Subscribe_To_These_Page_Versions_Link));
		...
		Assert.assertTrue("FAIL: Subscribe to these attachments link isn't displayed", sel.isElementPresent(WikisObjects.Subscribe_To_These_Attachments_Link));
		Assert.assertTrue("FAIL: Attachments heading isn't displayed", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: Add an Attachment isn't displayed", sel.isElementPresent(WikisObjects.AddanAttachment_Link));				
	}
	...
	public void VerifyUIofPageViewforaOwneroftheWiki() throws Exception {		
		...
		Assert.assertTrue("FAIL: Add Tags or Add or Remove Tags link is not displayed, an Editor has the privilege to Add Tags for the Wiki.",(sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link))||(sel.isElementPresent(WikisObjects.Add_tags_Link)));
		Assert.assertTrue("FAIL: Create New Page Link isn't displayed, an Owner has the privilege to Add New Pages to the Wiki.", sel.isElementPresent(WikisObjects.Create_New_Page_Link));
		Assert.assertTrue("FAIL: Members link is not displayed, an Owner has the privilege to View members for the Wiki.",sel.isElementPresent(WikisObjects.Members_Link));
		Assert.assertTrue("FAIL: Members section isn't displayed, an Editor has the privilege to access the Members page", sel.isElementPresent(WikisObjects.Members_Expand_Collapse_Arrow));
		Assert.assertTrue("FAIL: Index Link is not displayed, an Owner has the privilege to view Pages Index section",sel.isElementPresent(WikisObjects.Index_Link));
		
		Assert.assertTrue("FAIL: Comments Tab isn't displayed, an Owner has the privilege to Add Comments for the Wiki", sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Versions Tab is n't displayed, an Owner has the privilege to view Versions page", sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Attachments Tab isn't displayed, an Owner has the privilege to view the Attachments for the Page", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: About Tab isn't displayed, an Owner has the privilege to view About Info", sel.isElementPresent(WikisObjects.About_Tab));
		Assert.assertTrue("FAIL: Recommendation UI isn't displayed, an Owner has the privilege to Recommend a Wiki Page", sel.isElementPresent(WikisObjects.Recommendations_Info));
		Assert.assertTrue("FAIL: Navigation Table isn't displayed, an Owner has the privilege to View the Navigation Table of the Wiki", sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
		Assert.assertTrue("FAIL: Navigation Tree isn't displayed, an Owner has the privilege to view the Navigation Tree of the Wiki", sel.isElementPresent(WikisObjects.Wiki_Nav_Tree));
		Assert.assertTrue("FAIL: Subscribe to this Page Link isn't displayed, an Owner has the privilege to Subscribe for the Page", sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));
		Assert.assertTrue("FAIL: Subscribe to these Comments Link isn't displayed, an Owner has the privilege to Subscribe for Page comments.", sel.isElementPresent(WikisObjects.Subscribe_To_These_Comments_Link));
		
		Assert.assertTrue("FAIL: Edit Button isn't displayed, an Owner has the privilege to Edit the Wiki.", sel.isElementPresent(WikisObjects.Edit_Button));
		Assert.assertTrue("FAIL: Page Actions Button should be Present, an Owner has the privilege to Add New Child or Peer Pages to the Wiki Pages.", sel.isElementPresent(WikisObjects.Page_Actions_Button));
		Assert.assertTrue("FAIL: Wiki Actions Button isn't visible", sel.isElementPresent(WikisObjects.Wiki_Actions_Button));
		...
		clickLink(WikisObjects.Attachments_Tab);
		Assert.assertTrue("FAIL: Subscribe to these attachments link isn't displayed.", sel.isElementPresent(WikisObjects.Subscribe_To_These_Attachments_Link));
		Assert.assertTrue("FAIL: Attachments tab title is not displayed.", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: Add an Attachment is Not displayed.", sel.isElementPresent(WikisObjects.AddanAttachment_Link));		
	...
	public void VerifyUIofPageViewforanAnonymousUseroftheWiki() throws Exception {    
		...
		Assert.assertFalse("FAIL: Add or Remove Tags Link should not be Present. Since an Anonymous User has no Privilege to Add Tags for the Wiki.", sel.isElementPresent(WikisObjects.Add_or_RemoveTags_Link));
		Assert.assertFalse("FAIL: Edit Page Button should not be Present. Since an Anonymous User has no Privilege to Edit the Wiki.", sel.isElementPresent(WikisObjects.Edit_Button));
		Assert.assertFalse("FAIL: Create New Page Link should not be Present. Since an Anonymous User has no Privilege to Add New Pages to the Wiki.", sel.isElementPresent(WikisObjects.Create_New_Page_Link));
		//Assert.assertFalse("FAIL: Recommend a Page Image should not be Present. Since an Anonymous Reader has no Privilege to Recommend a Wiki Page.", sel.isElementPresent(Objects.Recommendations_Info));
		
		Assert.assertTrue("FAIL: Members Link is not present,but Anonymous User has Privilige to view Members Page.", sel.isElementPresent(WikisObjects.Members_Link));
		Assert.assertFalse("FAIL: Wiki Actions Button shouldn't be visible", sel.isElementPresent(WikisObjects.Wiki_Actions_Button));
		Assert.assertTrue("FAIL: About Tab is Not dislplayed but an Anonymous User has the Privilege to View About Tab Info.", sel.isElementPresent(WikisObjects.About_Tab));
		Assert.assertTrue("FAIL: Versions Tab is not displayed but an Anonymous User has the Privilege to View Versions Tab Info.", sel.isElementPresent(WikisObjects.Versions_Tab));
		Assert.assertTrue("FAIL: Index Link is not displayed but an Anonymous User has the Privilege to View Pages Index section.", sel.isElementPresent(WikisObjects.Index_Link));
		//Assert.assertTrue("FAIL: Search for Page Field is not displayed but an Anonymous User has the Privilege to Search for Pages in the Wiki.", sel.isElementPresent(Objects.Search_Page_Field));
		//Assert.assertTrue("FAIL: Search for Page Button is not displayed but an Anonymous User has the Privilege to Search for Pages in the Wiki.", sel.isElementPresent(Objects.Search_Page_Button));
		Assert.assertTrue("FAIL: Navigation Table is not displayed but an Anonymous User has the Privilege to View the Navigation Table of the Wiki.", sel.isElementPresent(WikisObjects.Wiki_Nav_Table));
		Assert.assertTrue("FAIL: Navigation Tree  is not displayed but an Anonymous User has the Privilege to View the Navigation Tree of the Wiki.", sel.isElementPresent(WikisObjects.Wiki_Nav_Tree));
		Assert.assertTrue("FAIL: Comments Tab is not displayed but an Anonymous Reader has the Privilege to View Comments for a Wiki Page.", sel.isElementPresent(WikisObjects.Comments_Tab));
		Assert.assertTrue("FAIL: Attachments Tab is Not displayed but an Anonymous User has the Privilege to View the Attachments for the Page.", sel.isElementPresent(WikisObjects.Attachments_Tab));
		Assert.assertTrue("FAIL: Subscribe to this Page Link is Not displayed but an Anonymous User has the Privilege to Subscribe for the Page.", sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));
		Assert.assertTrue("FAIL: Subscribe to these Comments Link is Not displayed but an Anonymous User has the Privilege to Subscribe to comments for the Page.", sel.isElementPresent(WikisObjects.Subscribe_To_These_Comments_Link));
		Assert.assertTrue("FAIL: Page Actions Button is not displayed but an Anonymous User has the Privilege to Work with Options in Page Actions.", sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Readers));
		Assert.assertTrue("FAIL: Login or Start Contributing Button is not displayed.", sel.isElementPresent(WikisObjects.LoginStartContributing_Button));
		...
		Assert.assertFalse("FAIL: Add a Comment Link should not be displayed since an Anonymous User has no privilege to add Comments for a Page.", sel.isVisible(WikisObjects.Add_Comment_Link));

		//Click on Versions Tab which is in the Page Footer.
		this.clickLink(WikisObjects.Versions_Tab);
		Assert.assertTrue("FAIL: Subscribe to these page versions link isn't displayed, an anonymous user has the privilege to Subscribe to these page versions",sel.isElementPresent(WikisObjects.Subscribe_To_These_Page_Versions_Link));
		
		//Click on Attachments Tab and verify that it isn't displaying Add an Attachment Link for an anonymous user
		this.clickLink(WikisObjects.Attachments_Tab);
		Assert.assertTrue("FAIL: Subscribe to these attachments link isn't displayed, an anonymous user has the privilege to Subscribe to these attachments",sel.isElementPresent(WikisObjects.Subscribe_To_These_Attachments_Link));
		Assert.assertTrue("FAIL: Attachments tab title is not displayed.But an Anonymous User has the privilege to View all the Attachments for a Page on the Attachments Page.",(sel.isElementPresent(WikisObjects.Attachments_Tab)));
		Assert.assertFalse("FAIL: Add an Attachment Link should not be Present.Since an Anonymous User has no privilege to Add Attachments for the Page.",(sel.isElementPresent(WikisObjects.AddanAttachment_Link)));
	...
	public void VerifyVersionsPageUIAsAReader() throws Exception {    
		...
		Assert.assertFalse("FAIL: Restore Link to Restore Second Most Recent Version should not be Present.Since a Reader has no privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreSecondMostRecent_Version)));
		Assert.assertFalse("FAIL: Restore Link to Restore Third Most Recent Version should not be Present.Since a Reader has no privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreThirdMostRecent_Version)));
		...
		Assert.assertFalse("FAIL: Delete drop down List to Delete the Prior Versions should not be Present.Since a Reader has no privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Versions_Delete_Dropdown)));
		Assert.assertFalse("FAIL: Delete Link next to Delete drop down List should not be Present.Since a Reader has no privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Delete_Link)));
	
		//Verify that for the User(bgossens) who is a Reader is able to see the View Links for the Versions in the Versions Page
		Assert.assertTrue("FAIL: View Link to View Second Most Recent Version is not displayed.But a Reader has the privilege to View any Previous Versions of a Page in the Wiki.", sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewSecondMostRecent_Version));
		Assert.assertTrue("FAIL: View Link to View Third Most Recent Version is not displayed.But a Reader has the privilege to View any Previous Versions of a Page in the Wiki.", sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewThirdMostRecent_Version));
		...
		Assert.assertTrue("FAIL: First Comparision Drop down List is not displayed.But a Reader has the privilege to View the Comparision between Versions of a Page in the Wiki.", sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown1));
		Assert.assertTrue("FAIL: Second Comparision Drop down List is not displayed.But a Reader has the privilege to View the Comparision between Versions of a Page in the Wiki.", sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown2));
		Assert.assertTrue("FAIL: Text before the Comparision Drop down Lists is not displayed.But a Reader has the privilege to View the Comparision between Versions of a Page in the Wiki.", sel.isTextPresent(WikisObjects.Versions_Compare_Text));
		Assert.assertTrue("FAIL: Show Comparision Link is not displayed.But a Reader has the privilege to View the Comparision between the Versions of a Page in the Wiki", sel.isElementPresent(WikisObjects.Versions_Show_Comparison_Link));
	...
	public void VerifyVersionsPageUIAsAnEditor() throws Exception {    
		...
		Assert.assertTrue("FAIL: Restore Link to Restore Second Most Recent Version is not displayed.But an Editor has the privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreSecondMostRecent_Version)));
		Assert.assertTrue("FAIL: Restore Link to Restore Third Most Recent Version is not displayed.But an Editor has the privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreThirdMostRecent_Version)));
		Assert.assertTrue("FAIL: View Link to View Second Most Recent Version is not displayed.But an Editor has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewSecondMostRecent_Version)));
		Assert.assertTrue("FAIL: View Link to View Third Most Recent Version is not displayed.But an Editor has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewThirdMostRecent_Version)));
		...
		Assert.assertTrue("FAIL: First Comparision Drop down List is not displayed.But an Editor has the privilege to View the Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown1)));
		Assert.assertTrue("FAIL: Second Comparision Drop down List is not displayed.But an Editor has the privilege to View the Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown2)));
		Assert.assertTrue("FAIL: Text before the Comparision Drop down Lists is not displayed.But an Editor has the privilege to View the Comparision between the Versions of a Page in the Wiki.",(sel.isTextPresent(WikisObjects.Versions_Compare_Text)));
		Assert.assertTrue("FAIL: Show Comparision Link is not displayed.But an Editor has the privilege to View the Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Show_Comparison_Link)));
	...
	public void VerifyVersionsPageUIAsAnOwner() throws Exception {    
		...
		Assert.assertTrue("FAIL: Restore Link to Restore Second Most Recent Version is not displayed.But a Owner has the privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreSecondMostRecent_Version)));
		Assert.assertTrue("FAIL: Restore Link to Restore Third Most Recent Version is not displayed.But a Owner has the privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreThirdMostRecent_Version)));
		Assert.assertTrue("FAIL: View Link to View Second Most Recent Version is not displayed.But a Owner has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewSecondMostRecent_Version)));
		Assert.assertTrue("FAIL: View Link to View Third Most Recent Version is not displayed.But a Owner has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewThirdMostRecent_Version)));
		...
		Assert.assertTrue("FAIL: Delete drop down List to Delete the Prior Versions is not displayed.But a Owner has the privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Versions_Delete_Dropdown)));
		Assert.assertTrue("FAIL: Text before the Delete drop down List is not displayed.But a Owner has the privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isTextPresent(WikisObjects.Versions_Deletion_Text)));
		Assert.assertTrue("FAIL: Delete Link next to the Delete drop down List is not displaayed.But a Owner has the privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Delete_Link)));
		...
		Assert.assertTrue("FAIL: First Comparision Drop down List is not displayed.But a Owner has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown1)));
		Assert.assertTrue("FAIL: Second Comparision Drop down List is not displayed.But a Owner has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown2)));
		Assert.assertTrue("FAIL: Text before the Comparision Drop down Lists is not displayed.But a Owner has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isTextPresent(WikisObjects.Versions_Compare_Text)));
		Assert.assertTrue("FAIL: Show Comparision Link is not displayed.But a Owner has the privilege to View Comparision between Versions of a Page in the Wiki",(sel.isElementPresent(WikisObjects.Versions_Show_Comparison_Link)));
	...
	public void VerifyVersionsPageUIAsAnAnonymousUser() throws Exception {    
		...
		Assert.assertFalse("FAIL: Restore Link to Restore Second Most Recent Version should not be Present.Since an Anonymous User has no privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreSecondMostRecent_Version)));
		Assert.assertFalse("FAIL: Restore Link to Restore Third Most Recent Version should not be Present.Since an Anonymous User has no privilege to Restore a Page to Previous Versions in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_Restore_Link_to_RestoreThirdMostRecent_Version)));
		...
		Assert.assertFalse("FAIL: Delete drop down List to Delete the Prior Versions should not be Present.Since an Anonymous User has no privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Versions_Delete_Dropdown)));
		Assert.assertFalse("FAIL: Delete Link next to the Delete drop down List should not be Present.Since an Anonymous User has no privilege to Delete Previous Versions of a Page in the Wiki.",(sel.isVisible(WikisObjects.Delete_Link)));
	
		//Verify that for the Anonymous User is able to see the View Links for the Versions in the Versions page
		Assert.assertTrue("FAIL: View Link to View Second Most Recent Version is not displayed.But an Anonymous User has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewSecondMostRecent_Version)));
		Assert.assertTrue("FAIL: View Link to View Third Most Recent Version is not displayed.But an Anonymous User has the privilege to View any Previous Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Page_View_Link_to_ViewThirdMostRecent_Version)));
		...
		Assert.assertTrue("FAIL: First Comparision Drop down List is not displayed.But Anonymous User has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown1)));
		Assert.assertTrue("FAIL: Second Comparision Drop down List is not displayed.But Anonymous User has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Comparison_Dropdown2)));
		Assert.assertTrue("FAIL: Text before the Comparision Drop down Lists is not displayed.But Anonymous User has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isTextPresent(WikisObjects.Versions_Compare_Text)));
		Assert.assertTrue("FAIL: Show Comparision Link is not displayed.But Anonymous User has the privilege to View Comparision between Versions of a Page in the Wiki.",(sel.isElementPresent(WikisObjects.Versions_Show_Comparison_Link)));
	}

	public void VerifyAboutTabUI(String Role) throws Exception {    
		//Verify the UI of About Tab for any user
		Assert.assertTrue("FAIL: About tab title is missing, all users have the privilege to view About page", sel.isElementPresent(WikisObjects.About_Tab));
		Assert.assertTrue("FAIL: Updated Field is missing, all users have the privilege to view Updated information in About", sel.isTextPresent(WikisObjects.Updated_Field + ":"));
		Assert.assertTrue("FAIL: Created Field is missing, all users have the privilege to view Created information in About", sel.isTextPresent(WikisObjects.Created_Field + ":"));
		Assert.assertTrue("FAIL: Size Field is missing, all users have the privilege to view Size information in About", sel.isTextPresent(WikisObjects.Size_Field + ":"));
		Assert.assertTrue("FAIL: Page views is missing, all users have the privilege to view Page views in About", sel.isTextPresent(WikisObjects.PageViews_Field + ":"));
		Assert.assertTrue("FAIL: Hierarchy label is missing, all users should see the hierarchy of a Page in About", sel.isTextPresent(WikisObjects.Hierarchy_Text + ":"));
		Assert.assertTrue("FAIL: Child Pages Heading is missing, all users have the privilege to view child pages in About", sel.isTextPresent(WikisObjects.Child_Pages_Text));

		if ((Role.equals("Owner"))||(Role.equals("Editor")))
			Assert.assertTrue("FAIL: Create New Child Page link should appear for Owners & Editors, as they have privilege to add child pages", sel.isElementPresent(WikisObjects.Create_newChildPage_Link));
		else
			Assert.assertFalse("FAIL: Create New Child Page link shouldn't appear for users with less than Editor access, as they don't have privilege to add child pages", sel.isElementPresent(WikisObjects.Create_newChildPage_Link));
	}
	...
	public void verifyRecommendationUpdatesToIcons(int pageRecommends, int index) throws Exception {
		...
		Assert.assertTrue("FAIL: First line of popup text for recommendation info is incorrect", sel.isTextPresent("You have recommended this page."));
			
		if (pageRecommends >= 1)
			Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(pageRecommends + " other person recommended this:"));
		else
			Assert.assertTrue("FAIL: Second line of popup text for recommendation info is incorrect", sel.isTextPresent(pageRecommends + " other people recommended this:"));
	}
	...
	public void selectMemberFromUserDropdown(String UserName) throws Exception {
		...
		Assert.assertTrue("FAIL: Member names dropdown list isn't visible", sel.isElementPresent(WikisObjects.Members_Textfield_Typeahead));
	...
	public void selectMemberFromGroupDropdown(String GroupName) throws Exception {
		Assert.assertTrue("FAIL: Group names typeahead dropdown list isn't visible", sel.isElementPresent(WikisObjects.Groups_Textfield_Typeahead));
	...
	public void verifyCorrectlyFilteredWikisInMyWikis(String wikiPartName, int loopInt) throws Exception {    
		...
		for (;index < totalWikisCreated; index++){
			Assert.assertTrue("FAIL: Wiki " + wikiPartName + index + " doesn't exist", sel.isElementPresent("link=" + wikiPartName + index));
		}
	}
	...
	public void checkChangedMembership(String newRole, String oldRole, String wikiName) throws Exception {    
		...
		if ((oldRole.equals(Data.Owner_Role))&&(newRole.equals(Data.Editor_Role))){
			...
			Assert.assertTrue("FAIL: Wiki " + wikiName + " shouldn't appear in this filter", !sel.isElementPresent(wikiName));
			
			this.clickLink(WikisObjects.MyWikis_Editor_Filter);
			Assert.assertTrue("FAIL: Wiki " + wikiName + " doesn't appear in this filter", sel.isElementPresent(wikiName));
		}
	...
	public boolean findWiki(String wikiTitle) throws Exception {    	
		...
		while (!sel.isElementPresent("link=" + wikiTitle)){
			...
			else if ((sel.isElementPresent(WikisObjects.Top_Next_Page_Link_InActive)) & (!sel.isElementPresent("link=" + wikiTitle)))
				break;
		}
	...
	public void VerifySpecialGroupsDropdownListandClickonAuthenticatedSpecialGroupintheDropdownList() throws Exception {    
		...
		Assert.assertTrue("FAIL: Wiki_Group_Names_dropdownList is not present.",(sel.isElementPresent(WikisObjects.Add_Groups_Dropdown)));
		Assert.assertTrue("FAIL: Wiki_Group_Names_dropdownList is not bringing the Matching Groups in the Directory.",("All Authenticated Users".equals(sel.getText("xpath=//*[contains(@id,'_selectGroup_popup1')]"))));
	...
	public void VerifySpecialGroupsDropdownListandClickonAnonymousUserpecialGroupintheDropdownList() throws Exception {    
		...
		Assert.assertTrue("FAIL: Wiki_Group_Names_dropdownList is not present.",(sel.isElementPresent(WikisObjects.Add_Groups_Dropdown)));
		Assert.assertTrue("FAIL: Wiki_Group_Names_dropdownList is not bringing the Matching Groups in the Directory.",("Anonymous User".equals(sel.getText("xpath=//*[contains(@id,'_selectGroup_popup0')]"))));
	...
	public void VerifyEditingPrivilegefortheUser(String TexttobeAdded) throws Exception {    
		...
		Assert.assertTrue("FAIL: Text typed in the Editor isn't present on the page", sel.isTextPresent(TexttobeAdded));
	}
	...
	public void openPageActionsAsReader() throws Exception {
		Assert.assertTrue("FAIL: Page Actions button is missing", sel.isElementPresent(WikisObjects.Page_Actions_Button_For_Readers));
	...
	public void VerifyLoginForm() throws Exception {
		...
		for(int i=1;i<=30;i++)
		{
			if (sel.isElementPresent(WikisObjects.WIKI_USERNAME_FIELD))
				break;
			Thread.sleep(1000);
		}
		//Verify that Username,Password and Login buttons are availabel on Login form
		Assert.assertTrue("Username field is not present", sel.isElementPresent(WikisObjects.WIKI_USERNAME_FIELD));
		Assert.assertTrue("Password field is not present", sel.isElementPresent(WikisObjects.WIKI_Password_FIELD));
		Assert.assertTrue("Login Button is not present", sel.isElementPresent(WikisObjects.WIKI_Login_Button));		
	}
	...
	public boolean FindWikiNameInMyWikisList(String Wiki_Title) throws Exception {
		...
		if(WhatBrowser.equals("Netscape")){
			...
			while (!sel.isElementPresent("link=" + Wiki_Title)){
				...
				if ((sel.isElementPresent(Objects.Top_Next_Page_Link_InActive ))&&(!sel.isElementPresent("link=" + Wiki_Title))){
					Wiki_Found=false;
					break;
				}
			}
	...
	public void VerifyUIofDeleteWikiLightBox() throws Exception {    
		...
		Assert.assertTrue("FAIL: Delete Wiki Light Box Heading Text is not Present", sel.isTextPresent(" Are you sure you want to delete this wiki?"));
		Assert.assertTrue("FAIL: Delete Wiki Light Box Body Text is not Present", sel.isTextPresent("Deleting this wiki will permanently delete all data associated with this wiki. This action cannot be undone, so make sure you want to delete this wiki."));
		Assert.assertTrue("FAIL: Delete this Wiki Button is not Present", sel.isElementPresent(WikisObjects.Delete_Button));
		Assert.assertTrue("FAIL: Cancel Link is not Present", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void VerifyUIofDeletePageLightBox() throws Exception {    
		...
		Assert.assertTrue("FAIL: Delete Page Light Box Heading Text is not Present", sel.isTextPresent("Are you sure you want to move page"));
		Assert.assertTrue("FAIL: Delete this Wiki Button is not Present", sel.isElementPresent(WikisObjects.OK_Button));
		Assert.assertTrue("FAIL: Cancel Link is not Present", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public boolean VerifyPageExists(String pageName) throws Exception {
		...
		if (sel.isElementPresent("link=" + pageName))
			foundPage = true;
		...
		while(sel.isElementPresent("xpath=//div[@id='list']/div[2]/table/tbody/tr[" + index1 + "]/td[" + index2 + "]/table/tbody/tr/td[2]/h4/a[1]")){
			while(sel.isElementPresent("xpath=//div[@id='list']/div[2]/table/tbody/tr[" + index1 + "]/td[" + index2 + "]/table/tbody/tr/td[2]/h4/a[1]")){
				...
				if(UsernameText.contains(UserFullName)){
					if(sel.isElementPresent("xpath=id('list')/div[2]/table/tbody/tr[" + index1 + "]/td[" + index2 + "]/table/tbody/tr/td[2]/div[2]/ul/li/a"))
						linkCount=1;
				}
				index2++;
			}
	...
	public void VerifyDisplayofPageTitleandLinkforPageTitle(String PageTitle,String PageLink) throws Exception {    
		...
		Assert.assertTrue("FAIL: Wiki Title is not Present",(sel.getText(WikisObjects.WikiHomePageTitleField)).equals(PageTitle));
		Assert.assertTrue("FAIL: Link for Wiki Title is not Present",sel.isElementPresent(PageLink));
	}
	...
	public void VerifyWikisGenericListUI() throws Exception {    
		...
		Assert.assertTrue("FAIL: Next Page Link at the Top of the list isn't present",sel.isElementPresent(WikisObjects.Next_PageLink_atthe_Topof_PublicWikisList));
		Assert.assertTrue("FAIL: Previous Page Link at the Top of the list isn't present",sel.isElementPresent(WikisObjects.Previous_PageLink_atthe_Topof_PublicWikisList));
		Assert.assertTrue("FAIL: Next Page Link at the Bottom of the list isn't present",sel.isElementPresent(WikisObjects.Next_PageLink_atthe_Bottomof_PublicWikisList));
		Assert.assertTrue("FAIL: Previous Page Link at the Bottom of the list isn't present",sel.isElementPresent(WikisObjects.Previous_PageLink_atthe_Bottomof_PublicWikisList));
		
		Assert.assertTrue("FAIL: Link toshow 25 Wikis per Page on the list isn't present",sel.isElementPresent(WikisObjects.Link_toshow_25Wikis_perpage));	
		Assert.assertTrue("FAIL: Link to show 50 Wikis per Page on the list isn't present",sel.isElementPresent(WikisObjects.Link_toshow_50Wikis_perpage));	
		Assert.assertTrue("FAIL: PageId showing 10 Wikis per Page on the Public Wikis Main Page isn't present",sel.isElementPresent(WikisObjects.PageID_Showing_10Wikis_perpage));
	...
	public void VerifyWikisListUIInTableView() throws Exception {    
		...
		Assert.assertTrue("FAIL: Sort by Name Option isn't present",sel.isElementPresent(WikisObjects.Sortby_Name_Option));
		Assert.assertTrue("FAIL: Sort by Created Option isn't present",sel.isElementPresent(WikisObjects.Sortby_Created_Option));
		Assert.assertTrue("FAIL: Sort by Updated Option isn't present",sel.isElementPresent(WikisObjects.Sortby_Updated_Option));	
		Assert.assertTrue("FAIL: 'Subscribe to public wikis' link isn't present", sel.isElementPresent(WikisObjects.Subscribe_To_Public_Wikis_Link));
	}
	...
	public void VerifyWikisListUIInListView() throws Exception {    
		...
		if (!sel.isTextPresent(WikisObjects.No_Wikis_In_List)){
			VerifyWikisGenericListUI();
			Assert.assertTrue("FAIL: Sort by Name Option isn't present", sel.isElementPresent(WikisObjects.Sortby_Name_Option));
			Assert.assertTrue("FAIL: Sort by Added Option isn't present", sel.isElementPresent(WikisObjects.Sortby_Created_Option));
			Assert.assertTrue("FAIL: Sort by Updated Option isn't present", sel.isElementPresent(WikisObjects.Sortby_Updated_Option));
		}
		Assert.assertTrue("FAIL: 'Subscribe to public wikis' link isn't present", sel.isElementPresent(WikisObjects.Subscribe_To_Public_Wikis_Link));		
	}
	...
	public void CancelCreateAWiki(String WikiName, String TagName, String Description) throws InterruptedException{
		...
		try
		{
			//Assert.assertTrue("FAIL: the lightbox to confirm cancelling of the wiki has not appeared", sel.isTextPresent("Confirm"));
			sel.chooseOkOnNextConfirmation();
		...
		try 
		{
			Assert.assertTrue("FAIL: 'I'm a Reader' is not present after I cancelled a wiki", sel.isTextPresent("I'm a Reader"));
	...
	public void WaitforPagetoLoad (String ObjectName) throws InterruptedException
	{
		for (int second = 0;; second++) 
		{
			...
			try { if (sel.isElementPresent(ObjectName)) break; } catch (Exception e) {}
			Thread.sleep(1000);
		}	}
	 ...
	 */
		...
		public void PrintPage ( ) throws InterruptedException
		{
			...
			do{
				if (sel.isElementPresent("xpath=id('dijit_MenuItem_"+index+"')/td[2]"))
					temp = sel.getText("xpath=id('dijit_MenuItem_"+index+"')/td[2]");
		...
		public void FollowAWiki(String FollowingOption, String FollowingOptionText, String ObjectUIMessage, String UIMessage)
		{
			try
			{
				...
				Thread.sleep(1000);
				Assert.assertTrue("Fail - the following message is not present in the UI", sel.getText(ObjectUIMessage).equals(UIMessage));
			}
			catch (Exception e)
			{
		...
		public void StopFollowingAWiki(String StopFollowingOption, String StopFollowingOptionText, String ObjectStopUIMessage, String UIStopMessage)
		{
			try
			{
				...
				Thread.sleep(2000);
				Assert.assertTrue("Fail - the following message is not present in the UI", sel.getText(ObjectStopUIMessage).equals(UIStopMessage));
			}
			catch (Exception e)
			{
		...
		public void VerifyFollowingFilterObjectsAfterMultipleWikisFollowed () throws Exception
		{
			...
			try
			{
				...
				for (int i = 1; i <= 4; i++) 
				{
					...
					Assert.assertTrue("FAIL: Pagination is not correct for this filter", sel.getText("css=ul[class='lotusInlinelist'] li:nth-child("+i+")").matches(aString));
					Assert.assertTrue(!sel.isTextPresent("undefined"));
				}
				
				//Verify the ... in the page links
				Assert.assertTrue("FAIL: check for ecilpse in the pagination failed", sel.getText("css=ul[class='lotusInlinelist'] li:nth-child(5)").matches("..."));
