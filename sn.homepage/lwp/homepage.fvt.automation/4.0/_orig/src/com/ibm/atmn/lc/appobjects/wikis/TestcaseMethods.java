...
package com.ibm.atmn.lc.appobjects.wikis;

import com.ibm.atmn.lc.tasks.wikis.WikisMethods;

import org.testng.Assert;
import org.testng.AssertJUnit;

public class TestcaseMethods extends WikisMethods {
	...
	public void ValidatePageNotCreatedYet() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Some descriptive text isn't correct", sel.isTextPresent(WikisObjects.NotCreatedTextOnCreatePage));
		AssertJUnit.assertTrue("FAIL: Some descriptive text isn't correct", sel.isTextPresent(WikisObjects.DescriptiveTextOnCreatePage));
		AssertJUnit.assertTrue("FAIL: problem with 'Create This Page' button", sel.isElementPresent(WikisObjects.Create_This_Page_Button));
		AssertJUnit.assertTrue("FAIL: problem with 'take me back' link", sel.isElementPresent(WikisObjects.TakeMeBackOnCreatePage));
	}
	...
	public void CheckPagesSectionUI() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Pages Expand/Collapse arrow is missing", sel.isElementPresent(WikisObjects.Pages_Expand_Collapse_Arrow));
		...
		AssertJUnit.assertTrue("FAIL: Go to Page... link is missing", sel.isVisible(WikisObjects.Go_To_Page_Link));
		AssertJUnit.assertTrue("FAIL: I Edited link is missing", sel.isVisible(WikisObjects.IEdited_Link));
		AssertJUnit.assertTrue("FAIL: Edited by... link is missing", sel.isVisible(WikisObjects.EditedBy_Link));
	}

	public void CheckTagsSection() throws Exception {

		AssertJUnit.assertTrue("FAIL: Tags Expand/Collapse arrow is missing", sel.isElementPresent(WikisObjects.Tags_Expand_Collapse_Arrow));
		...
		if (ValidateTagsSectionIsCollapsed()) clickLink(WikisObjects.Tags_Expand_Collapse_Arrow);

		if (!sel.isTextPresent("No tags yet")) this.CheckTagsLinks();
	}

	public void CheckDateSection() throws Exception {

		AssertJUnit.assertTrue("FAIL: Tags Expand/Collapse arrow is missing", sel.isElementPresent(WikisObjects.Date_Expand_Collapse_Arrow));
		...
		if (ValidateDateSectionIsCollapsed()) clickLink(WikisObjects.Date_Expand_Collapse_Arrow);
	...
	public void CheckMembersSection() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Members Expand/Collapse arrow is missing", sel.isElementPresent(WikisObjects.Members_Expand_Collapse_Arrow));
		...
		if (ValidateMembersSectionIsCollapsed()) clickLink(WikisObjects.Members_Expand_Collapse_Arrow);
	}
	...
	public boolean ValidateTagsSectionIsCollapsed() throws Exception {
		...
		if (sel.isElementPresent(WikisObjects.Tags_Expanded_Arrow)) PopularTagsCollapseState = false;
	...
	public boolean ValidateDateSectionIsCollapsed() throws Exception {
		...
		if (!sel.isTextPresent(WikisObjects.Date_Links_Last_Year)) DateCollapseState = true;
	...
	public boolean ValidateAllPagesSectionIsCollapsed() throws Exception {
		...
		if (!sel.isVisible(WikisObjects.Go_To_Page_Link)) AllPagesCollapseState = true;
	...
	public boolean ValidateMembersSectionIsExpanded() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: View all link is missing", sel.isElementPresent(WikisObjects.View_All_Link));

		//Calling ElementCount method to parse the numeric value of the link
		int CurrentNumberOfMembers = WikisMethods.ElementCount(sel.getText(WikisObjects.View_All_Link));

		for (index = 1; index <= CurrentNumberOfMembers; index++)
			AssertJUnit.assertTrue("FAIL: Member profile is missing", sel.isElementPresent("xpath=//*[contains(@id,'lconn_share_widget_MembersSummary_')]/div[1]/div[" + index
					+ "]/a/img"));

		index++;

		//Assert that the number of profiles in Members section matches the number in the link
		AssertJUnit.assertTrue("FAIL: Member profiles don't match the number shown in the link", !sel
				.isElementPresent("xpath=//*[contains(@id,'lconn_share_widget_MembersSummary_')]/div[1]/div[" + index + "]/a/img"));
	...
	public boolean ValidateMembersSectionIsCollapsed() throws Exception {
		...
		if (!sel.isElementPresent(WikisObjects.View_All_Link)) MembersCollapseState = true;
	...
	public void SelectCorrectUserNameFromEditedByDropdown(String UserName) throws Exception {
		...
		if (!UserName.contains("Amy")) UserName = "Amy " + UserName;
	...
	public void ActivateAllPagesSearch(String BrowserType) throws Exception {
		...
		else clickLink(WikisObjects.Search_Page_Button);
	}

	public void SelectCreateThisPage() throws Exception {

		AssertJUnit.assertTrue("FAIL: Problem encountered with Create this page screen", sel.isElementPresent(WikisObjects.Create_This_Page_Button));
		clickLink(WikisObjects.Create_This_Page_Button);
	}

	public void SelectPageSearch() throws Exception {

		AssertJUnit.assertTrue("FAIL: Page Search field is missing", sel.isElementPresent(WikisObjects.Search_Page_Field));
	...
	public void ValidateNewTitleInEditMode(String NewTitle) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Editable page title field isn't visible", sel.isElementPresent(WikisObjects.New_Page_Title_Textfield));
		String EditTitle = sel.getValue(WikisObjects.New_Page_Title_Textfield);

		AssertJUnit.assertTrue("FAIL: New title doesn't appear in edit title textfield", EditTitle.equals(NewTitle));
	}

	public void ValidateTypeAheadDropdown() throws Exception {

		AssertJUnit.assertTrue("FAIL: Page Search popup is missing", sel.isElementPresent(WikisObjects.Page_Search_popup));
	}
	...
	public void CheckTypeAheadEntries(String TypeAheadValue) throws Exception {
		...
		if (TypeAheadValue.equals("PagesSearch")) {
			/*
			 * AssertJUnit.assertTrue("FAIL: First peer page in type ahead field is missing",
			 * Data.New_Page_To_Verify_Page_Search1 .equals(sel.getText("xpath=//li[@id='pagenavsearchbox_popup0']")));
			 * AssertJUnit.assertTrue("FAIL: Second peer page in type ahead field is missing",
			 * Data.New_Page_To_Verify_Page_Search2 .equals(sel.getText("xpath=//li[@id='pagenavsearchbox_popup1']")));
			 * AssertJUnit.assertTrue("FAIL: First child page in type ahead field is missing",
			 * Data.New_Page_To_Verify_Page_Search3 .equals(sel.getText("xpath=//li[@id='pagenavsearchbox_popup2']")));
			 * AssertJUnit.assertTrue("FAIL: First child page in type ahead field is missing",
			 * Data.New_Page_To_Verify_Page_Search4 .equals(sel.getText("xpath=//li[@id='pagenavsearchbox_popup3']")));
			 */
		}
		else if (TypeAheadValue.equals("Welc")) {
			AssertJUnit.assertTrue("FAIL: First peer page in type ahead field is missing", WikisObjects.Welcome_Homepage.equals(sel
					.getText("xpath=//li[@id='pagenavsearchbox_popup0']")));
			AssertJUnit.assertTrue("FAIL: Only 1 page should appear in typeahead dropdown", !sel.isElementPresent("xpath=//li[@id='pagenavsearchbox_popup1']"));
		}
	...
	public void SelectItemFromPageSearchTypeAheadDropdown(String TypeAheadTitle, String ItemID) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: typeahead item doesn't exist", sel.isElementPresent(ItemID));
	...
	public void VerifyItemFromPageSearchTypeAheadDropdownOpens(String TypeAheadTitle) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Title isn't visible", sel.isTextPresent(TypeAheadTitle));
		AssertJUnit.assertTrue("FAIL: Wiki homepage title isn't visible", sel.isElementPresent(WikisObjects.WikiHomePageTitleField));
		AssertJUnit.assertTrue("FAIL: Type ahead title doesn't match wiki homepage title", TypeAheadTitle.equals(sel.getText(WikisObjects.WikiHomePageTitleField)));
	}
	...
	public void VerifyMembersPageUI(String Role) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Next Page Link at the Top of Members List is not Present", sel.isElementPresent(WikisObjects.Top_Next_Page_Link_InActive));
		AssertJUnit.assertTrue("FAIL: Previous Page Link at the Top of Members List is not Present", sel.isElementPresent(WikisObjects.Top_Previous_Page_Link_InActive));
		AssertJUnit.assertTrue("FAIL: Next Page Link at the Bottom of Members List is not Present", sel.isElementPresent(WikisObjects.Bottom_Next_Page_Link_InActive));
		AssertJUnit.assertTrue("FAIL: Previous Page Link at the Bottom of Members List is not Present", sel.isElementPresent(WikisObjects.Bottom_Previous_Page_Link_InActive));
		AssertJUnit.assertTrue("FAIL: Page Display Choice Option is not displayed.", sel.isElementPresent(WikisObjects.PageDisplay_Choice_Option));

		if (sel.isElementPresent(WikisObjects.PressedListIcon))
			AssertJUnit.assertTrue("FAIL: Table Display Option is missing", sel.isElementPresent(WikisObjects.UnPressedTableIcon));
		else AssertJUnit.assertTrue("FAIL: List Display Option is missing", sel.isElementPresent(WikisObjects.UnPressedListIcon));

		AssertJUnit.assertTrue("FAIL: Id for 10 Members per Page is missing", sel.isElementPresent(WikisObjects.PageID_Showing_16Members_perpage));
		AssertJUnit.assertTrue("FAIL: Link for 25 Members per Page is missing", sel.isElementPresent(WikisObjects.PageID_Showing_32Members_perpage));
		AssertJUnit.assertTrue("FAIL: Link for 50 Members per Page is missing", sel.isElementPresent(WikisObjects.PageID_Showing_64Members_perpage));

		//Verify Member Role section
		AssertJUnit.assertTrue("FAIL: Member Role heading is missing", sel.isTextPresent(WikisObjects.Member_Role_Heading));
		AssertJUnit.assertTrue("FAIL: Member Role Expand and Collapse Menu is missing", sel.isElementPresent(WikisObjects.Members_Role_Expand_Collapse_Arrow));
		AssertJUnit.assertTrue("FAIL: Tooltip for Member Role Expand and Collapse Menu is missing", sel.isElementPresent(WikisObjects.Tooltip_For_MemberRole_Field));
		AssertJUnit.assertTrue("FAIL: Link to filter members by reader is missing", sel.isElementPresent(WikisObjects.Reader_Filter_Link));
		AssertJUnit.assertTrue("FAIL: Link to filter members by editor is missing", sel.isElementPresent(WikisObjects.Editor_Filter_Link));
		AssertJUnit.assertTrue("FAIL: Link to filter members by manager is missing", sel.isElementPresent(WikisObjects.Owner_Filter_Link));

		//Verify Member Kind section
		AssertJUnit.assertTrue("FAIL: Member Kind Heading is missing", sel.isTextPresent(WikisObjects.Member_Kind_Heading));
		AssertJUnit.assertTrue("FAIL: Member Kind Expand and Collapse Menu is missing", sel.isElementPresent(WikisObjects.Members_Kind_Expand_Collapse_Arrow));
		AssertJUnit.assertTrue("FAIL: Tooltip for Member Kind Expand and Collapse Menu is missing", sel.isElementPresent(WikisObjects.Tooltip_For_MemberKind_Field));
		AssertJUnit.assertTrue("FAIL: Link to Filter Members by Person is missing", sel.isElementPresent(WikisObjects.Person_Filter_Link));
		AssertJUnit.assertTrue("FAIL: Link to Filter Members by Groups is missing", sel.isElementPresent(WikisObjects.Group_Filter_Link));

		if (Role.equals(Data.Owner_Role)) {
			AssertJUnit.assertTrue("FAIL: Add Members Button is missing", sel.isElementPresent(WikisObjects.Add_Members_Button));
			AssertJUnit.assertTrue("FAIL: Remove Members Button is missing", sel.isElementPresent(WikisObjects.Remove_Members_Button));
			AssertJUnit.assertTrue("FAIL: Manage Access Button is missing", sel.isElementPresent(WikisObjects.Manage_Access_Button));
		}
		else {
			AssertJUnit.assertFalse("FAIL: Add Members Button shouldn't appear for user with less than owner access", sel.isElementPresent(WikisObjects.Add_Members_Button));
			AssertJUnit.assertFalse("FAIL: Remove Members Button shouldn't appear for user with less than owner access", sel.isElementPresent(WikisObjects.Remove_Members_Button));
			AssertJUnit.assertFalse("FAIL: Manage Access Button shouldn't appear for user with less than owner access", sel.isElementPresent(WikisObjects.Manage_Access_Button));
			AssertJUnit.assertFalse("FAIL: Edit Link  shouldn't appear for user with less than owner access", sel.isElementPresent(WikisObjects.Edit_Link));
		}
	}
	...
	public void verifyAddMembersDialogUI() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Members field is missing", sel.isElementPresent(WikisObjects.Members_Field));
		...
		AssertJUnit.assertTrue("FAIL: Wiki_Membership_Roles_dropdown list for Users is missing", sel.isElementPresent(WikisObjects.Add_Members_Dropdown));
		String MembershipRolesforUsers_array[] = sel.getSelectOptions(WikisObjects.Add_Members_Dropdown);
		AssertJUnit.assertTrue("FAIL: Some Membership roles for Users are missing", 3 == MembershipRolesforUsers_array.length);

		AssertJUnit.assertTrue("FAIL: First membership role isn't Owner for Users", Data.Owner_Role.equals(MembershipRolesforUsers_array[0]));
		AssertJUnit.assertTrue("FAIL: Second membership role isn't Editor for Users", Data.Editor_Role.equals(MembershipRolesforUsers_array[1]));
		AssertJUnit.assertTrue("FAIL: Third membership role isn't Reader for Users", Data.Reader_Role.equals(MembershipRolesforUsers_array[2]));

		AssertJUnit.assertTrue("FAIL: Wiki_Membership_Roles_dropdown list for Groups is missing", sel.isElementPresent(WikisObjects.Add_Groups_Dropdown));
		String MembershipRolesforGroups_array[] = sel.getSelectOptions(WikisObjects.Add_Groups_Dropdown);
		AssertJUnit.assertTrue("FAIL: Some Membership roles for Groups are missing", 3 == MembershipRolesforGroups_array.length);

		AssertJUnit.assertTrue("FAIL: First Membership Role isn't Owner for Groups", Data.Owner_Role.equals(MembershipRolesforGroups_array[0]));
		AssertJUnit.assertTrue("FAIL: Second Membership Role isn't Editor for Groups", Data.Editor_Role.equals(MembershipRolesforGroups_array[1]));
		AssertJUnit.assertTrue("FAIL: Third Membership Role isn't Reader for Groups", Data.Reader_Role.equals(MembershipRolesforGroups_array[2]));
	}
	...
	public int getNumberOfUsersDisplayedOnMembersPage() throws Exception {
		...
		//Old code
		/*
		 * while(!(sel.getText("xpath=//div[@id='list']/div[2]/table/tbody/tr[" +index1+ "]/td[" +index2+
		 * "]/table/tbody/tr/td[2]/h4").equals(""))){ index2++;
		 * 
		 * if (index2 > 4){ index1++; index2 = 1; } noOfUsers++; } return noOfUsers;
		 */
	}
	...
	public void VerifyRemoveMemberConfirmation() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Remove Members title is incorrect", sel.isTextPresent("Remove Members"));
		AssertJUnit.assertTrue("FAIL: Remove Members text content is incorrect", sel.isTextPresent("Are you sure you want to remove selected members"));
		AssertJUnit.assertTrue("FAIL: OK button is missing", sel.isElementPresent(WikisObjects.OK_Button));
		AssertJUnit.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void setNewRoleOnEditMembersLightBox(String NewRole) throws Exception {
		...
		else if (NewRole.equals("Reader")) clickLink(WikisObjects.Reader_Role_Option_onEditMemberLightBox);
	...
	public boolean verifyNewRoleAssignedToUserOnMembersPage(String UserFullName, String NewRole) throws Exception {
		...
		while (index1 < 3) {
			...
			if ((UsernameText.contains(UserFullName)) || (UsernameText.equals(UserFullName))) {
				...
				AssertJUnit.assertTrue("FAIL: New Role appears incorrectly on Members page", UserTypeText.equals(NewRole));
	...
	public boolean VerifyRadioButtonIsCheckedForGivenRole(String Role) throws Exception {
		...
		if ((Role.equals("Owner")) && (sel.isChecked(WikisObjects.Owner_Role_Option_onEditMemberLightBox)))
			...
			radioSelected = true;
		else if ((Role.equals("Reader")) && (sel.isChecked(WikisObjects.Reader_Role_Option_onEditMemberLightBox))) radioSelected = true;
	...
	public void OpenVersionView(int VersionsVisible) throws Exception {
		...
		if (VersionsVisible > 10) {
			AssertJUnit.assertTrue("FAIL: pagination for versions is missing", (sel.isElementPresent(WikisObjects.Versions_Top_Pagination_Section))
					& (sel.isElementPresent(WikisObjects.Versions_Bottom_Pagination_Section)));

			if (ActivePage.equals("2")) clickLink("link=" + ActivePage);

			VersionsVisible = VersionsVisible - 10;

			AssertJUnit.assertTrue("FAIL: Version view link isn't visible", sel.isElementPresent("xpath=//div[@id='pageVersions']/div[2]/table/tbody/tr[" + VersionsVisible
					+ "]/td[4]/a"));
			...
			AssertJUnit.assertFalse("FAIL: pagination for versions shouldn't appear for less than 10 versions", (sel.isVisible(WikisObjects.Versions_Top_Pagination_Section))
		...
		else Assert.fail("FAIL: Version view link is inactive because there is only 1 version");
	...
	public void ValidateVersionsPage(int VersionsValue, String Role) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Versions Information is missing", sel.isElementPresent(WikisObjects.Versions_Complete_Info_Body));
		...
		AssertJUnit.assertTrue("FAIL: Subscribe to pages link is missing", sel.isElementPresent(WikisObjects.Subscribe_To_This_Page_Link));
		AssertJUnit.assertTrue("FAIL: Subscribe to page versions link is missing", sel.isElementPresent(WikisObjects.Subscribe_To_These_Page_Versions_Link));
	}
	...
	public void ValidateVersionLinks(int verValue, String UserRole) throws Exception {
		...
		if (verValue == 1) {
			AssertJUnit.assertTrue("FAIL: View link shouldn't be visible", !sel.isElementPresent(WikisObjects.Versions_View_Link));
			AssertJUnit.assertTrue("FAIL: Restore link shouldn't be visible", !sel.isElementPresent(WikisObjects.Versions_Restore_Link));
			AssertJUnit.assertTrue("FAIL: Current Version text is missing", sel.isTextPresent(WikisObjects.Versions_Current_Version_Text));
		}
		else {
			AssertJUnit.assertTrue("FAIL: View link isn't visible", sel.isElementPresent(WikisObjects.Versions_View_Link));

			if ((UserRole.equals(Data.Creator_Role)) || (UserRole.equals(Data.Owner_Role)) || (UserRole.equals(Data.Editor_Role)))
				AssertJUnit.assertTrue("FAIL: Restore link isn't visible", sel.isElementPresent(WikisObjects.Versions_Restore_Link));

			else AssertJUnit.assertTrue("FAIL: Restore link shouldn't be visible", !sel.isElementPresent(WikisObjects.Versions_Restore_Link));
		}
	}
	...
	public void MatchVersionsHeadingValueToContent() throws Exception {
		...
		if (sel.isElementPresent(WikisObjects.Versions_Active_Page)) {
			...
			if ((Versions_Value > 10) && (ActivePage.equals("2"))) {
				AssertJUnit.assertTrue("FAIL: Versions pagination is missing", (sel.isElementPresent(WikisObjects.Versions_Top_Pagination_Section))
						& (sel.isElementPresent(WikisObjects.Versions_Bottom_Pagination_Section)));
		...
		while (sel.isElementPresent("css=table[class='lotusTable lotusClear'] tr:nth-child(" + index + ")")) {
			AssertJUnit.assertTrue("FAIL: Versions aren't visible in page", sel.isElementPresent("css=table[class='lotusTable lotusClear'] tr:nth-child(" + index + ")"));
			index++;
			Versions_Visible++;
		}
		...
		AssertJUnit.assertTrue("FAIL: Versions indicated aren't visible", Versions_Value == Versions_Visible);
	}
	...
	public void ValidateVersionViewUI(String NewContentText) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Details field in Version View UI is missing", sel.isElementPresent(WikisObjects.Versions_View_Details_Field));
		...
		if (sel.isVisible(WikisObjects.Versions_View_First_Link)) {
			...
			if (!LinkText.equals(WikisObjects.Versions_View_CompareToMostRecent))
				AssertJUnit.assertTrue("FAIL: Restore link in Version View UI is missing", LinkText.equals(WikisObjects.Restore_Versions_Text));
		}

		AssertJUnit.assertTrue("FAIL: Compare to most recent link in Version View UI is missing", sel.isElementPresent(WikisObjects.Versions_View_CompareToMostRecent_Link));
		AssertJUnit.assertTrue("FAIL: View most recent link in Version View UI is missing", sel.isElementPresent(WikisObjects.Versions_View_ViewMostRecent_Link));

		AssertJUnit.assertTrue("FAIL: added content is missing", sel.isElementPresent(WikisObjects.Versions_View_Textfield));
		ViewText = sel.getText(WikisObjects.Versions_View_Textfield);
		AssertJUnit.assertTrue("FAIL: text in view doesn't match text in first version", ViewText.contains(NewContentText));
	}

	//Verify Version title is correct
	public void ValidateVersionViewTitle(String pageTitle, int versionValue) throws Exception {

		AssertJUnit.assertTrue("FAIL: Versions View title is incorrect", sel.isTextPresent(pageTitle + " [" + versionValue + "] : Versions"));
	}
	...
	public void SelectShowComparison() throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Show Comparison link isn't visible", sel.isElementPresent(WikisObjects.Versions_Show_Comparison_Link));
	...
	public void ValidateVersionViewLinkChange(int index) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: View link should have changed to 'Current Version'", TextStatus.equals(WikisObjects.Versions_Current_Version_Text));
	}
	...
	public String GetOptionFromCompareDropdown1(String Position) throws Exception {
		...
		else Selection1a = Dropdown1_Options[Dropdown1_Options.length - 2];
	...
	public String GetOptionFromCompareDropdown2(String Position) throws Exception {
		...
		else Selection2a = Dropdown2_Options[Dropdown2_Options.length - 2];
	...
	public void ValidateVersionComparisonPage(String WikiTitle, String ValueFromDropdown1, String ValueFromDropdown2) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Title text isn't correct", ComparisonPageTitle.equals(WikiTitle + " : Version Comparison"));

		AssertJUnit.assertTrue("FAIL: Heading doesn't exist", sel.isTextPresent(WikisObjects.Versions_Comparison_Page_Heading));
		AssertJUnit.assertTrue("FAIL: Comparison section 1 is missing", sel.isElementPresent(WikisObjects.Versions_Comparison_Page_Version_Section1));
		AssertJUnit.assertTrue("FAIL: Comparison section 2 is missing", sel.isElementPresent(WikisObjects.Versions_Comparison_Page_Version_Section2));
		AssertJUnit.assertTrue("FAIL: Refresh link is missing", sel.isElementPresent(WikisObjects.Versions_Comparison_Page_Refresh_Section));
		AssertJUnit.assertTrue("FAIL: Refresh icon is missing", sel.isElementPresent(WikisObjects.Versions_Comparison_Page_Refresh_Icon));
		AssertJUnit.assertTrue("FAIL: Comparison Deleted text is missing", sel.isTextPresent(WikisObjects.Versions_Comparison_Page_Deleted_Text));
		AssertJUnit.assertTrue("FAIL: Comparison New/Changed text is missing", sel.isTextPresent(WikisObjects.Versions_Comparison_Page_NewChanged_Text));
		...
		AssertJUnit.assertTrue("FAIL: Item in Version Comparison page dropdown 1 is incorrect", x.equals(ValueFromDropdown1));
		AssertJUnit.assertTrue("FAIL: Item in Version Comparison page dropdown 2 is incorrect", y.equals(ValueFromDropdown2));
	}
	...
	public void RestorePriorVersions(String RestoreNumber) throws Exception {
		...
		else Assert.fail("FAIL: cannot locate the version specified");
	}

	//Verify version hasn't been restored
	public void ValidateVersionsNotRestored(String RestoredVersion) throws Exception {

		AssertJUnit.assertTrue("FAIL: version restored text appears after cancel", !sel.isTextPresent("Restored from version " + RestoredVersion));
	}
	...
	public void ValidateVersionsAreRestored(String RestoredVersion) throws Exception {
		...
		Thread.sleep(2000);
		AssertJUnit.assertTrue("FAIL: version restored text doesn't appear", sel.isTextPresent("Restored from version " + RestoredVersion));
	}
	...
	public void ValidateVersionRestorePopup(String RestoreVersionNumber, String DateDetails, String UsernameDetails) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Restore confirmation text in popup isn't correct", sel
				.isTextPresent("You are about to replace the current version of this page with version " + RestoreVersionNumber + ", which was created " + DateDetails + " by "
						+ UsernameDetails + ". Do you want to proceed?"));
		String Restore_Header = sel.getText(WikisObjects.Lightbox_Header);

		AssertJUnit.assertTrue("FAIL: Restore Confirmation header is incorrect", Restore_Header.contains("Confirm"));
		AssertJUnit.assertTrue("FAIL: Restore button is missing", sel.isElementPresent(WikisObjects.OK_Button));
		AssertJUnit.assertTrue("FAIL: Cancel link is missing", sel.isElementPresent(WikisObjects.Cancel_Link));
	}
	...
	public void ValidateVersionDeletionPopup(String VersionNumber) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: confirmation popup heading is incorrect", sel.isTextPresent("Confirm"));
		AssertJUnit.assertTrue("FAIL: confirmation popup is missing", sel.isTextPresent("Are you sure you want to delete all versions prior to " + VersionNumber + "?"));
	}
	...
	public void ValidateVersionsStillVisible(int VersionsVisible) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Version has been incorrectly removed", sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr[" + VersionsVisible + "]/td[1]"));
	}
	...
	public void ValidateVersionsAreRemoved(int VersionsVisible) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Version hasn't been removed", !sel.isElementPresent("xpath=id('pageVersions')/div[2]/table/tbody/tr[" + VersionsVisible + "]/td[1]"));
	}
	...
	public void ValidateVersionDeletionUI(int versionValue, String UserRole) throws Exception {
		...
		if ((UserRole.equals(Data.Owner_Role)) && (versionValue > 1)) {
			AssertJUnit.assertTrue("FAIL: deletion text isn't visible", sel.isTextPresent(WikisObjects.Versions_Deletion_Text));
			AssertJUnit.assertTrue("FAIL: deletion dropdown isn't visible", sel.isElementPresent(WikisObjects.Versions_Delete_Dropdown));
			AssertJUnit.assertTrue("FAIL: Delete link isn't visible", sel.isElementPresent(WikisObjects.Delete_Link));
		}

		else {
			AssertJUnit.assertTrue("FAIL: deletion section shouldn't be visible for user with less than Editor access", !sel.isVisible(WikisObjects.Versions_Deletion_Section));
			AssertJUnit.assertTrue("FAIL: deletion dropdown shouldn't be visible for user with less than Editor access", !sel.isVisible(WikisObjects.Versions_Delete_Dropdown));
			AssertJUnit.assertTrue("FAIL: Delete link shouldn't be visible for user with less than Editor access", !sel.isVisible(WikisObjects.Delete_Link));
		}
	}
	...
	public void ValidateVersionComparisonUI(int NumberOfVersions) throws Exception {
		...
		if (NumberOfVersions > 1) {
			//Assert that comparison elements exists in Versions page
			AssertJUnit.assertTrue("FAIL: Some comparison text is missing", sel.isTextPresent(WikisObjects.Versions_Compare_Text));
			AssertJUnit.assertTrue("FAIL: First comparison dropdown is missing", sel.isVisible(WikisObjects.Versions_Comparison_Dropdown1));
			AssertJUnit.assertTrue("FAIL: Some comparison text is missing", sel.isTextPresent(WikisObjects.Versions_To_Text));
			AssertJUnit.assertTrue("FAIL: Second comparison dropdown is missing", sel.isVisible(WikisObjects.Versions_Comparison_Dropdown2));
			AssertJUnit.assertTrue("FAIL: Show Comparison link is missing", sel.isVisible(WikisObjects.Versions_Show_Comparison_Link));
		}
		else {
			AssertJUnit.assertTrue("FAIL: First comparison dropdown shouldn't appear", !sel.isVisible(WikisObjects.Versions_Comparison_Dropdown1));
			AssertJUnit.assertTrue("FAIL: Second comparison dropdown shouldn't appear", !sel.isVisible(WikisObjects.Versions_Comparison_Dropdown2));
			AssertJUnit.assertTrue("FAIL: Show Comparison link shouldn't appear", !sel.isVisible(WikisObjects.Versions_Show_Comparison_Link));
		}
	}
	...
	public void VerifyPageAddedToNavTree(String NewPageExists) throws InterruptedException {
		...
		AssertJUnit.assertTrue("FAIL: " + NewPageExists + " hasn't been created successfully", sel.isTextPresent(NewPageExists));
	}
	...
	public void ValidatePagePositionInNav(String PageName, String PageInReorder) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Page at specified nav position doesn't correspond to page expected", ("* " + PageName).equals(sel.getText(PageInReorder)));
	}
	...
	public void ValidateNewlyAddedComment(String New_Comment, int index) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: new comment text is missing", sel.isTextPresent(New_Comment));
		AssertJUnit.assertTrue("FAIL: new field for comment is missing", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div[" + index + "]/div[2]"));
		AssertJUnit.assertTrue("FAIL: Edit link is missing for comment", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div[" + index + "]/div[2]/div[2]/div/ul/li/a"));
		AssertJUnit.assertTrue("FAIL: Delete icon is missing for comment", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div[" + index + "]/div[2]/div[2]/a/img"));
	}
	...
	public void ValidateCommentsAreAdded(int PreviousCommentsTotal, int NewCommentCount) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: on comments being added", NewCommentCount == (PreviousCommentsTotal + 1));
	}
	...
	public void ValidateEditedComment(String EditedComment, int index) throws Exception {
		...
		AssertJUnit.assertTrue("FAIL: Comment hasn't been edited correctly", NewComment.equals(EditedComment));
		AssertJUnit.assertTrue("FAIL: Edited comment indicator is missing", sel.isElementPresent("xpath=id('wikiPageComments')/div[2]/div[" + index + "]/span"));
		String CommentIndicator = sel.getText("xpath=id('wikiPageComments')/div[2]/div[" + index + "]/span");
		AssertJUnit.assertTrue("FAIL: Edited comment indicator is missing", CommentIndicator.contains("Edited today at "));
	}
	...
	public void verifyTagWidget(String tag) throws Exception {
		...
		if (sel.isElementPresent(WikisObjects.Link_for_Tag_List)) {
			...
			AssertJUnit.assertTrue("FAIL: Tag(s) are missing from Tags list", sel.isElementPresent("link=" + tag));
		}

		if (sel.isElementPresent(WikisObjects.Link_for_Tag_Cloud)) {
			clickLink(WikisObjects.Link_for_Tag_Cloud);

			AssertJUnit.assertTrue("FAIL: Tag(s) are missing from Cloud list", sel.isElementPresent("link=" + tag));
		}
	}
	...
	public void verifyTagWidget(String[] tags, int[] tagCount) throws Exception {
		...
		if (sel.isElementPresent(WikisObjects.Link_for_Tag_List)) {
			...
			for (int i = 0; i < tags.length; i++)
				AssertJUnit.assertTrue("FAIL: Tag(s) are missing from Tags list", sel.isElementPresent("css=ul[id='lconnTagWidget_tagList'] [title='Filter by the tag " + tags[i]
						+ " with count " + tagCount[i] + "']"));
		}
		...
		if (sel.isElementPresent(WikisObjects.Link_for_Tag_Cloud)) {
			...
			for (int i = 0; i < tags.length; i++)
				AssertJUnit.assertTrue("FAIL: Tag(s) are missing from Cloud list", sel.isElementPresent("css=div[class='lotusTagCloud'] [title='Filter by the tag " + tags[i]
						+ " with count " + tagCount[i] + "']"));
		}
	}
	...
	public void RecommendCurrentPage() throws Exception {
		...
		else clickLink(WikisObjects.Recommendations_Info);
	...
	public void verifyRecommendationIconIsUpdated(int pageRecommends) throws Exception {
		...
		while (index <= 5) {
			if (sel.isElementPresent("xpath=id('list')/div[2]/table/tbody/tr[" + index + "]/td[4]/a")) {
				...
				AssertJUnit.assertTrue("FAIL: Recommendation Icon in Pages Index hasn't been updated", recommendsValueText.contains(Integer.toString(pageRecommends)));
			}
